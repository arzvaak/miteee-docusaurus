...
const subjectRoot = path.join(process.cwd(), "docs", "sem7", "psps");
...
test("SEM7 PSPS provides an overview and eight complete weekly notes", () => {
  ...
  for (let week = 1; week <= 8; week += 1) {
    ...
    assert.ok(metric.images >= 8, `Week ${week} has only ${metric.images} source-image placements`);
    assert.match(content, /## Practice Quiz/);
    assert.match(content, /## Source Provenance/);
  }
});

test("SEM7 PSPS local note and image links resolve", () => {
  const files = [path.join(subjectRoot, "overview.md"), ...Array.from({ length: 8 }, (_, index) => weekPath(index + 1))];
...
test("SEM7 PSPS KaTeX expressions compile", () => {
  let total = 0;
  for (let week = 1; week <= 8; week += 1) {
    const content = fs.readFileSync(weekPath(week), "utf8");
...
test("PSPS generation pipeline pins the requested current models and maximum output allowance", () => {
  const pipeline = fs.readFileSync(path.join(process.cwd(), "scripts", "psps_course_pipeline.py"), "utf8");
