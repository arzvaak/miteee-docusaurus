...
The normal production Compose stack starts the pinned DeepTutor image and persists its state under `data/deeptutor/`. I use the server-side Mistral secret for tutor responses, while a private local Ollama `all-minilm` profile builds the note embeddings without depending on paid embedding credits. The local profile raises the model's packaged 256-token default to its supported 512-token context, and DeepTutor uses 510-token text chunks so the model's two framing tokens also fit without truncation. Ollama has no published host port, is capped at 2 CPU and 512 MB RAM, and unloads the model after five idle minutes. The CPU allowance is used only while an index is being built; idle use stays near zero. To rebuild the generated `miteee-notes` knowledge base locally after changing `docs/**/*.md`:
