...
$$
frac(partial P_i, partial delta_i) = sum_(k=1)^(n) |V_i| |V_k| |Y_("ik")| sin(theta_("ik") - delta_i + delta_k) - |V_i|^2 |Y_("ii")| sin theta_("ii")
$$
...
- $theta_("ik")$: angle of the admittance $Y_("ik")$
...
$$
frac(partial P_i, partial delta_i) = -Q_i - |V_i|^2 |Y_("ii")| sin theta_("ii")
$$

Using the convention $Y = G + j B$ (where $G$ is conductance, $B$ is susceptance), we have:

$$
|Y_("ii")| sin theta_("ii") = B_("ii")
$$
...
**Approximation 2:** $|V_i|^2 tilde.eq |V_i|$ because voltages are very close to $1.0$ p.u. (the slack bus voltage is taken as 1).
...
$$
theta_("ik") - delta_i + delta_k approx theta_("ik")
$$

In the expression for the off-diagonal elements of $J_1$, we substitute:

$$
sin(theta_("ik") - delta_i + delta_k) approx sin(theta_("ik"))
$$
...
where $|Y_("ik")| sin(theta_("ik")) = B_("ik")$.
...
$$
frac(partial Q_i, partial V_i) = -2|V_i||Y_("ii")|sin theta_("ii") - sum_(k=1,k!= i)^(n)|V_i||V_k||Y_("ik")|sin(theta_("ik")-delta_i+delta_k)
$$
...
$$
Q_i = -sum_(k=1,k!= i)^(n)|V_i||V_k||Y_("ik")|sin(theta_("ik")-delta_i+delta_k)
$$

Substituting into equation 69:

$$
frac(partial Q_i, partial V_i) = -|V_i||Y_("ii")| sin theta_("ii") + Q_i
$$

Since $|Y_("ii")| sin theta_("ii") = B_("ii")$:

$$
frac(partial Q_i, partial V_i) = -|V_i| B_("ii") + Q_i
$$

With the assumption $B_("ii") ≫ Q_i$ (both in per unit), $Q_i$ may be neglected:
...
Assuming again $theta_("ik") - delta_i + delta_k approx theta_("ik")$, the off-diagonal elements of $J_4$ can be written as:
...
2.  **Small angular differences:** $delta_k - delta_i$ is small, allowing $sin(theta_("ik") - delta_i + delta_k) approx sin(theta_("ik"))$.
3.  **Flat voltage profile:** $|V_i| approx 1.0$ p.u., justifying $|V_i|^2 tilde.eq |V_i|$.
4.  **$B_("ii") ≫ Q_i$:** The shunt susceptance dominates the reactive power injection.
...
$$
I_q = -frac(1, a)(y_("pq") V_p - frac(y_("pq"), a) V_q)
$$
...
$$
mat(I_p; I_q) = mat(y_("pq"), -frac(y_("pq"), a); -frac(y_("pq"), a), frac(y_("pq"), a^2)) mat(V_p; V_q)
$$
...
$$
Y_("pp") = y_("pq") = frac(y_("pq"), a) + (frac(a-1, a)) y_("pq")
$$
...
$$
Y_("qq") = frac(y_("pq"), a^2) = frac(y_("pq"), a) + (frac(1-a, a^2)) y_("pq")
$$
...
- The shunt branch on the $P$ side is $(frac(a-1, a)) y_("pq")$
- The shunt branch on the $Q$ side is $(frac(1-a, a^2)) y_("pq")$
...
- Shunt branches: $(frac(a-1, a)) y_("pq")$ on one side and $(frac(1-a, a^2)) y_("pq")$ on the other side.
- Line charging admittance may also be present.
- If $a = 1$: simply $y_("pq")$.
- If $a != 1$: the shunt branches must be considered.
...
1.  **Units consistency for $B_("ii") ≫ Q_i$:** Both $Q_i$ and $B_("ii")$ must be in per unit values for this approximation to hold.
2.  **$|V_i|^2 tilde.eq |V_i|$ approximation:** Valid only because voltages are close to 1.0 p.u. (slack bus voltage taken as 1).
...
4.  **Transformer π-equivalent:** When $a = 1$, shunt branches vanish; when $a != 1$, shunt branches must be included.

### 36.9 Lecture-End Recap

- The FDLF method decouples the P-δ and Q-V problems, using constant matrices $B'$ and $B''$.
- The key approximations are: $B_("ii") ≫ Q_i$, $|V_i|^2 tilde.eq |V_i|$, and small angular differences.
...
$$
Delta V = mat(Delta V_2; Delta V_3)
$$
...
$$
Delta Q = mat(Delta Q_3; Delta Q_4)
$$
...
- Bus 2 has a load $P_(L 2) + j Q_(L 2)$.
- A shunt capacitor $Q_(C 2)$ is connected at bus 2.
- The relationship: $Q_2 = Q_(C 2) - Q_(L 2)$.
- Therefore: $Q_(C 2) = Q_2 + Q_(L 2)$.

**Procedure:**
1.  Run load flow with P bus and PQV bus.
2.  Obtain $Q_2$ from converged solution.
3.  Calculate required shunt capacitance: $Q_(C 2) = Q_2 + Q_(L 2)$.
...
- $"MWh" = 100 times 1 = 100$ MWh
- Heat rate = 0.86 MKcal/MWhr
- Heat input = $0.86 times 100 = 86$ MKcal
...
$$
C_i(P_("gi")) = k F_i(P_("gi")) = k P_("gi") dot H_i(P_("gi"))
$$
...
$$
frac(d C_i, d P_("gi")) = "IC"_i = b_i + 2 d_i P_("gi")
$$
...
c) Find incremental cost $frac(d C_i, d P_("gi"))$
...
2.  **$Q_2$ calculation:** After load flow converges, $Q_(C 2) = Q_2 + Q_(L 2)$ (must add the load reactive power to the injection).
...
- After convergence, the required shunt capacitance at the P bus is calculated as $Q_(C 2) = Q_2 + Q_(L 2)$.
...
**Data points:**
- 25% of $P_g = frac(25, 100) times 50 = 12.5$ MW, $H(P_g) = 10$ MKcal/MWhr
- 40% of $P_g = frac(40, 100) times 50 = 20$ MW, $H(P_g) = 8.6$ MKcal/MWhr
- 100% of $P_g = 50$ MW, $H(P_g) = 8$ MKcal/MWhr

**Three linear equations:**

$$
frac(alpha', 12.5) + beta' + 12.5 gamma' = 10 space "(Equation 2)"
$$

$$
frac(alpha', 20) + beta' + 20 gamma' = 8.6 space "(Equation 3)"
$$

$$
frac(alpha', 50) + beta' + 50 gamma' = 8 space "(Equation 4)"
$$
...
$$
alpha' = 55.56, space beta' = 5.11, space gamma' = 0.0355
$$

**Cost of fuel:** $k = 4$ Rs/MKcal

**Derived coefficients:**
- $a = k alpha' = 4 times 55.56 = 222.24$
- $b = k beta' = 4 times 5.11 = 20.44$
- $d = k gamma' = 4 times 0.0355 = 0.142$
...
$$
C(P_g) = 222.24 + 20.44 times 12.5 + 0.142 times (12.5)^2 = 500 " Rs/hr"
$$

**At 40% rating ($P_g = 20$ MW):**

$$
C(P_g) = 222.24 + 20.44 times 20 + 0.142 times (20)^2 = 688 " Rs/hr"
$$

**At 100% rating ($P_g = 50$ MW):**

$$
C(P_g) = 222.24 + 20.44 times 50 + 0.142 times (50)^2 = 1599 " Rs/hr"
$$

#### 38.2.4 Incremental Cost

$$
frac(d C, d P_g) = 20.44 + 0.284 P_g " Rs/hr"
$$

**At 100% rating ($P_g = 50$ MW):**

$$
"IC" = 20.44 + 0.284 times 50 = 34.64 " Rs/hr"
$$

**Exact cost for 51 MW:**

$$
C(P_g = 51) = 222.24 + 20.44 times 51 + 0.142 times (51)^2 = 1634 " Rs/hr"
$$
...
Consider a system with $m$ generators committed and all loads $"PL"_i$ given.

**Objective:** Find $P_("gi")$ and voltage magnitude $|V_i|$ for $i = 1, 2, dots.h, m$ to minimize total fuel cost.
...
1.  **Generator power limits:**
    $$
P_("gi")^(min) <= P_("gi") <= P_("gi")^(max) space "for " i = 1,2,dots.h,m
$$

2.  **Voltage magnitude limits:**
    $$
|V_i|^(min) <= |V_i| <= |V_i|^(max) space "for " i = 1,2,dots.h,m
$$

3.  **Line power flow limits:**
    $$
|P_("ij")| <= |P_("ij")|^(max) space "for all lines"
$$
...
$$
sum_(i=1)^(m) P_("gi") = "PL" = sum_(i=1)^(n) "PL"_i
$$
...
$$
w i det i l d e(C_T) = C_T - lambda(sum_(i=1)^(m) P_("gi") - P_L)
$$

Or:

$$
w i det i l d e(C_T) = sum_(i=1)^(m) C_i(P_("gi")) - lambda(sum_(i=1)^(m) P_("gi") - P_L)
$$
...
$$
frac(d w i det i l d e(C_T), d P_("gi")) = 0
$$

This gives:

$$
frac(d C_i, d P_("gi")) - lambda = 0
$$

Therefore:

$$
frac(d C_i, d P_("gi")) = lambda space "for " i = 1,2,dots.h,m
$$
...
$frac(d C_i, d P_("gi")) = "IC"_i$ is called the **incremental cost** of the $i$-th generator.

Equation 13 can be written as:

$$
frac(d C_1, d P_(g 1)) = frac(d C_2, d P_(g 2)) = frac(d C_3, d P_(g 3)) = dots.h = frac(d C_m, d P_("gm")) = lambda
$$
...
**Step 1:** Choose an initial value of $lambda$ (i.e., $"IC" = "IC"_0$)

**Step 2:** Solve for $P_("gi")$ using equation 14 (coordination equation)

**Step 3:** Check convergence:
$$
|sum_(i=1)^m P_("gi") - P_L| < epsilon
$$
...
- If $(sum_(i=1)^m P_("gi") - P_L) < 0$: increase $"IC" = "IC"_0 + Delta "IC"$ (increase generation)
- If $(sum_(i=1)^m P_("gi") - P_L) > 0$: decrease $"IC" = "IC"_0 - Delta "IC"$ (decrease generation)
...
$$
C_T = sum_(i=1)^(m) [ C_i(P_("gi")^0) + frac(d C_i(P_("gi")^0), d P_("gi")) Delta P_("gi") ]
$$
...
**Since $lambda = frac(d C_i(P_("gi")^0), d P_("gi"))$ (Equation 17):**
...
- Three incremental cost curves: $"IC"_1$, $"IC"_2$, $"IC"_3$.
...
3.  **First limit reached:** At $lambda_2$, generator 3 reaches $P_(g 3)^(max)$. Set $P_(g 3) = P_(g 3)^(max)$ (cannot increase further).
4.  **Further load increase:** Generators 1 and 2 operate on equal-λ basis (e.g., $lambda_3$); generator 3 stays fixed at $P_(g 3)^(max)$.
5.  **Second limit reached:** At $lambda_4$, generator 2 reaches $P_(g 2)^(max)$. Set $P_(g 2) = P_(g 2)^(max)$.
6.  **Further load increase:** Only generator 1 takes the load until it reaches $P_(g 1)^(max)$.

**Maximum total load (losses neglected):** $P_(g 1)^(max) + P_(g 2)^(max) + P_(g 3)^(max)$
...
Now suppose $"PL"$ increases and hence to provide more generation, $lambda$ is also increased and continuing this process in this way, incremental cost value is reached to $lambda_2$. Therefore, $P_(g 3)$ has reached to its upper limit and cannot be increased further, i.e., $P_(g 3) = P_(g 3)^(max)$.
...
$$
frac(d C_1, d P_(g 1)) = 0.18 P_(g 1) + 41
$$

$$
frac(d C_2, d P_(g 2)) = 0.36 P_(g 2) + 32
$$
...
**Task:** Determine $P_(g 1)$, $P_(g 2)$, and values of $lambda$ for the variation of load from 54 MW to 310 MW. Assume both generating units are operating at all times.

#### 39.3.2 Computing λ Limits for Unit 1

**Given:** $P_(g 1)^(min) = 32$ MW, $P_(g 1)^(max) = 180$ MW

$$
lambda_1 = frac(d C_1, d P_(g 1)) = 0.18 P_(g 1) + 41
$$

**At minimum:**

$$
lambda_1^(min) = 0.18 times 32 + 41 = 46.76 " Rs/MWhr"
$$

**At maximum:**

$$
lambda_1^(max) = 0.18 times 180 + 41 = 73.40 " Rs/MWhr"
$$

#### 39.3.3 Computing λ Limits for Unit 2

**Given:** $P_(g 2)^(min) = 22$ MW, $P_(g 2)^(max) = 130$ MW

$$
lambda_2 = frac(d C_2, d P_(g 2)) = 0.36 P_(g 2) + 32
$$

**At minimum:**

$$
lambda_2^(min) = 0.36 times 22 + 32 = 39.92 " Rs/MWhr"
$$

**At maximum:**

$$
lambda_2^(max) = 0.36 times 130 + 32 = 78.8 " Rs/MWhr"
$$
...
**At minimum load:**
- $P_(g 1)$ fixed at 32 MW
- $P_(g 2)$ fixed at 22 MW
...
#### 39.3.6 Region 1: $39.92 <= lambda <= 46.76$

**At $lambda = 46.76$:**

$$
0.36 P_(g 2) + 32 = 46.76
$$
...
**For this region:** $lambda = 0.36 P_(g 2) + 32$

Since $P_(g 1)^(min) = 32$ MW and $P_(g 2) = "PL" - 32$:

$$
lambda = 0.36 ("PL" - 32) + 32 = 0.36 "PL" + 20.48
$$

This is **equation 3**, valid for $54 <= "PL" <= 73$ MW.
...
**At $P_(g 1) = P_(g 1)^(max) = 180$ MW:**

$$
lambda = lambda_1^(max) = 73.40 " Rs/MWhr"
$$
...
|-----------------|--------------------|--------------|
| 54–73 | $P_(g 1)$ fixed at 32 MW; $P_(g 2)$ varies | $lambda = 0.36 "PL" + 20.48$ |
| 73–(to be determined) | Both units on equal-λ basis | $lambda = 0.18 P_(g 1) + 41 = 0.36 P_(g 2) + 32$ |
| Beyond $P_(g 1)^(max)$ | $P_(g 1)$ fixed at 180 MW; $P_(g 2)$ varies | $lambda = 0.36 P_(g 2) + 32$ |
...
**Step 1:** 2 × (Equation for Unit 1): $2 lambda = 0.36 dot P_(g 1) + 82$

**Step 2:** Add Equation for Unit 2: $2 lambda + lambda = 3 lambda = 0.36 dot P_(g 1) + 82 + 0.36 dot P_(g 2) + 32$

**Step 3:** $3 lambda = 0.36 dot(P_(g 1) + P_(g 2)) + 114$

**Step 4:** Since losses are ignored, the power balance is: $P_(g 1) + P_(g 2) = P_L$ (total load)

**Step 5:** Substituting and dividing by 3:
$$
lambda = 0.12 dot P_L + 38 space "(Equation 8)"
$$

#### 39.4.2 Determining the Load Range Boundaries

**Upper boundary of this regime:** The maximum IC for unit 1 is $lambda = 73.4$ (corresponding to $P_(g 1) = 180$ MW).

Substituting $lambda = 73.4$ into Equation 8:
$$
73.4 = 0.12 dot P_L + 38
$$
$$
P_L = (73.4 - 38)/0.12 = 295 " MW"
$$

**Load range for equal-λ operation:** $73 " MW" <= P_L <= 295 " MW"$

**Verification of generator outputs at the boundary:**
- At $lambda = 73.4$: $P_(g 1) = 180$ MW (at its maximum)
- $P_(g 2) = P_L - P_(g 1) = 295 - 180 = 115$ MW

#### 39.4.3 Beyond the Limit: Single-Generator Operation

**Regime 3 ($295 " MW" <= P_L <= 310 " MW"$):**

When $P_(g 1)$ is fixed at 180 MW (its maximum), the power balance becomes:
$$
P_(g 2) + P_(g 1)^(max) = P_L
$$
$$
P_(g 2) = P_L - 180
$$

Substituting into the IC equation for Unit 2 ($lambda = 0.36 dot P_(g 2) + 32$):
$$
lambda = 0.36 dot(P_L - 180) + 32
$$
$$
lambda = 0.36 dot P_L - 64.8 + 32
$$
$$
lambda = 0.36 dot P_L - 32.8 space "(Equation 9)"
$$

**Upper boundary:** $P_(g 2)$ reaches its maximum of 130 MW at:
$$
P_L = 180 + 130 = 310 " MW"
$$

**Load range:** $295 " MW" <= P_L <= 310 " MW"$
...
The lecturer presents a table showing $P_(g 1)$, $P_(g 2)$, and $P_L$ at various λ values:
...
- At λ = 42: $P_(g 2) = (42 - 32)/0.36 = 27.78$ MW; $P_L = 32 + 27.78 = 59.78$ MW
- At λ = 50: $P_(g 1) = (50 - 41)/0.18 = 50$ MW; $P_(g 2) = (50 - 32)/0.36 = 50$ MW
- At λ = 70: $P_(g 1) = (70 - 41)/0.18 = 161.1$ MW; $P_(g 2) = (70 - 32)/0.36 = 105.55$ MW
- At λ = 75: $P_(g 2) = (75 - 32)/0.36 = 119.44$ MW; $P_L = 180 + 119.44 = 299.44$ MW
- At λ = 77: $P_(g 2) = (77 - 32)/0.36 = 125$ MW; $P_L = 180 + 125 = 305$ MW
- At λ = 78.8: $P_(g 2) = (78.8 - 32)/0.36 = 130$ MW; $P_L = 180 + 130 = 310$ MW
...
2.  **Generator 1 behavior:** $P_(g 1)$ remains fixed at 32 MW (minimum) until λ reaches 46.76; then it participates in equal-λ dispatch until reaching 180 MW (maximum) at λ = 73.4.
3.  **Generator 2 behavior:** $P_(g 2)$ takes all load increases while generator 1 is at its minimum, then shares on equal-λ basis, then takes all load increases after generator 1 reaches its maximum.
...
- **Segment AB:** $39.92 <= lambda <= 46.76$, load 54 to 73 MW (generator 1 at minimum)
- **Segment BC:** $lambda = 0.12 dot P_L + 38$, load 73 to 295 MW (equal-λ operation)
- **Segment CD:** $lambda = 0.36 dot P_L - 32.8$, load 295 to 310 MW (generator 1 at maximum)
...
- Optimal dispatch (from previous table): $P_(g 1) = 161.11$ MW, $P_(g 2) = 105.55$ MW
- Equal sharing: $P_(g 1) = 133.33$ MW, $P_(g 2) = 133.33$ MW

**Cost functions (obtained by integrating the IC functions):**

For generator 1:
$$
C_1 = integral(d C_1/d P_(g 1)) dot d P_(g 1) = integral(0.18 dot P_(g 1) + 41) dot d P_(g 1)
$$
$$
C_1 = 0.09 dot P_(g 1)^2 + 41 dot P_(g 1) + K_1
$$

For generator 2:
$$
C_2 = integral(d C_2/d P_(g 2)) dot d P_(g 2) = integral(0.36 dot P_(g 2) + 32) dot d P_(g 2)
$$
$$
C_2 = 0.18 dot P_(g 2)^2 + 32 dot P_(g 2) + K_2
$$
...
For optimal dispatch ($P_(g 1) = 161.11$ MW, $P_(g 2) = 105.55$ MW):

$$
C_("total"_"optimal") = C_1(P_(g 1) = 161.11) + C_2(P_(g 2) = 105.55)
$$
$$
= [0.09 times (161.11)^2 + 41 times 161.11 + K_1] + [0.18 times (105.55)^2 + 32 times 105.55 + K_2]
$$
$$
= 14,324 + K_1 + K_2 " Rs/hr"
$$

#### 40.3.3 Cost Calculation: Equal Sharing

For equal sharing ($P_(g 1) = 133.33$ MW, $P_(g 2) = 133.33$ MW):

$$
C_("total"_"equal") = C_1(P_(g 1) = 133.33) + C_2(P_(g 2) = 133.33)
$$
$$
= [0.09 times (133.33)^2 + 41 times 133.33 + K_1] + [0.18 times (133.33)^2 + 32 times 133.33 + K_2]
$$
$$
= 14,533 + K_1 + K_2 " Rs/hr"
$$

#### 40.3.4 Savings Calculation

**Net saving** = $C_("total"_"equal") - C_("total"_"optimal")$
$$
...
$$
P_("Loss") = sum_(i=1)^(n) P_i = sum_(i=1)^(m) P_("gi") - sum_(i=1)^(n) P_("Li") space "(Equation 23)"
$$
...
**Key insight:** $P_(g 1)$ is a dependent variable found by solving the load flow equations. Therefore, only $(m - 1)$ of the $P_("gi")$ are independent variables.

**Functional dependence of losses:**
$$
P_("Loss") = P_("Loss")(P_(g 2), P_(g 3), dots.h, P_("gm")) space "(Equation 24)"
$$
...
$$
C_T = sum_(i=1)^(m) C_i(P_("gi")) space "(Equation 24a)"
$$
...
$$
sum_(i=1)^(m) P_("gi") - P_("Loss")(P_(g 2), P_(g 3), dots.h, P_("gm")) - P_L = 0 space "(Equation 25)"
$$
...
$$
P_("gi")^(min) <= P_("gi") <= P_("gi")^(max) space "(Equation 26)"
$$
...
$$
tilde(C)_T = sum_(i=1)^(m) C_i(P_("gi")) - lambda[sum_(i=1)^(m) P_("gi") - P_("Loss")(P_(g 2), P_(g 3), dots.h, P_("gm")) - P_L] space "(Equation 27)"
$$

**Symbol meanings:**
- $tilde(C)_T$ = augmented cost function (Rs/hr)
...
$$
frac(partial tilde(C)_T, partial lambda) = sum_(i=1)^(m) P_("gi") - P_("Loss") - P_L = 0 space "(Equation 28)"
$$

This recovers the power balance constraint.

**Derivative with respect to $P_(g 1)$ (slack bus generator):**
$$
frac(partial tilde(C)_T, partial P_(g 1)) = frac(d C_1, d P_(g 1)) - lambda = 0 space "(Equation 29)"
$$
Therefore:
$$
frac(d C_1, d P_(g 1)) = lambda
$$

**Physical intuition:** For the slack bus generator, the loss term does not appear in the derivative because $P_("Loss")$ does not depend on $P_(g 1)$ (it depends only on $P_(g 2)$ through $P_("gm")$).

**Derivative with respect to $P_("gi")$ (i = 2, 3, ..., m):**
$$
frac(partial tilde(C)_T, partial P_("gi")) = frac(d C_i(P_("gi")), d P_("gi")) - lambda[1 - frac(partial P_("Loss"), partial P_("gi"))] = 0 space "(Equation 30)"
$$
...
$$
lambda = frac(d C_i(P_("gi"))/d P_("gi"), 1 - partial P_("Loss")/partial P_("gi"))
$$

**Defining the penalty factor:**
$$
L_i = frac(1, 1 - partial P_("Loss")/partial P_("gi")) space "for " i = 2, 3, dots.h, m
$$

**Coordination equation:**
$$
L_i dot frac(d C_i, d P_("gi")) = lambda space "for " i = 2, 3, dots.h, m
$$

**For the slack bus generator:**
From Equation 29: $L_1 dot d C_1/d P_(g 1) = lambda$ with $L_1 = 1$

**Complete coordination equation:**
$$
L_1 dot frac(d C_1, d P_(g 1)) = L_2 dot frac(d C_2, d P_(g 2)) = L_3 dot frac(d C_3, d P_(g 3)) = dots.h = L_m dot frac(d C_m, d P_("gm")) = lambda space "(Equation 34)"
$$
...
1.  **$L_1 = 1$** for the slack bus generator (since $P_("Loss")$ does not depend on $P_(g 1)$).
...
**System configuration:**
- Bus 1: Generator $P_(g 1)$, load $P_(L 1) = 300$ MW
- Bus 2: Generator $P_(g 2)$, load $P_(L 2) = 70$ MW
- Transmission line connecting buses 1 and 2

**Incremental cost functions:**
- $"IC"_1 = 0.35 dot P_(g 1) + 41$ Rs/MWhr
- $"IC"_2 = 0.35 dot P_(g 2) + 41$ Rs/MWhr

**Loss expression:**
$$
P_("Loss") = 0.001 dot(P_(g 2) - 70)^2 " MW"
$$

**Objective:** Determine the optimal scheduling and power loss of the transmission link.

**Physical intuition:** The loss expression indicates that losses are minimized when $P_(g 2) = 70$ MW (matching the local load at bus 2), and increase quadratically as $P_(g 2)$ deviates from this value.
...
1.  **Forgetting $L_1 = 1$:** Students may incorrectly apply the penalty factor formula to the slack bus generator. The slack bus has $L_1 = 1$ because $P_("Loss")$ does not depend on $P_(g 1)$.
...
4.  **Forgetting that $P_("Loss")$ depends only on non-slack generators:** The functional dependence $P_("Loss") = P_("Loss")(P_(g 2), dots.h, P_("gm"))$ is crucial for the derivation.
...
![Transformer series admittance and ideal-transformer representation](/content-assets/studies/power-system-analysis/Week%208/Assets/img-9.jpg)
...
![Transformer pi-equivalent circuit and admittance expressions](/content-assets/studies/power-system-analysis/Week%208/Assets/img-14.jpg)
...
![Jacobian variable and mismatch structure for P and PQV buses](/content-assets/studies/power-system-analysis/Week%208/Assets/img-20.jpg)
...
![Solved heat-rate coefficients and fuel-cost function](/content-assets/studies/power-system-analysis/Week%208/Assets/img-42.jpg)
...
![Equal incremental-cost coordination equation](/content-assets/studies/power-system-analysis/Week%208/Assets/img-53.jpg)
...
![Generator incremental-cost curves with operating limits](/content-assets/studies/power-system-analysis/Week%208/Assets/img-61.jpg)
...
![Piecewise incremental-cost versus load graph](/content-assets/studies/power-system-analysis/Week%208/Assets/img-70.jpg)
...
![Two-bus lossy dispatch example and loss expression](/content-assets/studies/power-system-analysis/Week%208/Assets/img-94.jpg)
...
2.  **Misidentifying the tap side in the π-equivalent:** The shunt branch on the tap side is $(frac(1-a, a^2)) y_("pq")$, while on the non-tap side it is $(frac(a-1, a)) y_("pq")$. These are not symmetric.
...
4.  **Forgetting to add load reactive power:** When calculating the required shunt capacitance at a P bus, use $Q_(C 2) = Q_2 + Q_(L 2)$, not just $Q_2$.
...
4.  **Check λ consistency:** In an optimal dispatch, all generators (within limits) should have the same value of $L_i dot "IC"_i$.
...
|----------|------------|----------------|
| $frac(Delta P, |V_i|) = -B' Delta delta$ | P-δ problem | $B_("ii") ≫ Q_i$, $|V_i|^2 tilde.eq |V_i|$ |
...
| Shunt branch (non-tap side) | $(frac(a-1, a)) y_("pq")$ |
| Shunt branch (tap side) | $(frac(1-a, a^2)) y_("pq")$ |
...
- After load flow: $Q_(C 2) = Q_2 + Q_(L 2)$.

### Economic Dispatch (Neglecting Losses)

- **Objective:** Minimize $C_T = sum C_i(P_("gi"))$
- **Constraint:** $sum P_("gi") = P_L$
- **Coordination equation:** $frac(d C_1, d P_(g 1)) = frac(d C_2, d P_(g 2)) = dots.h = lambda$
...
- **Coordination equation:** $L_1 dot "IC"_1 = L_2 dot "IC"_2 = dots.h = lambda$
- **Slack bus:** $L_1 = 1$
...
| Input energy rate | $F_i(P_("gi")) = P_("gi") dot H_i(P_("gi"))$ | MKcal/hr |
| Fuel cost | $C_i(P_("gi")) = a_i + b_i P_("gi") + d_i P_("gi")^2$ | Rs/hr |
| Incremental cost | $"IC"_i = b_i + 2 d_i P_("gi")$ | Rs/MWhr |

---

## Practice Quiz

**Question 1 (MCQ):** In the fast decoupled load flow, the approximation $|V_i|^2 tilde.eq |V_i|$ is justified because:
...
> The correct answer is (b). The approximation $|V_i|^2 tilde.eq |V_i|$ is valid because in a well-designed power system, bus voltages are maintained close to the nominal value of 1.0 p.u. The slack bus voltage is taken as exactly 1.0 p.u., and other buses are typically within ±5% of this value. This makes the square of the voltage magnitude approximately equal to the voltage magnitude itself. Option (a) is incorrect because voltages are not exactly 1.0 p.u. at all buses. Option (c) relates to a different approximation ($B_("ii") ≫ Q_i$). Option (d) relates to the off-diagonal element approximation.
...
(a) $(frac(a-1, a)) y_("pq")$
(b) $(frac(1-a, a^2)) y_("pq")$
(c) $frac(y_("pq"), a)$
(d) $y_("pq")$

> Answer and explanation
> The correct answer is (b). The π-equivalent circuit has a series branch $y_("pq")/a$, a shunt branch $(frac(a-1, a)) y_("pq")$ on the non-tap side, and a shunt branch $(frac(1-a, a^2)) y_("pq")$ on the tap side. Option (a) is the shunt branch on the non-tap side. Option (c) is the series branch. Option (d) is the simple transformer admittance when $a = 1$.
...
(c) The shunt branch on the non-tap side is $(frac(a-1, a)) y_("pq")$.
(d) The shunt branch on the tap side is $(frac(a-1, a)) y_("pq")$.

> Answer and explanation
> The correct answers are (a), (b), and (c). When the tap ratio is nominal ($a = 1$), the shunt branches become zero, and the transformer is simply represented by its series admittance. The series branch is $y_("pq")/a$. The shunt branch on the non-tap side is $(frac(a-1, a)) y_("pq")$. Option (d) is incorrect because the shunt branch on the tap side is $(frac(1-a, a^2)) y_("pq")$, not $(frac(a-1, a)) y_("pq")$.
...
$$
L_1 dot frac(d C_1, d P_(g 1)) = L_2 dot frac(d C_2, d P_(g 2)) = dots.h = L_m dot frac(d C_m, d P_("gm")) = lambda
$$
...
$$
"IC"_i = frac(d C_i, d P_("gi")) = b_i + 2 d_i P_("gi")
$$
...
$$
a = k alpha' = 222.24, space b = k beta' = 20.44, space d = k gamma' = 0.142
$$
>
> The incremental cost is:
$$
"IC" = frac(d C, d P_g) = b + 2 d P_g = 20.44 + 2 times 0.142 times 50 = 20.44 + 14.2 = 34.64 " Rs/MWhr"
$$

---

**Question 15 (Numerical):** Two generators have incremental cost functions $"IC"_1 = 0.18 P_(g 1) + 41$ and $"IC"_2 = 0.36 P_(g 2) + 32$ Rs/MWhr. The generator limits are $32 <= P_(g 1) <= 180$ MW and $22 <= P_(g 2) <= 130$ MW. For a total load of 100 MW, determine the optimal dispatch.

> Answer and explanation
> For a load of 100 MW, both generators are within their limits and operate on an equal-λ basis:
$$
0.18 P_(g 1) + 41 = 0.36 P_(g 2) + 32 = lambda
$$
>
> From the power balance: $P_(g 1) + P_(g 2) = 100$
>
> From the equal-λ condition:
$$
0.18 P_(g 1) + 41 = 0.36 P_(g 2) + 32
$$
$$
0.18 P_(g 1) - 0.36 P_(g 2) = -9
$$
>
> Substituting $P_(g 2) = 100 - P_(g 1)$:
$$
0.18 P_(g 1) - 0.36(100 - P_(g 1)) = -9
$$
$$
0.18 P_(g 1) - 36 + 0.36 P_(g 1) = -9
$$
$$
0.54 P_(g 1) = 27
$$
...
> The system λ is:
$$
lambda = 0.18 times 50 + 41 = 50 " Rs/MWhr"
$$
>
...
> Answer and explanation
> From the tabulated solution, the optimal dispatch at $P_L = 266.66$ MW is $P_(g 1) = 161.11$ MW and $P_(g 2) = 105.55$ MW.
>
> The cost functions are obtained by integrating the IC functions:
$$
C_1 = 0.09 P_(g 1)^2 + 41 P_(g 1) + K_1
$$
$$
C_2 = 0.18 P_(g 2)^2 + 32 P_(g 2) + K_2
$$
>
...
$$
$$
C_("opt") = 14324 + K_1 + K_2 " Rs/hr"
$$
>
> **Equal sharing cost** ($P_(g 1) = P_(g 2) = 133.33$ MW):
$$
...
$$
$$
C_("eq") = 14533 + K_1 + K_2 " Rs/hr"
$$
>
> **Saving:**
$$
"Saving" = C_("eq") - C_("opt") = 14533 - 14324 = 209 " Rs/hr"
$$
>
...
$$
L_i dot "IC"_i = lambda
$$
> where $L_i$ is the penalty factor. A high penalty factor means that the effective cost contribution of this generator is high. To maintain the equality $L_i dot "IC"_i = lambda$, the generator with a high penalty factor must have a lower incremental cost $"IC"_i$ compared to generators with lower penalty factors.
>
