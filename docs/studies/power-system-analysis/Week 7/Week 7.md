...
In a power system, the "temperature" is the complex voltage at each bus. The "neighbors" are the buses connected to it through transmission lines. The "update rule" comes from the power flow equations. We start with an initial guess for all bus voltages (often a "flat start" where all voltages are $1.0 angle 0^degree$). Then, we visit each bus one by one, calculating a new voltage based on the scheduled power injection at that bus and the current voltages of its neighbors. Crucially, as soon as we calculate a new voltage for a bus, we use it immediately for all subsequent calculations in the same iteration. This "immediate update" is what distinguishes the Gauss-Seidel method from the older Jacobi method.
...
**System Data:**
- **Base MVA** = 100 MVA
- **Bus 1 (Slack Bus):** $V_1 = 1.05 angle 0^degree$ per unit (fixed).
- **Bus 2 (PQ Bus):** Load $P_(L 2) = 305.6$ MW, $Q_(L 2) = 140.2$ MVAr. Generation $P_(G 2) = 50$ MW, $Q_(G 2) = 30$ MVAr.
- **Bus 3 (PQ Bus):** Load $P_(L 3) = 138.6$ MW, $Q_(L 3) = 45.2$ MVAr. No generation.
...
**Note from the lecturer:** Although a flat voltage start (using 1.05 for all buses) is theoretically better, this example uses $1 + j 0$ for buses 2 and 3 to illustrate the method with simpler arithmetic.
...
**At Bus 2:**
- $P_(L 2) = 305.6 " MW" / 100 " MVA" = 3.056$ pu
- $Q_(L 2) = 140.2 " MVAr" / 100 " MVA" = 1.402$ pu
- $P_(G 2) = 50 " MW" / 100 " MVA" = 0.5$ pu
- $Q_(G 2) = 30 " MVAr" / 100 " MVA" = 0.3$ pu

**At Bus 3:**
- $P_(L 3) = 138.6 " MW" / 100 " MVA" = 1.386$ pu
- $Q_(L 3) = 45.2 " MVAr" / 100 " MVA" = 0.452$ pu
...
$$
P_2 = P_(g 2) - P_(L 2) = 0.5 - 3.056 = -2.556 " pu"
$$
$$
Q_2 = Q_(g 2) - Q_(L 2) = 0.3 - 1.402 = -1.102 " pu"
$$

**Net Injected Power at Bus 3:**
$$
P_3 = P_(g 3) - P_(L 3) = 0 - 1.386 = -1.386 " pu"
$$
$$
Q_3 = Q_(g 3) - Q_(L 3) = 0 - 0.452 = -0.452 " pu"
$$
...
$$
y_(12) = y_(21) = frac(1, 0.02 + j 0.04) = 10 - j 20
$$
$$
y_(13) = y_(31) = frac(1, 0.01 + j 0.03) = 10 - j 30
$$
$$
y_(23) = y_(32) = frac(1, 0.0125 + j 0.025) = 16 - j 32
$$

Now, we can build the $Y_("bus")$ matrix. Since line charging is neglected, there are no shunt admittances ($y_(i 0) = 0$).

**Diagonal Elements:**
$$
Y_(11) = y_(12) + y_(13) + y_(10) = (10 - j 20) + (10 - j 30) + 0 = 20 - j 50
$$
$$
Y_(22) = y_(21) + y_(23) = (10 - j 20) + (16 - j 32) = 26 - j 52
$$
$$
Y_(33) = y_(31) + y_(32) = (10 - j 30) + (16 - j 32) = 26 - j 62
$$

**Off-Diagonal Elements:**
$$
Y_(12) = Y_(21) = -y_(12) = -(10 - j 20) = -10 + j 20
$$
$$
Y_(13) = Y_(31) = -y_(13) = -(10 - j 30) = -10 + j 30
$$
$$
Y_(23) = Y_(32) = -y_(23) = -(16 - j 32) = -16 + j 32
$$
...
| :--- | :--- | :--- | :--- |
| $Y_(11)$ | $20 - j 50$ | 53.85 | -68.2° |
| $Y_(22)$ | $26 - j 52$ | 58.13 | -63.4° |
| $Y_(33)$ | $26 - j 62$ | 67.23 | -67.2° |
| $Y_(12)$ | $-10 + j 20$ | 22.36 | 116.6° |
| $Y_(13)$ | $-10 + j 30$ | 31.62 | 108.4° |
| $Y_(23)$ | $-16 + j 32$ | 35.77 | 116.6° |
...
$$
V_i^((p+1)) = frac(1, Y_("ii")) [ frac(P_i - j Q_i, (V_i^((p)))^(star)) - sum_(k=1, k != i)^(n) Y_("ik") V_k ]
$$
...
**Equation for Bus 2:**
$$
V_2^((p+1)) = frac(1, Y_(22)) [ frac(P_2 - j Q_2, (V_2^((p)))^(star)) - Y_(21) V_1 - Y_(23) V_3^((p)) ]
$$

**Equation for Bus 3:**
$$
V_3^((p+1)) = frac(1, Y_(33)) [ frac(P_3 - j Q_3, (V_3^((p)))^(star)) - Y_(31) V_1 - Y_(32) V_2^((p+1)) ]
$$
...
$$
frac(P_2 - j Q_2, Y_(22)) = frac(-2.556 + j 1.102, 58.13 angle -63.4^degree) = 0.0478 angle 220.1^degree
$$

$$
frac(Y_(21), Y_(22)) = frac(22.36 angle 116.6^degree, 58.13 angle -63.4^degree) = 0.3846 angle 180^degree = -0.3846
$$

$$
frac(Y_(23), Y_(22)) = frac(35.77 angle 116.6^degree, 58.13 angle -63.4^degree) = 0.6153 angle 180^degree = -0.6153
$$

The rewritten equation for $V_2$ becomes:
$$
V_2^((p+1)) = frac(0.0478 angle 220.1^degree, (V_2^p)^(star)) + 0.3846 V_1 + 0.6153 V_3^((p))
$$

Similarly, for the equation for $V_3$:
$$
frac(P_3 - j Q_3, Y_(33)) = 0.0217 angle 229.2^degree
$$
$$
frac(Y_(31), Y_(33)) = 0.47 angle 175.6^degree
$$
$$
frac(Y_(32), Y_(33)) = 0.532 angle 183.8^degree
$$

The rewritten equation for $V_3$ becomes:
$$
V_3^((p+1)) = frac(0.0217 angle 229.2^degree, (V_3^p)^(star)) - 0.47 angle 175.6^degree dot V_1 - 0.532 angle 183.8^degree dot V_2^((p+1))
$$
...
We start with a flat voltage start: $V_2^0 = 1 + j 0$ and $V_3^0 = 1 + j 0$.

**First Iteration (p = 0):**

For $V_2^1$:
$$
V_2^1 = frac(0.0478 angle 220.1^degree, 1 + j 0) + 0.3846(1.05 + j 0) + 0.6153(1 + j 0)
$$
$$
V_2^1 = 0.0478 angle 220.1^degree + 0.40383 + 0.6153
$$
$$
V_2^1 = (-0.0366 - j 0.0307) + 1.01913
$$
$$
V_2^1 = 0.98253 - j 0.0307 = 0.98305 angle -1.8^degree
$$

For $V_3^1$, we use the newly computed $V_2^1$:
$$
V_3^1 = frac(0.0217 angle 229.2^degree, 1 + j 0) - 0.47 angle 175.6^degree dot (1.05) - 0.532 angle 183.8^degree dot (0.98305 angle -1.8^degree)
$$
$$
V_3^1 = 0.0217 angle 229.2^degree - 0.4935 angle 175.6^degree - 0.523 angle 182^degree
$$
$$
V_3^1 = (-0.0142 - j 0.0164) - (-0.492 - j 0.038) - (-0.5227 - j 0.0182)
$$
$$
V_3^1 = 1.0005 - j 0.0362 = 1.0011 angle -2.06^degree
$$

**After First Iteration:**
$$
V_2^1 = 0.98305 angle -1.8^degree
$$
$$
V_3^1 = 1.0011 angle -2.06^degree
$$
...
For $V_2^2$:
$$
V_2^2 = frac(0.0478 angle 220.1^degree, (0.98305 angle -1.8^degree)^(star)) + 0.3846(1.05) + 0.6153(1.0011 angle -2.06^degree)
$$
$$
V_2^2 = frac(0.0478 angle 220.1^degree, 0.98305 angle +1.8^degree) + 0.40383 + 0.6153 angle -2.06^degree
$$
$$
V_2^2 = 0.0486 angle 218.3^degree + 0.40383 + (0.6149 - j 0.0221)
$$
$$
V_2^2 = (-0.0382 - j 0.0302) + 1.01873 - j 0.0221
$$
$$
V_2^2 = 0.98053 - j 0.0523 = 0.98265 angle -3.048^degree
$$

For $V_3^2$, we use the newly computed $V_2^2$:
$$
V_3^2 = frac(0.0217 angle 229.2^degree, (1.0011 angle -2.06^degree)^(star)) - 0.47 angle 175.6^degree dot (1.05) - 0.532 angle 183.8^degree dot (0.98265 angle -3.048^degree)
$$
$$
V_3^2 = frac(0.0217 angle 229.2^degree, 1.0011 angle +2.06^degree) - 0.4935 angle 175.6^degree - 0.5228 angle 180.75^degree
$$
$$
V_3^2 = 0.0217 angle 227.1^degree - (-0.492 - j 0.038) - (-0.5228 - j 0.0068)
$$
$$
V_3^2 = (-0.0148 - j 0.0159) + 0.492 + j 0.038 + 0.5228 + j 0.0068
$$
$$
V_3^2 = 1.0000 - j 0.0289 = 1.00099 angle -2.6^degree
$$

**After Second Iteration:**
$$
V_2^2 = 0.98265 angle -3.048^degree
$$
$$
V_3^2 = 1.00099 angle -2.6^degree
$$
...
$$
P_1 = sum_(k=1)^(n) |V_1| |V_k| |Y_(1 k)| cos(theta_(1 k) - delta_1 + delta_k)
$$
$$
Q_1 = -sum_(k=1)^(n) |V_1| |V_k| |Y_(1 k)| sin(theta_(1 k) - delta_1 + delta_k)
$$

For our three-bus system, expanding for $P_1$:
$$
P_1 = |V_1|^2 |Y_(11)| cos(theta_(11)) + |V_1||V_2||Y_(12)| cos(theta_(12) - delta_1 + delta_2) + |V_1||V_3||Y_(13)| cos(theta_(13) - delta_1 + delta_3)
$$

Using the values after the second iteration ($|V_1|=1.05, delta_1=0^degree, |V_2|=0.98265, delta_2=-3.048^degree, |V_3|=1.00099, delta_3=-2.6^degree$):
$$
P_1 = (1.05)^2(53.85) cos(-68.2^degree) + (1.05)(0.98265)(22.36) cos(116.56^degree - 0 - 3.048^degree) + (1.05)(1.00099)(31.62) cos(108.4^degree - 0 - 2.6^degree)
$$
$$
P_1 = 59.37(0.371) + 23.07 cos(113.5^degree) + 33.26 cos(105.8^degree)
$$
$$
P_1 = 22.03 - 9.18 - 9.05 = 3.80 " pu"
$$

$$
P_1 approx 3.84 " pu" = 384 " MW"
$$

Similarly, for $Q_1$:
$$
Q_1 = -|V_1|^2 |Y_(11)| sin(theta_(11)) - |V_1||V_2||Y_(12)| sin(theta_(12) - delta_1 + delta_2) - |V_1||V_3||Y_(13)| sin(theta_(13) - delta_1 + delta_3)
$$
$$
Q_1 = -(1.05)^2(53.85) sin(-68.2^degree) - (1.05)(0.98265)(22.36) sin(113.5^degree) - (1.05)(1.00099)(31.62) sin(105.8^degree)
$$
$$
Q_1 = -59.37(-0.928) - 23.07(0.917) - 33.26(0.962)
$$
$$
Q_1 = 55.10 - 21.16 - 31.99 = 1.95 " pu"
$$

$$
Q_1 approx 1.9786 " pu" = 197.86 " MVAr"
$$
...
$$
P_("ik") = |V_i|^2 |Y_("ik")| cos(theta_("ik")) - |V_i||V_k||Y_("ik")| cos(theta_("ik") - delta_i + delta_k)
$$
...
**Line 1-2:**
$$
P_(12) = (1.05)^2(22.36) cos(116.6^degree) - (1.05)(0.98265)(22.36) cos(116.6^degree - 0 - 3.048^degree)
$$
$$
P_(12) = 24.65(-0.448) - 23.07 cos(113.55^degree)
$$
$$
P_(12) = -11.04 - 23.07(-0.398) = -11.04 + 9.18 = -1.86 " pu"
$$
...
$$
P_("LOSS" 12) = P_(12) + P_(21) = 181.89 - 174.45 = "7.44 MW"
$$
$$
P_("LOSS" 13) = P_(13) + P_(31) = 200 - 195 = "5 MW"
$$
$$
P_("LOSS" 23) = P_(23) + P_(32) = -49.03 + 49.6 = "0.57 MW"
$$
...
$$
Q_("LOSS" 12) = Q_(12) + Q_(21) = 89.48 - 74.6 = "14.88 MVAr"
$$
$$
Q_("LOSS" 13) = Q_(13) + Q_(31) = 108.8 - 94.69 = "14.11 MVAr"
$$
$$
Q_("LOSS" 23) = Q_(23) + Q_(32) = -47.46 + 48.66 = "1.20 MVAr"
$$
...
| $P_("LOSS" 12)$ | 0.0744 | 7.44 MW |
| $P_("LOSS" 13)$ | 0.05 | 5 MW |
| $P_("LOSS" 23)$ | 0.0057 | 0.57 MW |
| $Q_("LOSS" 12)$ | 0.1488 | 14.88 MVAr |
| $Q_("LOSS" 13)$ | 0.1411 | 14.11 MVAr |
| $Q_("LOSS" 23)$ | 0.012 | 1.20 MVAr |
...
**System Modification:**
- **Bus 1:** Slack bus, $V_1 = 1.05 angle 0^degree$ pu.
- **Bus 2:** PV bus, $|V_2| = 1.0$ pu (specified), $P_(G 2) = 50$ MW (specified).
- **Bus 3:** PQ bus, $P_(L 3) = 138.6$ MW, $Q_(L 3) = 45.2$ MVAr.

The load at bus 2 is the same: $P_(L 2) = 305.6$ MW, $Q_(L 2) = 140.2$ MVAr.

**Net Power Injections:**
- $P_2 = P_(G 2) - P_(L 2) = 50 - 305.6 = -255.6$ MW = **-2.556 pu**
...
$$
Q_i = -sum_(k=1)^(n) |V_i| |V_k| |Y_("ik")| sin(theta_("ik") - delta_i + delta_k)
$$

For bus 2, this expands to:
$$
Q_2 = -|V_2||V_1||Y_(21)| sin(theta_(21) - delta_2 + delta_1) - |V_2|^2 |Y_(22)| sin(theta_(22)) - |V_2||V_3||Y_(23)| sin(theta_(23) - delta_2 + delta_3)
$$

**First Iteration (p=0):**
We start with $V_2^0 = 1.0 angle 0^degree$ and $V_3^0 = 1.0 angle 0^degree$. Note that $|V_2|$ is fixed at 1.0.
$$
Q_2^1 = -1.0 times 1.05 times 22.36 sin(116.6^degree - 0 + 0) - (1.0)^2 times 58.13 sin(-63.4^degree) - 1.0 times 1.0 times 35.77 sin(116.6^degree - 0 + 0)
$$
$$
Q_2^1 = -23.48 sin(116.6^degree) - 58.13 sin(-63.4^degree) - 35.77 sin(116.6^degree)
$$
$$
Q_2^1 = -23.48(0.894) - 58.13(-0.894) - 35.77(0.894)
$$
$$
Q_2^1 = -20.99 + 51.97 - 31.98 = -1.00 " pu"
$$
...
$$
V_(c 2)^(p+1) = frac(1, Y_(22)) ( frac(P_2 - j Q_2^(p+1), (V_2^p)^(star)) - Y_(21) V_1 - Y_(23) V_3^p )
$$

Using the pre-computed constants from the previous lecture:
$$
V_(c 2)^1 = frac(0.04725 angle 221.9^degree, (V_2^0)^(star)) + 0.3846 V_1 + 0.6153 V_3^0
$$

Let's calculate the first term:
$$
frac(P_2 - j Q_2^1, Y_(22)) = frac(-2.556 + j 1.0067, 58.13 angle -63.4^degree) = 0.04725 angle 221.9^degree
$$

So,
$$
V_(c 2)^1 = 0.04725 angle 221.9^degree + 0.3846(1.05) + 0.6153(1.0)
$$
$$
V_(c 2)^1 = (-0.0352 - j 0.0315) + 0.40383 + 0.6153
$$
$$
V_(c 2)^1 = 0.98393 - j 0.0315
$$

This is the *uncorrected* voltage. The magnitude is $|V_(c 2)^1| = sqrt(0.98393^2 + 0.0315^2) = 0.9844$, which is not 1.0. We must correct it.
...
**Corrected Voltage:**
$$
V_2^((1)) = 0.9995 - j 0.0315 = 1.0 angle -1.807^degree
$$
...
$$
V_3^(1) = frac(0.0217 angle 229.2^degree, (V_3^0)^(star)) - 0.47 angle 175.6^degree dot V_1 - 0.532 angle 183.8^degree dot V_2^(1)
$$
$$
V_3^(1) = 0.0217 angle 229.2^degree - 0.47 angle 175.6^degree times 1.05 - 0.532 angle 183.8^degree times (1.0 angle -1.807^degree)
$$
$$
V_3^(1) = (-0.0142 - j 0.0164) - (-0.492 - j 0.038) - (-0.531 - j 0.017)
$$
$$
V_3^(1) = 1.0088 - j 0.0354 = 1.0101 angle -2.03^degree
$$

**After First Iteration:**
$$
V_2^((1)) = 1.0 angle -1.807^degree
$$
$$
V_3^((1)) = 1.0101 angle -2.03^degree
$$
...
Using $delta_2^1 = -1.807^degree$, $delta_3^1 = -2.03^degree$, $|V_3^1| = 1.0101$:
$$
Q_2^2 = -1.0 times 1.05 times 22.36 sin(116.6^degree - (-1.807^degree) + 0) - (1.0)^2 times 58.13 sin(-63.4^degree) - 1.0 times 1.0101 times 35.77 sin(116.6^degree - (-1.807^degree) + (-2.03^degree))
$$
$$
Q_2^2 = -23.48 sin(118.4^degree) - 58.13 sin(-63.4^degree) - 36.13 sin(116.4^degree)
$$
$$
Q_2^2 = -23.48(0.879) - 58.13(-0.894) - 36.13(0.896)
$$
$$
Q_2^2 = -20.64 + 51.97 - 32.37 = -1.04 " pu"
$$
...
$$
V_(c 2)^2 = frac(0.04725 angle 221.9^degree, (V_2^1)^(star)) + 0.3846 V_1 + 0.6153 V_3^1
$$
$$
V_(c 2)^2 = frac(0.04725 angle 221.9^degree, 1.0 angle +1.807^degree) + 0.40383 + 0.6153(1.0101 angle -2.03^degree)
$$
$$
V_(c 2)^2 = 0.04725 angle 220.1^degree + 0.40383 + 0.6215 angle -2.03^degree
$$
$$
V_(c 2)^2 = (-0.0362 - j 0.0304) + 0.40383 + (0.6211 - j 0.0220)
$$
$$
V_(c 2)^2 = 0.9887 - j 0.0524
$$
...
$$
V_2^((2)) = 0.9986 - j 0.0524 = 1.0 angle -3^degree
$$

**Step 3: Calculate $V_3^2$:**
$$
V_3^(2) = frac(0.0217 angle 229.2^degree, (V_3^1)^(star)) - 0.47 angle 175.6^degree dot V_1 - 0.532 angle 183.8^degree dot V_2^(2)
$$
$$
V_3^(2) = frac(0.0217 angle 229.2^degree, 1.0101 angle +2.03^degree) - 0.4935 angle 175.6^degree - 0.532 angle 183.8^degree times (1.0 angle -3^degree)
$$
$$
V_3^(2) = 0.0215 angle 227.2^degree - (-0.492 - j 0.038) - 0.532 angle 180.8^degree
$$
$$
V_3^(2) = (-0.0146 - j 0.0158) + 0.492 + j 0.038 - (-0.532 - j 0.0074)
$$
$$
V_3^(2) = 1.0094 - j 0.0462 = 1.0103 angle -2.6^degree
$$

**After Second Iteration:**
$$
V_2^((2)) = 1.0 angle -3^degree
$$
$$
V_3^((2)) = 1.0103 angle -2.6^degree
$$
...
- **Angle Caution:** When using a calculator, always be careful about the quadrant of angles. For example, $angle 175.6^degree$ is in the second quadrant, and its sine is positive while its cosine is negative.
...
$$
y_1 = f_1(x_1, x_2, dots.h, x_n)
$$
$$
y_2 = f_2(x_1, x_2, dots.h, x_n)
$$
$$
dots.v
$$
$$
y_n = f_n(x_1, x_2, dots.h, x_n)
$$

Let $x_1^0, x_2^0, dots.h, x_n^0$ be the initial estimates. We want to find the corrections $Delta x_1, Delta x_2, dots.h, Delta x_n$ such that:
$$
y_i = f_i(x_1^0 + Delta x_1, x_2^0 + Delta x_2, dots.h, x_n^0 + Delta x_n)
$$

Using a Taylor series expansion and neglecting higher-order terms, we get:
$$
y_i approx f_i(x_1^0, x_2^0, dots.h, x_n^0) + sum_(j=1)^(n) Delta x_j frac(partial f_i, partial x_j) g|_0
$$

This can be written in matrix form as:
$$
mat(y_1 - f_1(x^0); y_2 - f_2(x^0); dots.v; y_n - f_n(x^0)) = mat(frac(partial f_1, partial x_1), frac(partial f_1, partial x_2), dots.h, frac(partial f_1, partial x_n); frac(partial f_2, partial x_1), frac(partial f_2, partial x_2), dots.h, frac(partial f_2, partial x_n); dots.v, dots.v, dots.down, dots.v; frac(partial f_n, partial x_1), frac(partial f_n, partial x_2), dots.h, frac(partial f_n, partial x_n)) mat(Delta x_1; Delta x_2; dots.v; Delta x_n)
$$
...
$$
mat(Delta P_2^((p)); Delta P_3^((p)); dots.v; Delta P_n^((p)); Delta Q_2^((p)); Delta Q_3^((p)); dots.v; Delta Q_n^((p))) = mat(frac(partial P_2, partial delta_2), frac(partial P_2, partial delta_3), dots.h, frac(partial P_2, partial delta_n), frac(partial P_2, partial V_2), frac(partial P_2, partial V_3), dots.h, frac(partial P_2, partial V_n); frac(partial P_3, partial delta_2), frac(partial P_3, partial delta_3), dots.h, frac(partial P_3, partial delta_n), frac(partial P_3, partial V_2), frac(partial P_3, partial V_3), dots.h, frac(partial P_3, partial V_n); dots.v, dots.v, dots.down, dots.v, dots.v, dots.v, dots.down, dots.v; frac(partial P_n, partial delta_2), frac(partial P_n, partial delta_3), dots.h, frac(partial P_n, partial delta_n), frac(partial P_n, partial V_2), frac(partial P_n, partial V_3), dots.h, frac(partial P_n, partial V_n); frac(partial Q_2, partial delta_2), frac(partial Q_2, partial delta_3), dots.h, frac(partial Q_2, partial delta_n), frac(partial Q_2, partial V_2), frac(partial Q_2, partial V_3), dots.h, frac(partial Q_2, partial V_n); frac(partial Q_3, partial delta_2), frac(partial Q_3, partial delta_3), dots.h, frac(partial Q_3, partial delta_n), frac(partial Q_3, partial V_2), frac(partial Q_3, partial V_3), dots.h, frac(partial Q_3, partial V_n); dots.v, dots.v, dots.down, dots.v, dots.v, dots.v, dots.down, dots.v; frac(partial Q_n, partial delta_2), frac(partial Q_n, partial delta_3), dots.h, frac(partial Q_n, partial delta_n), frac(partial Q_n, partial V_2), frac(partial Q_n, partial V_3), dots.h, frac(partial Q_n, partial V_n)) mat(Delta delta_2^((p)); Delta delta_3^((p)); dots.v; Delta delta_n^((p)); Delta V_2^((p)); Delta V_3^((p)); dots.v; Delta V_n^((p)))
$$

This can be written in compact form as:
$$
mat(Delta P; Delta Q) = mat(J_1, J_2; J_3, J_4) mat(Delta delta; Delta V)
$$

Where:
- $J_1 = frac(partial P, partial delta)$: $(n-1) times (n-1)$
- $J_2 = frac(partial P, partial V)$: $(n-1) times (n-1)$
- $J_3 = frac(partial Q, partial delta)$: $(n-1) times (n-1)$
- $J_4 = frac(partial Q, partial V)$: $(n-1) times (n-1)$
...
- $J_1$ remains $(n-1) times (n-1)$ because we still need to solve for the angle at every non-slack bus.
- $J_4$ becomes $(n-1-m) times (n-1-m)$ because the voltage magnitudes at PV buses are known, so we don't need to solve for them.

For example, a 10-bus system with 3 PV buses: $J_1$ is $9 times 9$, and $J_4$ is $6 times 6$.
...
$$
frac(partial P_i, partial delta_i) = sum_(k=1, k != i)^(n) |V_i| |V_k| |Y_("ik")| sin(theta_("ik") - delta_i + delta_k)
$$

**Derivation Illustration (n=4, i=2):**
First, expand $P_2$:
$$
P_2 = |V_2||V_1||Y_(21)| cos(theta_(21) - delta_2 + delta_1) + |V_2|^2 |Y_(22)| cos(theta_(22)) + |V_2||V_3||Y_(23)| cos(theta_(23) - delta_2 + delta_3) + |V_2||V_4||Y_(24)| cos(theta_(24) - delta_2 + delta_4)
$$

Now, differentiate with respect to $delta_2$:
$$
frac(partial P_2, partial delta_2) = |V_2||V_1||Y_(21)| sin(theta_(21) - delta_2 + delta_1) + 0 + |V_2||V_3||Y_(23)| sin(theta_(23) - delta_2 + delta_3) + |V_2||V_4||Y_(24)| sin(theta_(24) - delta_2 + delta_4)
$$
...
- The power flow problem can be formulated as a matrix equation $mat(Delta P; Delta Q) = J mat(Delta delta; Delta V)$.
...
- The diagonal elements of $J_1$ are summations over all $k != i$.
...
The off-diagonal elements of $J_1$ are $frac(partial P_i, partial delta_k)$ where $k != i$. The general expression is:
$$
frac(partial P_i, partial delta_k) = -|V_i| |V_k| |Y_("ik")| sin(theta_("ik") - delta_i + delta_k), space k != i
$$
...
$$
frac(partial P_2, partial delta_3) = -|V_2||V_3||Y_(23)| sin(theta_(23) - delta_2 + delta_3)
$$
...
$$
frac(partial Q_i, partial V_i) = -2|V_i||Y_("ii")| sin(theta_("ii")) - sum_(k=1, k != i)^(n) |V_k||Y_("ik")| sin(theta_("ik") - delta_i + delta_k)
$$
...
The off-diagonal elements of $J_4$ are $frac(partial Q_i, partial V_k)$ where $k != i$. The general expression is:
$$
frac(partial Q_i, partial V_k) = -|V_i||Y_("ik")| sin(theta_("ik") - delta_i + delta_k), space k != i
$$

**Derivation Illustration (i=2, k=3):**
From the expanded $Q_2$, only the term containing $V_3$ survives differentiation with respect to $V_3$:
$$
frac(partial Q_2, partial V_3) = -|V_2||Y_(23)| sin(theta_(23) - delta_2 + delta_3)
$$
...
| :--- | :--- | :--- |
| $frac(partial P_i, partial delta_i)$ | $sum_(k != i) |V_i||V_k||Y_("ik")| sin(theta_("ik") - delta_i + delta_k)$ | Diagonal of $J_1$ |
| $frac(partial P_i, partial delta_k)$ | $-|V_i||V_k||Y_("ik")| sin(theta_("ik") - delta_i + delta_k)$ | Off-diagonal of $J_1$ |
| $frac(partial Q_i, partial V_i)$ | $-2|V_i||Y_("ii")| sin(theta_("ii")) - sum_(k != i) |V_k||Y_("ik")| sin(theta_("ik") - delta_i + delta_k)$ | Diagonal of $J_4$ |
| $frac(partial Q_i, partial V_k)$ | $-|V_i||Y_("ik")| sin(theta_("ik") - delta_i + delta_k)$ | Off-diagonal of $J_4$ |
...
$$
Delta P_i^((p)) = P_i^("scheduled") - P_i^("calculated"(p))
$$
$$
Delta Q_i^((p)) = Q_i^("scheduled") - Q_i^("calculated"(p))
$$

Where:
- $P_i^("scheduled")$ and $Q_i^("scheduled")$ are the known net injections (generation minus load).
- $P_i^("calculated"(p))$ and $Q_i^("calculated"(p))$ are computed from the power flow equations using the current voltage estimates.

The convergence criteria are based on the maximum absolute residual:
$$
Delta P_("max") = max(|Delta P_2|, |Delta P_3|, dots.h, |Delta P_n|)
$$
$$
Delta Q_("max") = max(|Delta Q_2|, |Delta Q_3|, dots.h, |Delta Q_n|)
$$

The solution has converged when both $Delta P_("max") <= epsilon$ and $Delta Q_("max") <= epsilon$, where $epsilon$ is a small tolerance (e.g., $10^(-4)$ or $10^(-5)$).
...
Now we put the Newton-Raphson theory into practice with two detailed worked examples. The first example is a three-bus system with all PQ buses, which allows us to see the full $2 times 2$ Jacobian matrices. The second example introduces a PV bus, which changes the structure of the Jacobian and the iterative process. These examples will solidify our understanding of the NR algorithm.
...
**System Data:**
- Base MVA = 100
- $V_1 = 1.05 angle 0^degree$ pu
- $P_2 = -2.556$ pu, $Q_2 = -1.102$ pu
- $P_3 = -1.386$ pu, $Q_3 = -0.452$ pu

**Y-bus Matrix (from Lecture 31):**
- $Y_(11) = 53.85 angle -68.2^degree$
- $Y_(12) = 22.36 angle 116.6^degree$
- $Y_(13) = 31.62 angle 108.4^degree$
- $Y_(22) = 58.13 angle -63.4^degree$
- $Y_(23) = 35.77 angle 116.6^degree$
- $Y_(33) = 67.23 angle -67.2^degree$
...
**Elements of $J_1$:**
$$
frac(partial P_2, partial delta_2) = |V_2||V_1||Y_(21)| sin(theta_(21) - delta_2 + delta_1) + |V_2||V_3||Y_(23)| sin(theta_(23) - delta_2 + delta_3)
$$
$$
= 1.0 times 1.05 times 22.36 sin(116.6^degree) + 1.0 times 1.0 times 35.77 sin(116.6^degree)
$$
$$
...
$$
frac(partial P_2, partial delta_3) = -|V_2||V_3||Y_(23)| sin(theta_(23) - delta_2 + delta_3) = -1.0 times 1.0 times 35.77 sin(116.6^degree) = -31.98
$$

$$
frac(partial P_3, partial delta_2) = -|V_3||V_2||Y_(32)| sin(theta_(32) - delta_3 + delta_2) = -1.0 times 1.0 times 35.77 sin(116.6^degree) = -31.98
$$

$$
frac(partial P_3, partial delta_3) = |V_3||V_1||Y_(31)| sin(theta_(31) - delta_3 + delta_1) + |V_3||V_2||Y_(32)| sin(theta_(32) - delta_3 + delta_2)
$$
$$
= 1.0 times 1.05 times 31.62 sin(108.4^degree) + 1.0 times 1.0 times 35.77 sin(116.6^degree)
$$
$$
= 33.20(0.949) + 35.77(0.894) = 31.51 + 31.98 = 63.49
$$

So, $J_1 = mat(52.97, -31.98; -31.98, 63.49)$

**Elements of $J_4$:**
$$
frac(partial Q_2, partial V_2) = -|V_1||Y_(21)| sin(theta_(21) - delta_2 + delta_1) - 2|V_2||Y_(22)| sin(theta_(22)) - |V_3||Y_(23)| sin(theta_(23) - delta_2 + delta_3)
$$
$$
= -1.05 times 22.36 sin(116.6^degree) - 2 times 1.0 times 58.13 sin(-63.4^degree) - 1.0 times 35.77 sin(116.6^degree)
$$
$$
...
$$
frac(partial Q_2, partial V_3) = -|V_2||Y_(23)| sin(theta_(23) - delta_2 + delta_3) = -1.0 times 35.77 sin(116.6^degree) = -31.98
$$

$$
frac(partial Q_3, partial V_2) = -|V_3||Y_(32)| sin(theta_(32) - delta_3 + delta_2) = -1.0 times 35.77 sin(116.6^degree) = -31.98
$$

$$
frac(partial Q_3, partial V_3) = -|V_1||Y_(31)| sin(theta_(31) - delta_3 + delta_1) - |V_2||Y_(32)| sin(theta_(32) - delta_3 + delta_2) - 2|V_3||Y_(33)| sin(theta_(33))
$$
$$
= -1.05 times 31.62 sin(108.4^degree) - 1.0 times 35.77 sin(116.6^degree) - 2 times 1.0 times 67.23 sin(-67.2^degree)
$$
$$
...
So, $J_4 = mat(50.97, -31.98; -31.98, 60.48)$
...
**Calculated Powers:**
$$
P_2^("calc"(0)) = |V_2||V_1||Y_(21)| cos(theta_(21) - delta_2 + delta_1) + |V_2|^2|Y_(22)| cos(theta_(22)) + |V_2||V_3||Y_(23)| cos(theta_(23) - delta_2 + delta_3)
$$
$$
= 1.0 times 1.05 times 22.36 cos(116.6^degree) + 1.0 times 58.13 cos(-63.4^degree) + 1.0 times 1.0 times 35.77 cos(116.6^degree)
$$
$$
...
$$
P_3^("calc"(0)) = 1.0 times 1.05 times 31.62 cos(108.4^degree) + 1.0 times 1.0 times 35.77 cos(116.6^degree) + 1.0 times 67.23 cos(-67.2^degree)
$$
$$
...
$$
Q_2^("calc"(0)) = -1.0 times 1.05 times 22.36 sin(116.6^degree) - 1.0 times 58.13 sin(-63.4^degree) - 1.0 times 1.0 times 35.77 sin(116.6^degree)
$$
$$
...
$$
Q_3^("calc"(0)) = -1.0 times 1.05 times 31.62 sin(108.4^degree) - 1.0 times 1.0 times 35.77 sin(116.6^degree) - 1.0 times 67.23 sin(-67.2^degree)
$$
$$
...
**Power Residuals:**
$$
Delta P_2^((0)) = P_2^("sched") - P_2^("calc"(0)) = -2.556 - (-0.50) = -2.056
$$
$$
Delta P_3^((0)) = P_3^("sched") - P_3^("calc"(0)) = -1.386 - (-0.42) = -0.966
$$
$$
Delta Q_2^((0)) = Q_2^("sched") - Q_2^("calc"(0)) = -1.102 - (-1.00) = -0.102
$$
$$
Delta Q_3^((0)) = Q_3^("sched") - Q_3^("calc"(0)) = -0.452 - (-1.50) = 1.048
$$
...
$$
mat(Delta delta_2^((0)); Delta delta_3^((0))) = mat(52.97, -31.98; -31.98, 63.49)^(-1) mat(-2.056; -0.966)
$$

The solution is:
$$
Delta delta_2^((0)) = -0.08687 " rad" = -4.98^degree
$$
$$
Delta delta_3^((0)) = -0.0495 " rad" = -2.84^degree
$$

**Solve for $Delta V$:**
$$
mat(Delta V_2^((0)); Delta V_3^((0))) = mat(50.97, -31.98; -31.98, 60.48)^(-1) mat(-0.102; 1.048)
$$
...
**Update Voltages:**
$$
delta_2^((1)) = 0 + (-4.98^degree) = -4.98^degree
$$
$$
delta_3^((1)) = 0 + (-2.84^degree) = -2.84^degree
$$
$$
...
$$
Delta delta_2^((1)) = -0.229^degree
$$
$$
Delta delta_3^((1)) = -0.5^degree
$$
$$
Delta V_2^((1)) = -0.037
$$
...
**Update Voltages:**
$$
delta_2^((2)) = -4.98 + (-0.229) = -5.21^degree
$$
$$
delta_3^((2)) = -2.84 + (-0.5) = -3.34^degree
$$
$$
...
**After Second Iteration:**
- $delta_2^((2)) = -5.21^degree$, $V_2^((2)) = 0.9762$ pu
- $delta_3^((2)) = -3.34^degree$, $V_3^((2)) = 1.0000$ pu
...
**System Description:**
- **Bus 1:** Slack bus, $V_1 = 1.0 angle 0^degree$ pu, with a dummy load of $2 + j 1$ pu.
- **Bus 2:** PQ bus, injection $0.5 + j 1.0$ pu (no load).
- **Bus 3:** PV bus, load $1.5 + j 0.6$ pu, voltage magnitude to be maintained at $|V_3| = 1.0$ pu.

**Y-bus Matrix (given):**
- $Y_(11) = 26.925 angle -68.2^degree$
- $Y_(12) = 11.18 angle 116.6^degree$
- $Y_(13) = 15.81 angle 108.4^degree$
- $Y_(22) = 29.065 angle -63.4^degree$
- $Y_(23) = 17.885 angle 116.6^degree$
- $Y_(33) = 33.615 angle -67.2^degree$

**Scheduled Powers:**
- $P_2^("sched") = 0.5$ pu, $Q_2^("sched") = 1.0$ pu
- $P_3^("sched") = 0 - 1.5 = -1.5$ pu

**Note:** $Q_3$ is unknown because $Q_(g 3)$ is unknown. This is why bus 3 is a PV bus.
...
- $J_1$ is $2 times 2$ (for $Delta delta_2$ and $Delta delta_3$).
- $J_4$ is $1 times 1$ (for $Delta V_2$ only).

The decoupled equations are:
$$
mat(Delta P_2; Delta P_3) = mat(frac(partial P_2, partial delta_2), frac(partial P_2, partial delta_3); frac(partial P_3, partial delta_2), frac(partial P_3, partial delta_3)) mat(Delta delta_2; Delta delta_3)
$$
$$
...
**Elements of $J_1$:**
$$
frac(partial P_2, partial delta_2) = 1.0 times 1.0 times 11.18 sin(116.6^degree) + 1.0 times 1.0 times 17.885 sin(116.6^degree) = 11.18(0.894) + 17.885(0.894) = 10.0 + 16.0 = 26.0
$$
$$
frac(partial P_2, partial delta_3) = -1.0 times 1.0 times 17.885 sin(116.6^degree) = -16.0
$$
$$
frac(partial P_3, partial delta_2) = -1.0 times 1.0 times 17.885 sin(116.6^degree) = -16.0
$$
$$
frac(partial P_3, partial delta_3) = 1.0 times 1.0 times 15.81 sin(108.4^degree) + 1.0 times 1.0 times 17.885 sin(116.6^degree) = 15.81(0.949) + 17.885(0.894) = 15.0 + 16.0 = 31.0
$$

So, $J_1 = mat(26, -16; -16, 31)$

**Element of $J_4$:**
$$
frac(partial Q_2, partial V_2) = -1.0 times 11.18 sin(116.6^degree) - 2 times 1.0 times 29.065 sin(-63.4^degree) - 1.0 times 17.885 sin(116.6^degree)
$$
$$
...
**Calculated Powers:**
$$
P_2^("calc"(0)) = 1.0 times 1.0 times 11.18 cos(116.6^degree) + 1.0 times 29.065 cos(-63.4^degree) + 1.0 times 1.0 times 17.885 cos(116.6^degree)
$$
$$
...
$$
P_3^("calc"(0)) = 1.0 times 1.0 times 15.81 cos(108.4^degree) + 1.0 times 1.0 times 17.885 cos(116.6^degree) + 1.0 times 33.615 cos(-67.2^degree)
$$
$$
...
$$
Q_2^("calc"(0)) = -1.0 times 1.0 times 11.18 sin(116.6^degree) - 1.0 times 29.065 sin(-63.4^degree) - 1.0 times 1.0 times 17.885 sin(116.6^degree)
$$
$$
...
**Solve for $Delta delta$:**
$$
mat(Delta delta_2^((0)); Delta delta_3^((0))) = mat(26, -16; -16, 31)^(-1) mat(0.5; -1.5)
$$

The solution is:
$$
Delta delta_2^((0)) = -0.015 " rad" = -0.86^degree
$$
$$
Delta delta_3^((0)) = -0.056 " rad" = -3.2^degree
$$
...
**Update Voltages:**
$$
delta_2^((1)) = 0 + (-0.86^degree) = -0.86^degree
$$
$$
delta_3^((1)) = 0 + (-3.2^degree) = -3.2^degree
$$
$$
...
**Solve for Corrections (Iteration 1):**
$$
Delta delta_2^((1)) = -1.3^degree
$$
$$
Delta delta_3^((1)) = -0.15^degree
$$
$$
...
**Update Voltages:**
$$
delta_2^((2)) = -0.86 + (-1.3) = -2.16^degree
$$
$$
delta_3^((2)) = -3.2 + (-0.15) = -3.35^degree
$$
$$
...
**After Second Iteration:**
- $V_2^((2)) = 1.0464$, $delta_2^((2)) = -2.16^degree$
- $delta_3^((2)) = -3.35^degree$ ($V_3$ remains 1.0 pu)
...
In Example 3, the slack bus has a load of $2 + j 1$ pu. This load is a "dummy parameter" during the iterative process because it does not affect the voltage calculations at other buses. However, after convergence, when we calculate the slack bus generation, we must add this load to the calculated power injection.

If the calculated generation at the slack bus is $P_(g 1)^("calc")$ and $Q_(g 1)^("calc")$, then the actual generation is:
$$
P_(g 1)^("actual") = P_(g 1)^("calc") + P_(L 1)
$$
$$
Q_(g 1)^("actual") = Q_(g 1)^("calc") + Q_(L 1)
$$
...
![Three-bus example with slack and PQ buses](/content-assets/studies/power-system-analysis/Week%207/Assets/img-0.jpg)
...
![Gauss–Seidel voltage update calculation](/content-assets/studies/power-system-analysis/Week%207/Assets/img-10.jpg)
...
![Worked line-flow power calculation](/content-assets/studies/power-system-analysis/Week%207/Assets/img-31.jpg)
...
![Second-iteration bus-voltage result](/content-assets/studies/power-system-analysis/Week%207/Assets/img-60.jpg)
...
![Newton–Raphson correction and update equations](/content-assets/studies/power-system-analysis/Week%207/Assets/img-70.jpg)
...
![Reactive-power summation used for Jacobian elements](/content-assets/studies/power-system-analysis/Week%207/Assets/img-90.jpg)
...
![Newton–Raphson correction vector and updated voltages](/content-assets/studies/power-system-analysis/Week%207/Assets/img-110.jpg)
...
![PV-bus Jacobian submatrices](/content-assets/studies/power-system-analysis/Week%207/Assets/img-120.jpg)
...
2.  **Angle Quadrants:** When using a calculator, always check the quadrant of angles. For example, $angle 175.6^degree$ has a negative cosine and a positive sine. Incorrect quadrant selection leads to wrong results.
...
5.  **Slack Bus Load:** If there is a load at the slack bus, add it to the computed $P_(G 1)$ and $Q_(G 1)$ to find the total generation.
...
8.  **Jacobian Elements:** Diagonal elements of $J_1$ and $J_4$ are summations over $k != i$. Off-diagonal elements are single terms.
...
12. **Jacobian Size with PV Buses:** $J_1$ is always $(n-1) times (n-1)$. $J_4$ is $(n-m-1) times (n-m-1)$, where $m$ is the number of PV buses.
...
| **Y-bus Diagonal** | $Y_("ii") = sum_(k != i) y_("ik") + y_(i 0)$ |
| **Y-bus Off-Diagonal** | $Y_("ik") = -y_("ik")$ |
| **Gauss-Seidel Update (PQ)** | $V_i^((p+1)) = frac(1, Y_("ii")) [ frac(P_i - j Q_i, (V_i^((p)))^(star)) - sum_(k != i) Y_("ik") V_k ]$ |
| **GS PV Bus Q Calculation** | $Q_i = -sum_(k=1)^(n) |V_i||V_k||Y_("ik")| sin(theta_("ik") - delta_i + delta_k)$ |
| **GS PV Bus Voltage Correction** | Keep imaginary part $f$, set $e = sqrt(|V_i|^2 - f^2)$ |
| **Power Flow Equations** | $P_i = sum |V_i||V_k||Y_("ik")| cos(theta_("ik") - delta_i + delta_k)$ |
| | $Q_i = -sum |V_i||V_k||Y_("ik")| sin(theta_("ik") - delta_i + delta_k)$ |
| **NR Matrix Form** | $mat(Delta P; Delta Q) = mat(J_1, J_2; J_3, J_4) mat(Delta delta; Delta V)$ |
| **Decoupled NR** | $Delta P = J_1 Delta delta$, $Delta Q = J_4 Delta V$ |
| **$J_1$ Diagonal** | $frac(partial P_i, partial delta_i) = sum_(k != i) |V_i||V_k||Y_("ik")| sin(theta_("ik") - delta_i + delta_k)$ |
| **$J_1$ Off-Diagonal** | $frac(partial P_i, partial delta_k) = -|V_i||V_k||Y_("ik")| sin(theta_("ik") - delta_i + delta_k)$ |
| **$J_4$ Diagonal** | $frac(partial Q_i, partial V_i) = -2|V_i||Y_("ii")| sin(theta_("ii")) - sum_(k != i) |V_k||Y_("ik")| sin(theta_("ik") - delta_i + delta_k)$ |
| **$J_4$ Off-Diagonal** | $frac(partial Q_i, partial V_k) = -|V_i||Y_("ik")| sin(theta_("ik") - delta_i + delta_k)$ |
| **Power Residuals** | $Delta P_i^((p)) = P_i^("sched") - P_i^("calc"(p))$, $Delta Q_i^((p)) = Q_i^("sched") - Q_i^("calc"(p))$ |
...
| **Jacobian Size** | $J_1$: $(n-1) times (n-1)$; $J_4$: $(n-m-1) times (n-m-1)$ |
...
Options: (a) $(n-1) times (n-1)$ (b) $(n-m) times (n-m)$ (c) $(n-m-1) times (n-m-1)$ (d) $m times m$

> Answer and explanation
> The correct answer is (c) $(n-m-1) times (n-m-1)$. The $J_4$ matrix relates reactive power mismatches ($Delta Q$) to voltage magnitude corrections ($Delta V$). Since the slack bus (1) and all PV buses (m) have fixed voltage magnitudes, they are excluded from this sub-problem. Therefore, the size is $(n-1-m) times (n-1-m)$.
...
Options: (a) Diagonal elements of $J_1$ are summations over all $k != i$ (b) Off-diagonal elements of $J_1$ are single terms (c) The diagonal elements of $J_4$ have a factor of 2 on the self-term (d) The Jacobian matrix is always constant and does not change between iterations
...
> Answer and explanation
> The power residual $Delta P_i$ is the difference between the scheduled real power injection ($P_i^("scheduled")$, which is the known generation minus load) and the calculated real power injection ($P_i^("calculated")$) obtained from the power flow equations using the current voltage estimates. Mathematically, $Delta P_i^((p)) = P_i^("scheduled") - P_i^("calculated"(p))$. The NR method iterates to drive all residuals to zero.

**Q12. Why is the factor of 2 present in the diagonal elements of the Jacobian sub-matrix $J_4$?**

> Answer and explanation
> The diagonal elements of $J_4$ are $frac(partial Q_i, partial V_i)$. The reactive power equation $Q_i$ contains a term for $k=i$ which is $|V_i|^2 |Y_("ii")| sin(theta_("ii"))$. When this term is differentiated with respect to $|V_i|$, the power rule brings down a factor of 2, resulting in $2|V_i||Y_("ii")| sin(theta_("ii"))$. This factor of 2 is unique to the diagonal elements.

**Q13. What is the "flat voltage start" and why is it commonly used?**

> Answer and explanation
> A "flat voltage start" is an initial guess for the load-flow iteration where all unknown bus voltages are set to $1.0 angle 0^degree$ per unit (or the slack bus voltage magnitude with an angle of 0°). It is commonly used because it is a simple and reasonable initial guess that generally leads to good convergence behavior for both the Gauss-Seidel and Newton-Raphson methods.

**Q14. For the three-bus system in Lecture 31, after the second Gauss-Seidel iteration, the voltage at bus 2 is $V_2 = 0.98265 angle -3.048^degree$ pu. If the slack bus voltage is $V_1 = 1.05 angle 0^degree$ pu and $Y_(12) = 22.36 angle 116.6^degree$, calculate the real power flow $P_(12)$ on the line from bus 1 to bus 2.**

> Answer and explanation
> The real power flow from bus 1 to bus 2 is given by:
> $P_(12) = |V_1|^2 |Y_(12)| cos(theta_(12)) - |V_1||V_2||Y_(12)| cos(theta_(12) - delta_1 + delta_2)$
> Substituting the values:
> $P_(12) = (1.05)^2(22.36) cos(116.6^degree) - (1.05)(0.98265)(22.36) cos(116.6^degree - 0 + (-3.048^degree))$
> $P_(12) = 24.65(-0.448) - 23.07 cos(113.55^degree)$
> $P_(12) = -11.04 - 23.07(-0.398)$
> $P_(12) = -11.04 + 9.18 = -1.86 " pu"$
...
**Q15. In the Newton-Raphson example from Lecture 35 (Example 3), the initial power residuals are $Delta P_2 = 0.5$ pu, $Delta P_3 = -1.5$ pu, and $Delta Q_2 = 1.0$ pu. The Jacobian is $J_1 = mat(26, -16; -16, 31)$ and $J_4 = [26]$. Calculate the voltage corrections $Delta delta_2$, $Delta delta_3$, and $Delta V_2$ for the first iteration.**

> Answer and explanation
> First, solve for $Delta delta$:
> $mat(Delta delta_2; Delta delta_3) = mat(26, -16; -16, 31)^(-1) mat(0.5; -1.5)$
> The inverse of $J_1$ is $frac(1, (26)(31) - (-16)(-16)) mat(31, 16; 16, 26) = frac(1, 806 - 256) mat(31, 16; 16, 26) = frac(1, 550) mat(31, 16; 16, 26)$
> $Delta delta_2 = frac(31(0.5) + 16(-1.5), 550) = frac(15.5 - 24, 550) = frac(-8.5, 550) = -0.01545 " rad" = -0.885^degree$
> $Delta delta_3 = frac(16(0.5) + 26(-1.5), 550) = frac(8 - 39, 550) = frac(-31, 550) = -0.05636 " rad" = -3.23^degree$
...
**Q16. A transmission line has a series impedance of $Z = 0.01 + j 0.1$ pu. What is the approximate R/X ratio, and what does this imply for the decoupled load-flow method?**
