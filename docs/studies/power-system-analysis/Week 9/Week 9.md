...
$$
"L" = C_T + lambda(P_L + P_("Loss") - sum_(i=1)^(m) P_("gi"))
$$

Taking the partial derivative with respect to each $P_("gi")$:

$$
frac(partial "L", partial P_("gi")) = frac(d C_i, d P_("gi")) + lambda(frac(partial P_("Loss"), partial P_("gi")) - 1) = 0
$$

This gives the **coordination equation**:

$$
frac(d C_i, d P_("gi")) + lambda frac(d P_("Loss"), d P_("gi")) = lambda, space i = 1, 2, dots.h, m
$$

Rearranging:

$$
frac(d C_i, d P_("gi")) = lambda(1 - frac(d P_("Loss"), d P_("gi")))
$$

$$
lambda = frac(frac(d C_i, d P_("gi")), 1 - frac(d P_("Loss"), d P_("gi")))
$$
...
$$
L_i = frac(1, 1 - frac(d P_("Loss"), d P_("gi")))
$$

Then the coordination equation becomes:

$$
L_i dot frac(d C_i, d P_("gi")) = lambda
$$
...
$$
Delta C_T = sum_(i=1)^(m) frac(d C_i(P_("gi")^0), d P_("gi")) Delta P_("gi")
$$
...
$$
P_("Loss") = P_("Loss")^0 + sum_(i=2)^(m) frac(partial P_("Loss")(P_g^0), partial P_("gi")) dot Delta P_("gi")
$$

we get:

$$
Delta P_("Loss") = sum_(i=2)^(m) frac(partial P_("Loss")(P_g^0), partial P_("gi")) dot Delta P_("gi")
$$

Substituting into the power balance:

$$
Delta P_(g 1) + sum_(i=2)^(m)[1 - frac(partial P_("Loss")(P_g^0), partial P_("gi"))]Delta P_("gi") = Delta P_L
$$
...
Using the coordination condition $L_i times "IC"_i = lambda$:
...
**Given:**
- Loss expression: $P_("Loss") = 0.001(P_(g 2) - 70)^2$ MW
- Loads: $"PL"_1 = 300$ MW, $"PL"_2 = 70$ MW, total $= 370$ MW
- Cost functions: $C_1 = 500 + 41 P_(g 1) + 0.175 P_(g 1)^2$, $C_2 = 500 + 41 P_(g 2) + 0.175 P_(g 2)^2$

**Step 1 — Derivative of loss:**

$$
frac(d P_("Loss"), d P_(g 2)) = 0.002 P_(g 2) - 0.14
$$

**Step 2 — Penalty factors:**
- $L_1 = 1$ (slack/reference generator)
- $L_2 = frac(1, 1 - frac(d P_("Loss"), d P_(g 2))) = frac(1, 1.14 - 0.002 P_(g 2))$

**Step 3 — Coordination equations:**
- Unit 1: $L_1 frac(d C_1, d P_(g 1)) = 0.35 P_(g 1) + 41 = lambda$
- Unit 2: $L_2 frac(d C_2, d P_(g 2)) = frac(0.35 P_(g 2) + 41, 1.14 - 0.002 P_(g 2)) = lambda$

**Step 4 — Solving for generations:**
- From unit 1: $P_(g 1) = frac(lambda - 41, 0.35)$
- From unit 2: $P_(g 2) = frac(1.14 lambda - 41, 0.35 + 0.002 lambda)$

**Key observation:** $P_(g 1)$ vs λ is linear (since $L_1 = 1$), but $P_(g 2)$ vs λ is **non-linear** (λ appears in both numerator and denominator). Therefore, **iterative solution is required**.
...
$$
P_("Loss") = 0.001(159.029 - 70)^2 = 7.926 " MW"
$$

**Step 6 — Check power balance:**

$$
P_(g 1) + P_(g 2) - P_("Loss") = 218.857 + 159.029 - 7.926 = 369.96 " MW" approx 370 " MW" = "PL"_1 + "PL"_2
$$
...
$$
frac(d C_i, d P_("gi")) = lambda
$$

With quadratic cost $C_i = a_i + b_i P_("gi") + d_i P_("gi")^2$:

$$
b_i + 2 d_i P_("gi") = lambda
$$

Therefore:

$$
P_("gi") = frac(lambda - b_i, 2 d_i)
$$
...
$$
f(lambda) = sum_(i=1)^(m) P_("gi") = sum_(i=1)^(m) frac(lambda - b_i, 2 d_i)
$$
...
$$
f(lambda)^K + (frac("df"(lambda), dif lambda))^K Delta lambda^K = P_L
$$

- **Correction formula:**

$$
Delta lambda^K = frac(P_L - f(lambda)^K, (frac("df"(lambda), dif lambda))^K)
$$
...
$$
frac("df"(lambda), dif lambda) = sum_(i=1)^(m) frac(1, 2 d_i)
$$

- **Combining:**

$$
Delta lambda^((K)) = frac(Delta P_g^((K)), sum_(i=1)^(m) frac(1, 2 d_i))
$$
...
$$
frac(d C_i, d P_("gi")) = lambda - lambda frac(d P_("Loss"), d P_("gi")) = lambda (1 - frac(d P_("Loss"), d P_("gi")))
$$
...
1. **Non-linearity trap:** When losses are present, $P_(g 2)$ vs λ is non-linear. Do NOT try to solve directly — you must iterate.

2. **Slack bus penalty factor:** $L_1 = 1$ always for the slack generator. Do not compute it from the formula.

3. **Sign of loss derivative:** $frac(d P_("Loss"), d P_("gi"))$ can be positive or negative. A negative derivative means the penalty factor is less than 1, which would encourage more generation from that unit.

4. **Units:** λ is in Rs/MWhr (or USD/MWhr). Ensure all costs are in consistent units.

### Lecture-End Recap

- The coordination equation with losses is $frac(d C_i, d P_("gi")) + lambda frac(d P_("Loss"), d P_("gi")) = lambda$
- Penalty factors modify incremental costs: $L_i dot "IC"_i = lambda$
...
$$
frac(d P_("Loss"), d P_("gi")) = 2 sum_(j=1)^(m) B_("ij") P_("gj")
$$

**Illustrative example (m = 2):**

$$
P_("Loss") = P_(g 1) B_(11) P_(g 1) + P_(g 1) B_(12) P_(g 2) + P_(g 2) B_(21) P_(g 1) + P_(g 2) B_(22) P_(g 2)
$$

$$
= P_(g 1)^(2) B_(11) + B_(12) P_(g 1) P_(g 2) + B_(21) P_(g 1) P_(g 2) + B_(22) P_(g 2)^(2)
$$

If $B_(12) = B_(21)$:

$$
P_("Loss") = P_(g 1)^(2) B_(11) + 2 B_(12) P_(g 1) P_(g 2) + B_(22) P_(g 2)^(2)
$$

**Derivative with respect to $P_(g 1)$:**

$$
frac(d P_("Loss"), d P_(g 1)) = 2 P_(g 1) B_(11) + 2 B_(12) P_(g 2)
$$
...
$$
b_i + 2 d_i P_("gi") + 2 lambda sum_(j=1)^(m) B_("ij") P_("gj") = lambda
$$

Separating the $j = i$ term:

$$
b_i + 2 d_i P_("gi") + 2 lambda B_("ii") P_("gi") + 2 lambda sum_(j=1 j != i)^(m) B_("ij") P_("gj") = lambda
$$

**Solving for $P_("gi")$:**

$$
P_("gi") = frac(lambda - b_i - 2 lambda sum_(j=1 j != i)^(m) B_("ij") P_("gj"), 2(d_i + lambda B_("ii")))
$$

**At the $k^("th")$ iteration:**

$$
P_("gi")^((K)) = frac(lambda^((K)) - b_i - 2 lambda^((K)) sum_(j=1 j != i)^(m) B_("ij") P_("gj")^((K)), 2(d_i + lambda^((K)) B_("ii")))
$$
...
$$
f(lambda)^((K)) + frac("df"(lambda)^((K)), dif lambda) Delta lambda^((K)) = P_L + P_("Loss")^((K))
$$

**Correction formula:**

$$
Delta lambda^((k)) = frac(Delta P_g^((k)), sum_(i=1)^m (frac(d P_("gi"), dif lambda))^((k)))
$$
...
$$
sum_(i=1)^m (frac(d P_("gi"), dif lambda))^((k)) = sum_(i=1)^m [ frac(d_i + B_("ii") b_i - 2 d_i sum_(j=1 j != i)^(m) B_("ij") P_("gj")^((k)), 2(d_i + lambda^((k)) B_("ii"))^2) ]
$$
...
This corresponds to setting $B_("ij") = 0.0$ for $i != j$.
...
$$
sum_(i=1)^(m) ( frac(d P_("gi"), dif lambda) )^((K)) = sum_(i=1)^(m) frac(d_i + B_("ii") b_i, 2(d_i + lambda^((K)) B_("ii"))^2)
$$
...
$$
C_1 = 500 + 41 P_(g 1) + 0.15 P_(g 1)^2
$$
$$
C_2 = 400 + 44 P_(g 2) + 0.1 P_(g 2)^2
$$
$$
C_3 = 300 + 40 P_(g 3) + 0.18 P_(g 3)^2
$$
...
**Data extraction:**
$$
a_1 = 500, space b_1 = 41, space d_1 = 0.15
$$
$$
a_2 = 400, space b_2 = 44, space d_2 = 0.10
$$
$$
a_3 = 300, space b_3 = 40, space d_3 = 0.18
$$

**Using the generation formula:**

$$
P_("gi")^((K)) = frac(lambda^((K)) - b_i, 2 d_i)
$$
...
$$
P_(g 1)^((1)) = frac(50 - 41, 2 times 0.15) = 30 " MW"
$$

$$
P_(g 2)^((1)) = frac(50 - 44, 2 times 0.1) = 30 " MW"
$$

$$
P_(g 3)^((1)) = frac(50 - 40, 2 times 0.18) = 27.77 " MW"
$$

**Mismatch:**

$$
Delta P_g^((1)) = 850 - (30 + 30 + 27.77) = 762.23 " MW"
$$

**Using the correction formula:**

$$
Delta lambda^((1)) = frac(762.23, frac(1, 2 times 0.15) + frac(1, 2 times 0.10) + frac(1, 2 times 0.18)) = 68.007
$$
...
$$
P_(g 1)^((2)) = frac(118.6007 - 41, 2 times 0.15) = 258.669 " MW"
$$

$$
P_(g 2)^((2)) = frac(118.6007 - 44, 2 times 0.10) = 373.0035 " MW"
$$

$$
P_(g 3)^((2)) = frac(118.6007 - 40, 2 times 0.18) = 218.335 " MW"
$$
...
$$
= 500 + 41 times 258.669 + 0.15 times (258.669)^2 + 400 + 44 times 373.0035 + 0.10 times (373.0035)^2 + 300 + 40 times 218.335 + 0.18 times (218.335)^2
$$
...
**Issue:** After second iteration of Example 5, $P_(g 2) = 373.0035$ MW exceeds the upper limit of 350 MW.

**Action:** Fix $P_(g 2) = 350$ MW (at its upper limit) and keep it constant.

**New mismatch at iteration 2:**

$$
Delta P_g^((2)) = 850 - (258.669 + 350 + 218.335) = 23 " MW"
$$

**Note:** Since $P_(g 2)$ is fixed, it is excluded from the denominator sum:

$$
Delta lambda^((2)) = frac(23, frac(1, 2 times 0.15) + frac(1, 2 times 0.18)) = 3.763
$$
...
$$
P_(g 1)^((3)) = frac(122.3637 - 41, 2 times 0.15) = 271.21 " MW" space "(within limits)"
$$

$$
P_(g 2) = 350 " MW" space "(fixed at limit)"
$$

$$
P_(g 3)^((3)) = frac(122.3637 - 40, 2 times 0.18) = 228.79 " MW" space "(within limits)"
$$
...
- $P_L = 370$ MW
- Loss expression: $P_("Loss") = 0.0005 P_(g 1)^2 + 0.00008 P_(g 2)^2$
...
$$
P_(g 1)^((1)) = frac(60-41, 2(0.175+60 times 0.00005)) = 53.37 " MW"
$$

$$
P_(g 2)^((1)) = frac(60-41, 2(0.175+60 times 0.00008)) = 52.83 " MW"
$$

**Power loss:**

$$
P_("Loss")^((1)) = 0.00005 times (53.37)^2 + 0.00008 times (52.83)^2 = 0.3657 " MW"
$$

**Mismatch:**

$$
Delta P_g^((1)) = 370 + 0.3657 - (53.37 + 52.83) = 264.1657 " MW"
$$

**Derivative sum (m = 2):**

$$
sum_(i=1)^(2) ( frac(d P_("gi"), dif lambda) )^((1)) = frac(0.175 + 41 times 0.00005, 2(0.175 + 60 times 0.00005)^2) + frac(0.175 + 41 times 0.00008, 2(0.175 + 60 times 0.00008)^2) = 5.551
$$
...
$$
P_(g 1)^((2)) = frac(107.59 - 41, 2(0.175 + 107.59 times 0.00005)) = 184.58 " MW"
$$

$$
P_(g 2)^((2)) = frac(107.59 - 41, 2(0.175 + 107.59 times 0.00008)) = 181.34 " MW"
$$

**Power loss:**

$$
P_("Loss")^((2)) = 0.00005 times (184.58)^2 + 0.00008 times (181.34)^2 = 4.334 " MW"
$$
...
2. **Diagonal approximation:** In many practical studies, off-diagonal B coefficients are neglected ($B_("ij") = 0$ for $i != j$). This simplifies the computation significantly.
...
- The derivative of the quadratic loss formula is $frac(d P_("Loss"), d P_("gi")) = 2 sum_j B_("ij") P_("gj")$
- The iterative solution for generation with losses is $P_("gi")^((K)) = frac(lambda^((K)) - b_i - 2 lambda^((K)) sum_(j!= i) B_("ij") P_("gj")^((K)), 2(d_i + lambda^((K)) B_("ii")))$
...
$$
Delta P_g^((2)) = 370 + 4.334 - (184.58 + 181.34) = 8.414 " MW"
$$

**Derivative sum at iteration 2:**

$$
sum_(i=1)^(2) ( frac(d P_("gi"), dif lambda) )^((2)) = frac(0.175 + 41 times 0.00005, 2(0.175 + 107.59 times 0.00005)^2) + frac(0.175 + 41 times 0.00008, 2(0.175 + 107.59 times 0.00008)^2) = 5.364
$$
...
$$
P_(g 1)^((3)) = frac(109.1586 - 41, 2(0.175 + 109.1586 times 0.00005)) = 188.849 " MW"
$$

$$
P_(g 2)^((3)) = frac(109.1586 - 41, 2(0.175 + 109.1586 times 0.00008)) = 185.483 " MW"
$$

**Power loss:**

$$
P_("Loss")^((3)) = 0.00005 times (188.849)^2 + 0.00008 times (185.483)^2 = 4.535 " MW"
$$

**Mismatch:**

$$
Delta P_g^((3)) = 370 + 4.535 - (188.849 + 185.483) = 0.203 " MW"
$$

**Derivative sum at iteration 3:** $sum_(i=1)^(2) ( frac(d P_("gi"), dif lambda) )^((3)) = 5.358$ (given as final answer; students should verify)
...
$$
P_("Loss")^((4)) = 0.00005 times (188.95)^2 + 0.00008 times (185.58)^2 = 4.54 " MW"
$$

**Mismatch:**

$$
Delta P_g^((4)) = 370 + 4.54 - (188.95 + 185.58) = 0.01 " MW"
$$
...
- $P_("gi")$ = real power generation at bus $i$, $i = 1, 2, dots.h, n$
- $P_("Li")$ = real power load at bus $i$, $i = 1, 2, dots.h, n$
- $P_i$ = net real power injected at bus $i$, $i = 1, 2, dots.h, n$

**Expanded form:**

$$
P_("Loss") = P_1 + P_2 + dots.h + P_m + P_(m+1) + P_(m+2) + dots.h + P_n
$$

**Derivative with respect to δ:**

$$
frac(d P_("Loss"), d delta_K) = frac(d P_1, d delta_K) + frac(d P_2, d delta_K) + dots.h + frac(d P_m, d delta_K) + frac(d P_(m+1), d delta_K) + dots.h + frac(d P_n, d delta_K)
$$

for $k = 2, 3, dots.h, n$

**Key point:** For a given set of $delta_2, delta_3, dots.h, delta_n$, the derivatives $frac(d P_i, d delta_K)$ can be computed explicitly from load flow equations.
...
$$
frac(d P_i, d delta_K) = frac(d P_("gi"), d delta_K)
$$

**Chain rule of differentiation:**

$$
frac(d P_("Loss"), d delta_K) = frac(d P_("Loss"), d P_(g 2)) frac(d P_(g 2), d delta_K) + frac(d P_("Loss"), d P_(g 3)) frac(d P_(g 3), d delta_K) + dots.h + frac(d P_("Loss"), d P_("gm")) frac(d P_("gm"), d delta_K)
$$

for $k = 2, 3, dots.h, n$

**Note:** $P_("Loss")$ does not include $P_(g 1)$ (slack bus generation), so $frac(d P_("Loss"), d P_(g 1)) = 0$.

**Using the relationship to substitute:**

$$
frac(d P_("gi"), d delta_K) = frac(d P_i, d delta_K)
$$

**Subtracting the equations:**

$$
frac(d P_1, d delta_K) + frac(d P_2, d delta_K)(1 - frac(d P_("Loss"), d P_(g 2))) + dots.h + frac(d P_m, d delta_K)(1 - frac(d P_("Loss"), d P_("gm"))) + frac(d P_(m+1), d delta_K) + dots.h + frac(d P_n, d delta_K) = 0
$$

**Rearranging:**

$$
frac(d P_2, d delta_K)(1 - frac(d P_("Loss"), d P_(g 2))) + dots.h + frac(d P_m, d delta_K)(1 - frac(d P_("Loss"), d P_("gm"))) + frac(d P_(m+1), d delta_K) + dots.h + frac(d P_n, d delta_K) = -frac(d P_1, d delta_K)
$$

**Matrix form:**

$$
mat(frac(d P_2, d delta_2), dots.h, frac(d P_m, d delta_2), dots.h, frac(d P_n, d delta_2); dots.v, dots.down, dots.v, dots.v, dots.v; frac(d P_2, d delta_n), dots.h, frac(d P_m, d delta_n), dots.h, frac(d P_n, d delta_n)) mat(1 - frac(d P_("Loss"), d P_(g 2)); dots.v; 1 - frac(d P_("Loss"), d P_("gm")); 1; dots.v; 1) = - mat(frac(d P_1, d delta_2); dots.v; frac(d P_1, d delta_n))
$$
...
$$
frac(d P_1, d delta_K) = |V_1||V_K|[G_(1 K) sin(delta_1 - delta_k) - B_(1 K) cos(delta_1 - delta_K)]
$$
...
4. Solve the matrix equation for the vector containing $(1 - frac(d P_("Loss"), d P_("gi")))$ terms
...
3. **Constant loads:** Loads $P_("Li")$ are assumed constant, so $frac(d P_i, d delta_K) = frac(d P_("gi"), d delta_K)$.
...
- $V_1 = 1 angle 0^degree$ (slack/reference bus)
- $V_2 = 1 angle delta_2$
- $V_3 = 1 angle delta_3$
- $P_(L 3) = 2.0$ p.u. (load at bus 3)
- Line 1-2 is **not connected**
- Incremental costs: $"IC"_1 = 4 + 0.6 P_(g 1)$, $"IC"_2 = 4 + 0.6 P_(g 2)$

**Y_BUS matrix:**

$$
Y_("BUS") = mat(1 - j 10, 0, -1 + j 10; 0, 0.5 - j 5, -0.5 + j 5; -1 + j 10, -0.5 + j 5, 1.5 - j 15)
$$

**Objective:** Find optimal generation scheduling $P_(g 1)$ and $P_(g 2)$.

**Step 1 — Power injection formula:**

$$
P_i = sum_(k=1)^(n)|V_i||V_k||Y_("ik")|cos(theta_("ik") - delta_i + delta_k)
$$

$$
P_i = sum_(k=1)^(n)|V_i||V_k||Y_("ik")|[cos theta_("ik") cos(delta_i-delta_k) + sin theta_("ik") sin(delta_i-delta_k)]
$$
...
$$
|Y_("ik")|cos theta_("ik") = G_("ik"); space |Y_("ik")|sin theta_("ik") = B_("ik"); space Y_("ik") = G_("ik") + j B_("ik")
$$
...
$$
frac(d P_2, d delta_2) = 0.5 sin delta_(23) + 5 cos delta_(23)
$$

$$
frac(d P_2, d delta_3) = -0.5 sin delta_(23) - 5 cos delta_(23)
$$

$$
frac(d P_3, d delta_2) = 0.5 sin delta_(23) - 5 cos delta_(23)
$$

$$
frac(d P_3, d delta_3) = sin delta_(31) + 10 cos delta_(31) - 0.5 sin delta_(23) + 5 cos delta_(23)
$$

$$
frac(d P_1, d delta_2) = 0.0 space "(P₁ not a function of δ₂)"
$$

$$
frac(d P_1, d delta_3) = sin delta_(31) - 10 cos delta_(31)
$$

**Step 8 — Matrix equation:**

$$
mat(frac(d P_2, d delta_2), frac(d P_3, d delta_2); frac(d P_2, d delta_3), frac(d P_3, d delta_3)) mat(1 - frac(d P_("Loss"), d P_(g 2)); 1) = - mat(0; frac(d P_1, d delta_3))
$$

**Step 9 — Iterative solution:**

Assume initial $P_(g 2)^0 = 1.0$ p.u.

Since bus 2 is a generator bus: $P_2 = P_(g 2)^0 = 1.0$ p.u.
...
**Solving iteratively (trial and error):** $delta_(23) = 11.5^degree$

At bus 3 (load only): $P_3 = -2.0$

From the equation for $P_3$ with $delta_(23) = 11.5^degree$:

$$
10 sin delta_(31) - cos delta_(31) = -2.0132
$$

**Solving iteratively:** $delta_(31) = -5.85^degree$

**Step 10 — Slack bus power:**

$$
P_1 = P_(g 1) = 1 - cos(-5.85^degree) - 10 sin(-5.85^degree) = 1.024 " p.u."
$$

**Incremental costs:**

$$
"IC"_1 = 4 + 0.6 times 1.024 = 4.6144
$$

$$
"IC"_2 = 4 + 0.6 times 1.0 = 4.60
$$

**Step 11 — Solve the matrix equation for penalty factor:**

$$
L_2 = frac(1, 1 - frac(d P_("Loss"), d P_(g 2))) = 1.0203
$$

**Step 12 — Check coordination:**

$$
L_1 "IC"_1 = 4.6144 space (L_1 = 1.0)
$$

$$
L_2 "IC"_2 = 1.0203 times 4.60 = 4.6933
$$

**Since $L_2 "IC"_2 > L_1 "IC"_1$:** Need to **decrease** $P_(g 2)$.
...
- Generator currents: $I_(g 1)$, $I_(g 2)$
...
$$
I_K = A_(K 1) I_(g 1) + A_(K 2) I_(g 2)
$$
...
$$
P_("Li") - j Q_("Li") = V_i^(star) I_("Li")
$$

$$
I_("Li") = frac(P_("Li") - j Q_("Li"), V_i^(star))
$$

$$
I_("Li") = frac(sqrt(P_("Li")^2 + Q_("Li")^2) angle-phi.alt_i, |V_i|angle-delta_i) = frac(sqrt(P_("Li")^2 + Q_("Li")^2) angle(delta_i - phi.alt_i), |V_i|)
$$

**Key assumption:** $(delta_i - phi.alt_i) = beta_i$ is the same for all load currents.

**Consequence:** $I_(K 1)$ and $I_L$ have the same phase angle; $I_(K 2)$ and $I_L$ have the same phase angle. Therefore, **$A_(K 1)$ and $A_(K 2)$ are real quantities.**
...
3. **Same load current phase angle:** All load currents have the same phase angle $(delta_i - phi.alt_i) = beta_i$. This is an approximation that holds when loads have similar power factors and voltage angles.
...
4. **Coordination check:** After computing penalty factors, always verify that $L_i dot "IC"_i$ is the same for all generators. If not, adjust the generation schedule.
...
$$
I_(g 1) = |I_(g 1)| angle alpha_1, space I_(g 2) = |I_(g 2)| angle alpha_2
$$

**Substituting into the superposition equation:**

$$
I_K = A_(K 1)|I_(g 1)|angle alpha_1 + A_(K 2)|I_(g 2)|angle alpha_2
$$

**Expanding into real and imaginary parts:**

$$
I_K = (A_(K 1)|I_(g 1)|cos alpha_1 + A_(K 2)|I_(g 2)|cos alpha_2) + j(A_(K 1)|I_(g 1)|sin alpha_1 + A_(K 2)|I_(g 2)|sin alpha_2)
$$

**Magnitude squared:**

$$
|I_K|^2 = A_(K 1)^2|I_(g 1)|^2 + A_(K 2)^2|I_(g 2)|^2 + 2 A_(K 1) A_(K 2)|I_(g 1)||I_(g 2)|cos(alpha_1 - alpha_2)
$$

#### Generator Currents in Terms of Power

$$
|I_(g 1)| = frac(P_1, sqrt(3)|V_1|cos phi_1)
$$

$$
|I_(g 2)| = frac(P_2, sqrt(3)|V_2|cos phi_2)
$$
...
$$
P_("Loss") = ( frac(P_1^2, |V_1|^2 cos^2 phi.alt_1) ) sum_(K=1)^("NBR") A_(K 1)^2 R_K + ( frac(2 P_1 P_2 cos(alpha_1 - alpha_2), |V_1| |V_2| cos phi.alt_1 cos phi.alt_2) ) sum_(K=1)^("NBR") A_(K 1) A_(K 2) R_K + ( frac(P_2^2, |V_2|^2 cos^2 phi.alt_2) ) sum_(K=1)^("NBR") A_(K 2)^2 R_K
$$
...
$$
P_("Loss") = B_(11) P_1^2 + 2 P_1 P_2 B_(12) + B_(22) P_2^2
$$

where:

$$
B_(11) = ( frac(1, |V_1|^2 cos^2 phi.alt_1) ) sum_(K=1)^("NBR") A_(K 1)^2 R_K
$$

$$
B_(12) = ( frac(cos(alpha_1 - alpha_2), |V_1| |V_2| cos phi.alt_1 cos phi.alt_2) ) sum_(K=1)^("NBR") A_(K 1) A_(K 2) R_K
$$

$$
B_(22) = ( frac(1, |V_2|^2 cos^2 phi.alt_2) ) sum_(K=1)^("NBR") A_(K 2)^2 R_K
$$

**Units of B coefficients:** Since $P_("Loss")$ is in MW, the B coefficients have units of $"MW"^(-1)$.

**General form:**

$$
B_("pq") = ( frac(cos(alpha_p - alpha_q), |V_p||V_q| cos phi.alt_p cos phi.alt_q) ) sum_(K=1)^("NBR") A_("Kp") A_("Kq") R_K
$$
...
- Bus 1: reference bus, $V_1 = 1 angle 0^degree$, generator current $I_(g 1)$
- Bus 2: generator current $I_(g 2)$
...
$$
I_L = I_3 + I_4 = (7.2 - j 1.8) + (2 - j 0.5) = (9.2 - j 2.3) " p.u."
$$

**Step 2 — Plant 2 off (Figure 14):**
- Direction of current in branch 2 changes
- $I_1 = I_3 + I_2 = I_3 + I_4 = I_L = (9.2 - j 2.3)$ p.u.
- $I_2 = I_4 = (2 - j 0.50)$ p.u.

**Using the distribution factor formula ($A_(K 1) = frac(I_(K 1), I_L)$):**
...
$$
A_(21) = frac(-I_2, I_L) = frac(-(2-j 0.5), (9.2-j 2.3)) = -0.2174
$$

$$
A_(31) = frac(I_3, I_L) = frac((7.2-j 1.8), (9.2-j 2.3)) = 0.7826
$$

$$
A_(41) = frac(I_4, I_L) = frac((2-j 0.50), (9.2-j 2.3)) = 0.2174
$$
...
**Bus 1 Voltage:**
- Given: $V_r = 1 angle 0^degree$, $I_1 = 4 - j 1$, $Z_1 = 0.02 + j 0.08$
- Formula: $V_1 = V_r + I_1 Z_1$
- Computation: $V_1 = 1 angle 0^degree + (4 - j 1)(0.02 + j 0.08)$
- Result: $V_1 = 1.198 angle 14.5^degree$
- Conclusion: $delta_1 = 14.5^degree$ (voltage angle at bus 1)

**Bus 2 Voltage:**
- Given: $V_r = 1 angle 0^degree$, $I_2 = 3.2 - j 0.8$, $Z_2 = 0.02 + j 0.08$
- Formula: $V_2 = V_r + I_2 Z_2$
- Computation: $V_2 = 1 angle 0^degree + (3.2 - j 0.8)(0.02 + j 0.08)$
- Result: $V_2 = 1.153 angle 12^degree$
- Conclusion: $delta_2 = 12^degree$ (voltage angle at bus 2)

#### Generator Currents and Angles

**Generator 1 Current:**
- Given: $I_(g 1) = 4 - j 1$
- Magnitude-angle form: $I_(g 1) = 4.123 angle -14^degree$
- Conclusion: $alpha_1 = -14^degree$ (current angle for generator 1)

**Generator 2 Current (via Kirchhoff's First Law):**
- Given: $I_2 = 3.2 - j 0.8$, $I_4 = 2 - j 0.5$
- Formula (KCL at bus): $I_(g 2) = I_2 + I_4$
- Computation: $I_(g 2) = (3.2 - j 0.8) + (2 - j 0.5) = 5.2 - j 1.3$
- Magnitude-angle form: $I_(g 2) = 5.36 angle -14^degree$
- Conclusion: $alpha_2 = -14^degree$ (current angle for generator 2)

#### Generating Station Power Factors

**Angle Difference Between Generator Currents:**
- Given: $alpha_1 = -14^degree$, $alpha_2 = -14^degree$
- Computation: $alpha_1 - alpha_2 = 0^degree$
- Result: $cos(alpha_1 - alpha_2) = cos 0^degree = 1.0$

**Power Factor for Generator 1:**
- Formula: $cos phi.alt_1 = cos(delta_1 + alpha_1)$
- Given: $delta_1 = 14.5^degree$, $alpha_1 = -14^degree$
- Computation: $cos phi.alt_1 = cos(14.5^degree + 14^degree) = cos(28.5^degree)$
- Result: $cos phi.alt_1 = 0.8788$

**Power Factor for Generator 2:**
- Formula: $cos phi.alt_2 = cos(delta_2 + alpha_2)$
- Given: $delta_2 = 12^degree$, $alpha_2 = -14^degree$
- Computation: $cos phi.alt_2 = cos(12^degree + 14^degree) = cos(26^degree)$
- Result: $cos phi.alt_2 = 0.8988$

**Physical Interpretation:** Using the reference line, $V_1$ has angle $14.5^degree$; $I_(g 1)$ lags by $14^degree$. The angle between $V_1$ and $I_(g 1)$ is $14.5^degree + 14^degree = 28.5^degree$. This yields $cos phi.alt_1 = cos(28.5^degree) = 0.8788$.
...
$$
B_(11) = frac(sum_(K=1)^(4) A_(K 1)^(2) R_(K), |V_1|^(2) cos^(2) phi.alt_1)
$$

**Expanded Form:**

$$
B_(11) = frac(R_1 A_(11)^(2) + R_2 A_(21)^(2) + R_3 A_(31)^(2) + R_4 A_(41)^(2), |V_1|^(2) cos^(2) phi.alt_1)
$$
...
$$
B_(22) = frac(sum_(K=1)^(4) A_(K 2)^(2) R_(K), |V_2|^(2) cos^(2) phi.alt_2)
$$

**Expanded Form:**

$$
B_(22) = frac(R_1 A_(12)^(2) + R_2 A_(22)^(2) + R_3 A_(32)^(2) + R_4 A_(42)^(2), |V_2|^(2) cos^(2) phi.alt_2)
$$
...
2. **Same load current phase angle:** All load currents have the same phase angle $(delta_i - phi.alt_i) = beta_i$.
...
1. **Sign convention in power factor calculation:** The angle between voltage and current is $delta + alpha$ (not $delta - alpha$) when the current angle is negative (lagging). The instructor emphasizes: $cos phi.alt_1 = cos(14.5^degree + 14^degree)$, not $cos(14.5^degree - 14^degree)$.

2. **KCL application:** For $I_(g 2)$, both $I_2$ and $I_4$ must be summed at the bus — omitting either branch current gives an incorrect generator current.
...
- The total loss is the sum of $3|I_K|^2 R_K$ over all branches
...
![First-order Taylor expansion of transmission loss](/content-assets/studies/power-system-analysis/Week%209/Assets/img-4.jpg)
...
![Iterative generator-output formula with B coefficients](/content-assets/studies/power-system-analysis/Week%209/Assets/img-14.jpg)
...
![Boxed converged economic-dispatch solution](/content-assets/studies/power-system-analysis/Week%209/Assets/img-24.jpg)
...
![Load-flow derivative used for penalty-factor computation](/content-assets/studies/power-system-analysis/Week%209/Assets/img-48.jpg)
...
![Three-bus network and Y-bus data for penalty-factor example](/content-assets/studies/power-system-analysis/Week%209/Assets/img-49.jpg)
...
![Penalty-factor matrix equation and initial generator assumption](/content-assets/studies/power-system-analysis/Week%209/Assets/img-56.jpg)
...
![B-coefficient loss formula and coefficient definitions](/content-assets/studies/power-system-analysis/Week%209/Assets/img-68.jpg)
...
![Four-bus network and data for B-coefficient computation](/content-assets/studies/power-system-analysis/Week%209/Assets/img-70.jpg)
...
|--------|----------------|-------------------------------------|
| Coordination equation | $frac(d C_i, d P_("gi")) = lambda$ | $frac(d C_i, d P_("gi")) + lambda frac(d P_("Loss"), d P_("gi")) = lambda$ |
| Generation formula | $P_("gi") = frac(lambda - b_i, 2 d_i)$ | $P_("gi") = frac(lambda - b_i - 2 lambda sum_(j != i) B_("ij") P_("gj"), 2(d_i + lambda B_("ii")))$ |
| Power balance | $sum P_("gi") = P_L$ | $sum P_("gi") = P_L + P_("Loss")$ |
| Gradient correction | $Delta lambda = frac(Delta P_g, sum frac(1, 2 d_i))$ | $Delta lambda = frac(Delta P_g, sum frac(d P_("gi"), dif lambda))$ |
...
| Generator 1 current | $I_(g 1)$ | 4.123∠−14° | p.u. | Given |
| Generator 2 current | $I_(g 2)$ | 5.36∠−14° | p.u. | $I_(g 2) = I_2 + I_4$ (KCL) |
| Power factor 1 | $cos phi.alt_1$ | 0.8788 | — | $cos(delta_1 + alpha_1)$ |
| Power factor 2 | $cos phi.alt_2$ | 0.8988 | — | $cos(delta_2 + alpha_2)$ |
| Current angle difference | $cos(alpha_1 - alpha_2)$ | 1.0 | — | Both angles = −14° |
| Loss coefficient | $B_(11)$ | 0.02485 | p.u. | $frac(sum A_(K 1)^2 R_K, |V_1|^2 cos^2 phi.alt_1)$ |
| Loss coefficient | $B_(22)$ | 0.01755 | p.u. | $frac(sum A_(K 2)^2 R_K, |V_2|^2 cos^2 phi.alt_2)$ |
...
| Generator limit violation | $P_(g 2)$ = 373 MW exceeds 350 MW limit in Example 5 | Fix at limit, exclude from Δλ denominator, redistribute mismatch |
| Power factor sign | $cos phi.alt_1 = cos(14.5° + 14°)$, not $cos(14.5° − 14°)$ | Use $cos(delta + alpha)$ when current lags (α negative) |
| Loss verification | Check $P_(g 1) + P_(g 2) − P_("Loss") = P_L$ | Always verify power balance after convergence (Example 4: 369.96 ≈ 370 MW) |
...
| Diagonal-only approximation | $B_("ij") = 0$ for $i != j$ simplifies Equation 58 to Equation 68 | Verify whether cross-terms are negligible before using simplified form |
...
1. **Non-linearity trap:** When losses are present, $P_(g 2)$ vs λ is non-linear (λ appears in both numerator and denominator). Direct solution is not possible; iteration is required.
...
4. **Units:** B coefficients have units of $"MW"^(-1)$. Ensure consistency when using them in formulas.

5. **Power factor sign:** The angle between voltage and current is $delta + alpha$ (not $delta - alpha$) when the current angle is negative (lagging). The instructor emphasizes: $cos phi.alt_1 = cos(14.5^degree + 14^degree)$, not $cos(14.5^degree - 14^degree)$.

6. **KCL application:** For $I_(g 2)$, both $I_2$ and $I_4$ must be summed at the bus — omitting either branch current gives an incorrect generator current.
...
2. **Coordination check:** Verify that $L_i dot "IC"_i$ is the same for all generators at the optimal solution.
...
|----------|---------|-------------|
| Coordination (no losses) | $frac(d C_i, d P_("gi")) = lambda$ | All generators at same incremental cost |
| Coordination (with losses) | $frac(d C_i, d P_("gi")) + lambda frac(d P_("Loss"), d P_("gi")) = lambda$ | Modified by loss derivative |
| Penalty factor | $L_i = frac(1, 1 - frac(d P_("Loss"), d P_("gi")))$ | Modifies incremental cost |
| Generation from λ (no losses) | $P_("gi") = frac(lambda - b_i, 2 d_i)$ | For quadratic cost |
| Generation from λ (with losses) | $P_("gi")^((K)) = frac(lambda^((K)) - b_i - 2 lambda^((K)) sum_(j!= i) B_("ij") P_("gj")^((K)), 2(d_i + lambda^((K)) B_("ii")))$ | Iterative formula |
| Gradient correction (no losses) | $Delta lambda^((K)) = frac(Delta P_g^((K)), sum_(i=1)^(m) frac(1, 2 d_i))$ | Simple denominator |
| Gradient correction (with losses) | $Delta lambda^((k)) = frac(Delta P_g^((k)), sum_(i=1)^m (frac(d P_("gi"), dif lambda))^((k)))$ | Complex denominator |
| Loss derivative | $frac(d P_("Loss"), d P_("gi")) = 2 sum_(j=1)^(m) B_("ij") P_("gj")$ | From quadratic loss formula |
...
| General B coefficient | $B_("pq") = ( frac(cos(alpha_p - alpha_q), |V_p||V_q| cos phi.alt_p cos phi.alt_q) ) sum_(K=1)^("NBR") A_("Kp") A_("Kq") R_K$ | From distribution factors |
...
| Quantity | Value |
|----------|-------|
| $V_1$ | $1.198 angle 14.5^degree$ p.u. |
| $V_2$ | $1.153 angle 12^degree$ p.u. |
| $I_(g 1)$ | $4.123 angle -14^degree$ p.u. |
| $I_(g 2)$ | $5.36 angle -14^degree$ p.u. |
| $cos phi.alt_1$ | 0.8788 |
| $cos phi.alt_2$ | 0.8988 |
| $B_(11)$ | 0.02485 p.u. |
| $B_(22)$ | 0.01755 p.u. |
...
(a) $frac(d C_i, d P_("gi")) = lambda$

(b) $frac(d C_i, d P_("gi")) + lambda frac(d P_("Loss"), d P_("gi")) = lambda$

(c) $frac(d C_i, d P_("gi")) - lambda frac(d P_("Loss"), d P_("gi")) = lambda$

(d) $frac(d C_i, d P_("gi")) dot lambda frac(d P_("Loss"), d P_("gi")) = lambda$

> Answer and explanation
The correct answer is (b). The coordination equation with losses is derived from the Lagrangian: $"L" = C_T + lambda(P_L + P_("Loss") - sum P_("gi"))$. Taking the partial derivative with respect to $P_("gi")$ and setting to zero gives $frac(d C_i, d P_("gi")) + lambda frac(d P_("Loss"), d P_("gi")) = lambda$. Option (a) is the lossless case. Option (c) has the wrong sign on the loss term. Option (d) is dimensionally incorrect.
...
(a) $L_i = 1 - frac(d P_("Loss"), d P_("gi"))$

(b) $L_i = frac(1, 1 - frac(d P_("Loss"), d P_("gi")))$

(c) $L_i = 1 + frac(d P_("Loss"), d P_("gi"))$

(d) $L_i = frac(1, 1 + frac(d P_("Loss"), d P_("gi")))$

> Answer and explanation
The correct answer is (b). The penalty factor is defined as $L_i = frac(1, 1 - frac(d P_("Loss"), d P_("gi")))$. This comes from rearranging the coordination equation: $frac(d C_i, d P_("gi")) = lambda(1 - frac(d P_("Loss"), d P_("gi")))$, which gives $lambda = frac(d C_i/d P_("gi"), 1 - d P_("Loss")/d P_("gi")) = L_i dot "IC"_i$. Option (a) is the reciprocal of the penalty factor. Options (c) and (d) have incorrect signs.
...
(a) $Delta lambda^((K)) = frac(Delta P_g^((K)), sum_(i=1)^(m) frac(1, 2 d_i))$

(b) $Delta lambda^((K)) = Delta P_g^((K)) dot sum_(i=1)^(m) frac(1, 2 d_i)$

(c) $Delta lambda^((K)) = frac(sum_(i=1)^(m) frac(1, 2 d_i), Delta P_g^((K)))$

(d) $Delta lambda^((K)) = Delta P_g^((K)) dot sum_(i=1)^(m) 2 d_i$

> Answer and explanation
The correct answer is (a). The gradient method expands $f(lambda) = sum P_("gi")$ in a Taylor series: $f(lambda)^K + frac("df", dif lambda) Delta lambda^K = P_L$. Solving for $Delta lambda^K$: $Delta lambda^K = frac(P_L - f(lambda)^K, "df"/dif lambda) = frac(Delta P_g^K, sum 1/(2 d_i))$. The derivative $"df"/dif lambda = sum 1/(2 d_i)$ because $P_("gi") = (lambda - b_i)/(2 d_i)$.
...
> Answer and explanation
The correct answer is (b). Since $P_("Loss")$ is in MW and $P_("gi") dot P_("gj")$ is in MW², the B coefficients must have units of MW⁻¹ to make the product dimensionally consistent. This is explicitly stated in the lecture notes.
...
> Answer and explanation
The correct answers are (a), (b), and (d). The derivation requires: (a) all load currents have the same phase angle $(delta_i - phi.alt_i) = beta_i$, (b) the X/R ratio is the same for all branches (this ensures the current distribution factors are real), and (d) the network is linear so superposition applies. Option (c) is NOT an assumption — generators have different incremental costs, and the whole point of economic dispatch is to find the optimal λ where $L_i dot "IC"_i = lambda$ for all generators.
...
(d) The derivative sum $sum (d P_("gi")/dif lambda)$ is constant across iterations

> Answer and explanation
The correct answers are (a), (b), and (c). (a) is correct for the diagonal-only approximation where $B_("ij") = 0$ for $i != j$. (b) is the correct mismatch definition including losses. (c) is correct — Example 4 with losses converges in 4 iterations vs 2 without losses. (d) is incorrect — the derivative sum changes with λ and the generation values, as shown in the example where it changes from 5.551 to 5.364 to 5.358 across iterations.
...
> Answer and explanation
The correct answers are (a), (b), and (c). (a) The matrix in Equation 76 is the J₁ transpose from load flow studies. (b) Since $P_("Loss")$ does not include $P_(g 1)$ (slack bus generation), $frac(d P_("Loss"), d P_(g 1)) = 0$ and $L_1 = 1$. (c) The right-hand side is $-mat(d P_1/d delta_2; dots.v; d P_1/d delta_n)$. (d) is incorrect — this method does NOT require B coefficients; it computes penalty factors directly from load flow results.
...
> Answer and explanation
The penalty factor $L_i = frac(1, 1 - frac(d P_("Loss"), d P_("gi")))$ represents how much additional generation is needed at plant $i$ to supply one additional MW of load, accounting for the incremental transmission loss caused by that generation. If $frac(d P_("Loss"), d P_("gi")) = 0.1$, then $L_i = 1/0.9 = 1.111$, meaning 1.111 MW of additional generation is needed at plant $i$ to deliver 1 MW more to the load. Generators that cause more losses have higher penalty factors, making them less economically attractive.
...
> Answer and explanation
The two key assumptions are: (1) all load currents have the same phase angle $(delta_i - phi.alt_i) = beta_i$, and (2) the X/R ratio is the same for all network branches. These assumptions ensure that the current distribution factors $A_(K 1)$ and $A_(K 2)$ are real quantities (not complex). This is needed because the derivation expresses branch currents as $I_K = A_(K 1) I_(g 1) + A_(K 2) I_(g 2)$, and if the distribution factors were complex, the magnitude squared $|I_K|^2$ would involve additional cross-terms that complicate the loss formula.

---

### Question 13 (Short Answer)

In the 3-bus example, why is $L_2 "IC"_2 > L_1 "IC"_1$ at the initial guess, and what adjustment is needed?

> Answer and explanation
At the initial guess $P_(g 2)^0 = 1.0$ p.u., the computed penalty factors give $L_1 "IC"_1 = 4.6144$ and $L_2 "IC"_2 = 4.6933$. Since $L_2 "IC"_2 > L_1 "IC"_1$, generator 2's effective incremental cost is higher than generator 1's. To restore the optimality condition $L_1 "IC"_1 = L_2 "IC"_2$, we need to decrease $P_(g 2)$ (which decreases $"IC"_2$) and increase $P_(g 1)$ (which increases $"IC"_1$). The final converged solution has $P_(g 1) = 1.05$ p.u. and $P_(g 2) = 0.963$ p.u. with $L_1 "IC"_1 = L_2 "IC"_2 = 4.63$.

---

### Question 14 (Numerical)

For the system in Example 5 (no losses), with cost functions $C_1 = 500 + 41 P_(g 1) + 0.15 P_(g 1)^2$, $C_2 = 400 + 44 P_(g 2) + 0.1 P_(g 2)^2$, $C_3 = 300 + 40 P_(g 3) + 0.18 P_(g 3)^2$ and total load 850 MW, compute the generations at λ = 118.6007 Rs/MWhr.

> Answer and explanation
Using the formula $P_("gi") = frac(lambda - b_i, 2 d_i)$:

$P_(g 1) = frac(118.6007 - 41, 2 times 0.15) = frac(77.6007, 0.3) = 258.669$ MW

$P_(g 2) = frac(118.6007 - 44, 2 times 0.10) = frac(74.6007, 0.2) = 373.0035$ MW

$P_(g 3) = frac(118.6007 - 40, 2 times 0.18) = frac(78.6007, 0.36) = 218.335$ MW
...
For the system in Example 4 with losses, given $B_(11) = 0.0005$, $B_(22) = 0.0008$, $b_1 = b_2 = 41$, $d_1 = d_2 = 0.175$, and $P_L = 370$ MW, compute $P_(g 1)$ and $P_(g 2)$ at λ = 109.196 Rs/MWhr using the diagonal-only formula.

> Answer and explanation
Using the formula $P_("gi") = frac(lambda - b_i, 2(d_i + lambda B_("ii")))$:

$P_(g 1) = frac(109.196 - 41, 2(0.175 + 109.196 times 0.00005)) = frac(68.196, 2(0.175 + 0.00546)) = frac(68.196, 2 times 0.18046) = frac(68.196, 0.36092) = 188.95$ MW

$P_(g 2) = frac(109.196 - 41, 2(0.175 + 109.196 times 0.00008)) = frac(68.196, 2(0.175 + 0.008736)) = frac(68.196, 2 times 0.183736) = frac(68.196, 0.367472) = 185.58$ MW
...
In the B-coefficient example, compute $cos phi.alt_1$ given $delta_1 = 14.5^degree$ and $alpha_1 = -14^degree$.
...
$phi.alt_1 = delta_1 + alpha_1$ (taking absolute values since both are measured from the reference)

$phi.alt_1 = 14.5^degree + 14^degree = 28.5^degree$

$cos phi.alt_1 = cos(28.5^degree) = 0.8788$

The key point is that we ADD the magnitudes of the angles because the current lags the voltage. Using $cos(14.5^degree - 14^degree) = cos(0.5^degree) approx 1$ would be incorrect — it would give a power factor of essentially 1, which is wrong for this lagging load.
...
1. **Step size too large:** The gradient correction $Delta lambda = Delta P_g / sum(d P_("gi")/dif lambda)$ may be overcorrecting. This can happen if the derivative sum is computed incorrectly (e.g., using the wrong formula or including fixed generators).
...
In the 3-bus penalty factor example, after computing $L_2 = 1.0203$ at the initial guess, the coordination check shows $L_2 "IC"_2 > L_1 "IC"_1$. The engineer decides to increase $P_(g 2)$ to reduce the difference. Is this correct?

> Answer and explanation
No, this is incorrect. When $L_2 "IC"_2 > L_1 "IC"_1$, it means generator 2's effective incremental cost is too high. To restore the optimality condition $L_1 "IC"_1 = L_2 "IC"_2$, we need to DECREASE $P_(g 2)$ (which decreases $"IC"_2$ since $"IC"_2 = 4 + 0.6 P_(g 2)$) and INCREASE $P_(g 1)$ (which increases $"IC"_1$). The final converged solution confirms this: $P_(g 2)$ decreases from 1.0 to 0.963 p.u., while $P_(g 1)$ increases from 1.024 to 1.05 p.u. Increasing $P_(g 2)$ would make the inequality worse, moving further from the optimum.
...
1. **Example 4 continuation (Lecture 41):** Optimal scheduling with transmission loss $P_("Loss") = 0.001(P_(g 2) - 70)^2$. Fully worked: penalty factors, coordination equations, iterative solution, verification.
