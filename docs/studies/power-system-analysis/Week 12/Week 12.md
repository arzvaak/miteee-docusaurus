...
$$
"KE" = frac(1, 2) J omega_(s-"mech")^2 times 10^(-6) " MJ"
$$
...
$$
theta_e = frac(P, 2) theta_m
$$

$$
omega_(s-"elect") = frac(P, 2) omega_(s-"mech")
$$
...
$$
"KE" = frac(1, 2) [ J (frac(2, P))^2 omega_(s-"elect") times 10^(-6) ] omega_(s-"elect") = frac(1, 2) M omega_(s-"elect")
$$

Where we define the **angular momentum M**:

$$
M = J (frac(2, P))^2 omega_(s-"elect") times 10^(-6) " MJ-sec/elect-radian"
$$
...
$$
G times H = "KE" = frac(1, 2) M omega_(s-"elect") " MJ"
$$
...
$$
M = frac(2 "GH", omega_(s-"elect")) = frac(2 "GH", 2 pi f) = frac("GH", pi f) " MJ-sec/elect-radian"
$$

In electrical degrees:

$$
M = frac("GH", 180 f) " MJ-sec/elect-degree"
$$

The per-unit inertia constant is obtained by dividing by G:

$$
M_("pu") = frac(H, pi f) " sec"^2/"elect-radian"
$$

$$
M_("pu") = frac(H, 180 f) " sec"^2/"elect-degree"
$$
...
$$
T_e dot omega_s = T_i dot omega_s
$$
...
Where $P_i = T_i dot omega_s$ is the input power and $P_e = T_e dot omega_s$ is the electrical power.

**Departure from steady state**: If a change in load or a fault occurs, $P_i != P_e$, and the left side of Equation 13 is not zero. An acceleration torque comes into play. The equation of motion becomes:

$$
M frac(d^2 theta_e, dif t^2) + D frac(d theta_e, dif t) = P_a
$$

Where:
- $P_a = P_i - P_e$ = accelerating or decelerating power
- $D$ = damping coefficient
- $theta_e$ = electrical angular position of the rotor
- $M$ as defined in Equation 9 or 10

The **power angle** (or torque angle) δ is defined as:

$$
delta = theta_e - omega_s t
$$

This represents the angular displacement of the rotor from a synchronously rotating reference frame. Taking derivatives:

$$
frac(d delta, dif t) = frac(d theta_e, dif t) - omega_s
$$

$$
frac(d^2 delta, dif t^2) = frac(d^2 theta_e, dif t^2)
$$

Substituting into Equation 14 with $D = 0$ (neglecting damping for the basic swing equation):

$$
M frac(d^2 delta, dif t^2) = P_i - P_e
$$

In MW (using Equation 7):

$$
frac("GH", pi f) frac(d^2 delta, dif t^2) = (P_i - P_e) " MW"
$$

In per unit (using Equation 9):

$$
M_("pu") frac(d^2 delta, dif t^2) = (P_i - P_e) " pu"
$$

$$
frac(H, pi f) frac(d^2 delta, dif t^2) = (P_i - P_e) " pu"
$$
...
$$
frac(H_("machine"), pi f) frac(d^2 delta, dif t^2) = P_i - P_e
$$

Multiplying by $G_("machine")$:

$$
G_("machine") times H_("machine") times frac(1, pi f) frac(d^2 delta, dif t^2) = (P_i - P_e) times G_("machine") " MW"
$$

Dividing by $G_("system")$ (the system base):

$$
frac(G_("machine"), G_("system")) times frac(H_("machine"), pi f) frac(d^2 delta, dif t^2) = (P_i - P_e) times frac(G_("machine"), G_("system"))
$$

This gives the swing equation on the system base:

$$
frac(H_("system"), pi f) frac(d^2 delta, dif t^2) = P_i - P_e " p.u. on system base"
$$

Where the system inertia constant is:

$$
H_("system") = G_("machine") times frac(H_("machine"), G_("system"))
$$
...
4. **Damping is neglected**: The damping term $D frac(d delta, dif t)$ is small compared to the accelerating power during the first swing. Neglecting damping gives a conservative estimate of stability (the actual system is more stable than predicted).
...
2. **Radians vs. degrees**: When using Equation 8 ($M = "GH"/(180 f)$), δ must be in degrees. When using Equation 7 ($M = "GH"/(pi f)$), δ must be in radians. Mixing these up is a common error.
...
4. **Base conversion**: When computing $H_("system")$, remember to multiply by the ratio $G_("machine")/G_("system")$. A machine with H = 5 MJ/MVA on its own base of 100 MVA has $H_("system") = 5 times 100/100 = 5$ on a 100 MVA system base, but $H_("system") = 5 times 100/1000 = 0.5$ on a 1000 MVA system base.

5. **Pole pairs vs. poles**: The relationship $theta_e = (P/2) theta_m$ uses the number of poles P, not pole pairs. A 4-pole machine has P = 4, so $theta_e = 2 theta_m$.
...
- The swing equation $frac(H, pi f) frac(d^2 delta, dif t^2) = P_i - P_e$ describes rotor dynamics
...
$$
frac(H_1, pi f) frac(d^2 delta_1, dif t^2) = P_(i 1) - P_(e 1)
$$

$$
frac(H_2, pi f) frac(d^2 delta_2, dif t^2) = P_(i 2) - P_(e 2)
$$
...
$$
frac(H_1 + H_2, pi f) frac(d^2 delta, dif t^2) = (P_(i 1) + P_(i 2)) - (P_(e 1) + P_(e 2))
$$

This gives the combined swing equation:

$$
frac(H_("eq"), pi f) frac(d^2 delta, dif t^2) = P_i - P_e
$$
...
$$
H_("eq") = frac(G_(1,m a chi n e), G_("system")) times H_(1,m a chi n e) + frac(G_(2,m a chi n e), G_("system")) times H_(2,m a chi n e)
$$

For n machines:

$$
H_("eq") = frac(G_1 H_1 + G_2 H_2 + dots.h + G_n H_n, G_("system"))
$$

**Physical interpretation**: The equivalent inertia constant is the total kinetic energy of all machines divided by the system base. This makes sense because $G times H = "KE"$, so $H_("eq") = sum("KE"_i)/G_("system")$.
...
$$
S_S = P_S + j Q_S = V_S I^(star) = V_S (frac(V_S - V_R, j X))^(star) = frac(V_S(V_S^(star) - V_R^(star)), -j X)
$$

Expanding this expression:

$$
S_S = frac(|V_S|^2 - |V_S||V_R|e^(-j delta), -j X) = frac(|V_S|^2 - |V_S||V_R|(cos delta - j sin delta), -j X)
$$

Separating real and imaginary parts:

$$
P_S = frac(|V_S||V_R|, X) sin delta
$$

$$
Q_S = frac(|V_S|^2 - |V_S||V_R| cos delta, X)
$$
...
$$
Q_R = frac(|V_S||V_R| cos delta - |V_R|^2, X)
$$

For a lossless line, the real power is the same at both ends:

$$
P_S = P_R = frac(|V_S||V_R|, X) sin delta
$$
...
$$
P_e = P_t = frac(|E_g||V_t|, x_d) sin delta
$$
...
2. **Reduce the rest of the system to a Thevenin equivalent**: Combine all other generators, loads, and transmission lines into a single voltage source $E_B$ behind a single equivalent impedance $Z_("eq") = r_e + "jx"_e$.
...
$$
"KE" = G times H = 100 times 10 = 1000 " MJ"
$$
...
$$
P_a = P_i - P_e = 60 - 50 = 10 " MW"
$$

Using M in electrical degrees (Equation 8):

$$
M = frac("GH", 180 f) = frac(100 times 10, 180 times 60) = frac(1000, 10800) = frac(5, 54) " MJ-sec/elect-degree"
$$

From the swing equation $M frac(d^2 delta, dif t^2) = P_a$:

$$
frac(5, 54) times frac(d^2 delta, dif t^2) = 10
$$

$$
frac(d^2 delta, dif t^2) = frac(10 times 54, 5) = 108 " elect-degree/sec"^2
$$
...
$$
Delta t = frac(12, 60) = 0.2 " sec"
$$

The change in torque angle (assuming constant acceleration):

$$
Delta delta = frac(1, 2) alpha (Delta t)^2 = frac(1, 2) times 108 times (0.2)^2 = 2.16 " elect-degree"
$$
...
$$
N_s = frac(120 f, P) = frac(120 times 60, 4) = 1800 " rpm"
$$

The rotor speed at the end of 12 cycles:

$$
N = N_s + alpha times Delta t = 1800 + 18 times 0.2 = 1800 + 3.6 = 1803.6 " rpm"
$$
...
$$
"KE" = G_1 H_1 + G_2 H_2 = 400 times 4.6 + 1200 times 3 = 1840 + 3600 = 5440 " MJ"
$$
...
$$
H_("eq") = frac(400, 100) times 4.6 + frac(1200, 100) times 3 = 4 times 4.6 + 12 times 3 = 18.4 + 36 = 54.4 " MJ/MVA"
$$
...
$$
N_s = frac(120 f, P) = frac(120 times 50, 2) = 3000 " rpm" = 50 " rps"
$$

For a 2-pole machine, the mechanical speed equals the electrical speed:

$$
omega_m = omega_e = 2 pi times 50 = 314.16 " rad/sec"
$$

The kinetic energy is:

$$
"KE" = frac(1, 2) J omega_m^2 times 10^(-6) = frac(1, 2) times 40 times 10^3 times (314.16)^2 times 10^(-6)
$$

$$
"KE" = frac(1, 2) times 40 times 10^3 times 98696 times 10^(-6) = 1973.92 " MJ"
$$

More precisely: $"KE" = frac(1, 2) times 40 times 10^3 times (frac(2 pi times 3000, 60))^2 times 10^(-6) = 1974.43 " MJ"$
...
$$
M = J times omega_m times 10^(-6) = 40 times 10^3 times 314.16 times 10^(-6) = 12.566 " MJ-sec/mech-rad"
$$

**Part (c): Inertia Constant**

$$
H = frac("KE", "MVA") = frac(1974.43, 100) = 19.7443 " MJ/MVA"
$$
...
$$
|V_S| = |V_R| = frac(230, sqrt(3)) = 132.79 " kV"
$$

From Equation 41, the maximum power occurs when sin δ = 1 (δ = 90°):

$$
P_(R(max)) = P_(S(max)) = frac(|V_S||V_R|, X) = frac(|V_R|^2, X) = frac((132.79 times 10^3)^2, 14)
$$

$$
P_("max") = frac(17.63 times 10^9, 14) = 1259.5 " MW/phase"
$$

The three-phase total is:

$$
P_(max,3 phi.alt) = 3 times 1259.5 = 3778.5 " MW"
$$
...
$$
cos phi = 0.80, space phi.alt = 36.87° " (lagging)"
$$

$$
I = frac(P, V cos phi) angle -phi.alt = frac(1.0, 1 times 0.80) angle -36.87° = 1.25 angle -36.87° " p.u."
$$

**Step 3: Calculate Internal Voltage E_g**

$$
|E_g| angle delta = |V_2| angle 0° + j X_("eq") times I
$$

$$
|E_g| angle delta = 1 angle 0° + j 0.5 times 1.25 angle -36.87°
$$
...
$$
|E_g| angle delta = 1 + 0.625(cos 53.13° + j sin 53.13°) = 1 + 0.625(0.6 + j 0.8)
$$

$$
|E_g| angle delta = 1 + 0.375 + j 0.5 = 1.375 + j 0.5
$$

$$
|E_g| = sqrt(1.375^2 + 0.5^2) = sqrt(1.891 + 0.25) = sqrt(2.141) = 1.463 " p.u."
$$

$$
delta = tan^(-1)(frac(0.5, 1.375)) = tan^(-1)(0.3636) = 20°
$$
...
$$
P_e = frac(E_g times V_2, X_("eq")) sin delta = frac(1.463 times 1, 0.5) sin delta = 2.926 sin delta
$$
...
$$
frac(H, 180 f) frac(d^2 delta, dif t^2) = P_i - P_e
$$

With $P_i = 1$ p.u.:

$$
frac(H, 180 f) frac(d^2 delta, dif t^2) = 1 - 2.926 sin delta
$$
...
5. **Power factor angle**: For lagging power factor, the current lags the voltage by angle φ. The current phasor is $I = |I|angle-phi.alt$ when the voltage is the reference.
...
- Terminal voltage: $V_t = 1.1 angle theta$
...
$$
|E_g| angle delta = "jx"_d times I + V_t
$$
...
$$
I = frac(V_t - V, "jx") = frac(1.1 angle theta - 1 angle 0°, j 1)
$$

**Step 3: Substitute and Simplify**

$$
|E_g| angle delta = 1.1 angle theta + j 0.4 times frac((1.1 angle theta - 1), j 1)
$$

$$
|E_g| angle delta = 1.1 angle theta + 0.4(1.1 angle theta - 1)
$$

$$
|E_g| angle delta = 1.1(cos theta + j sin theta) + 0.4(1.1 cos theta - 1 + j 1.1 sin theta)
$$

$$
|E_g| angle delta = 1.1 cos theta + j 1.1 sin theta + 0.44 cos theta - 0.4 + j 0.44 sin theta
$$

$$
|E_g| angle delta = 1.54 cos theta - 0.4 + j 1.54 sin theta
$$
...
$$
1.54 cos theta - 0.4 = 0
$$

$$
cos theta = frac(0.4, 1.54) = 0.2597
$$
...
$$
|E_g| = 1.54 sin theta = 1.54 sin 74.9° = 1.54 times 0.965 = 1.486 " p.u."
$$
...
$$
P_("max") = frac(|E_g||V|, x_d + x) = frac(1.486 times 1.0, 0.4 + 1) = frac(1.486, 1.4) = 1.061 " p.u."
$$
...
$$
frac(d^2 delta, dif t^2) = frac(P_a, M)
$$

Where $P_a = P_i - P_e$ is the accelerating power.

To derive the equal area criterion, we multiply both sides by $2 frac(d delta, dif t)$:

$$
2 frac(d delta, dif t) times frac(d^2 delta, dif t^2) = 2 frac(P_a, M) frac(d delta, dif t)
$$

The left side can be recognized as the derivative of $(frac(d delta, dif t))^2$:

$$
frac(d, dif t)(frac(d delta, dif t))^2 = 2 frac(P_a, M) frac(d delta, dif t)
$$

Integrating both sides with respect to time:

$$
(frac(d delta, dif t))^2 = frac(2, M) integral_(delta_0)^(delta) P_a d delta
$$

The relative speed of the machine with respect to the synchronously revolving reference frame is:

$$
frac(d delta, dif t) = sqrt(frac(2, M) integral_(delta_0)^(delta) P_a d delta)
$$

**Stability condition**: For the system to be stable, the speed must become zero at some time after the disturbance. This means:

$$
integral_(delta_0)^(delta) P_a d delta = 0
$$
...
Consider a machine operating at equilibrium point δ₀ with mechanical power input $P_(i 0)$. The electrical power is $P_(e 0) = P_("max") sin delta_0 = P_(i 0)$.

Now, suppose the input power is suddenly increased to $P_i$ (where $P_i > P_(e 0)$). The analysis proceeds as follows:

1. **Initial acceleration**: Since $P_i > P_(e 0)$, the accelerating power is positive, and δ increases. The rotor accelerates from point 'a' (δ₀) toward point 'b' (δ₁).

2. **Area A₁ (accelerating energy)**: The excess energy stored in the rotor during the initial acceleration is:

$$
A_1 = integral_(delta_0)^(delta_1) (P_i - P_("max") sin delta) d delta
$$

3. **At point b (δ = δ₁)**: The electrical power matches the new input power ($P_(e 1) = P_("max") sin delta_1 = P_i$). However, the rotor is running above synchronous speed, so δ continues to increase.
...
$$
A_2 = integral_(delta_1)^(delta_2) (P_e - P_i) d delta
$$
...
$$
integral_(delta_0)^(delta_1) (P_i - P_("max") sin delta) d delta = integral_(delta_1)^(delta_2) (P_("max") sin delta - P_i) d delta
$$

After integration:

$$
P_i(delta_1 - delta_0) + P_("max")(cos delta_1 - cos delta_0) = P_i(delta_1 - delta_2) + P_("max")(cos delta_1 - cos delta_2)
$$

Using $P_i = P_("max") sin delta_1$:

$$
P_("max")(delta_1 - delta_0) sin delta_1 + P_("max")(cos delta_1 - cos delta_0) = P_("max")(delta_1 - delta_2) sin delta_1 + P_("max")(cos delta_1 - cos delta_2)
$$
...
$$
P_(e 0) = P_("max") sin delta_0 = 500 sin 8° = 500 times 0.1392 = 69.6 " MW"
$$
...
$$
(pi - 2 times 0.8727) times sin(0.8727) - cos(0.8727) - 0.9903
$$

$$
= (3.1416 - 1.7453) times 0.7660 - 0.6428 - 0.9903
$$

$$
= 1.3963 times 0.7660 - 0.6428 - 0.9903
$$
...
$$
(pi - 2 times 0.9599) times sin(0.9599) - cos(0.9599) - 0.9903
$$

$$
= (3.1416 - 1.9198) times 0.8192 - 0.5736 - 0.9903
$$

$$
= 1.2218 times 0.8192 - 0.5736 - 0.9903
$$
...
$$
(pi - 2 times 1.0472) times sin(1.0472) - cos(1.0472) - 0.9903
$$

$$
= (3.1416 - 2.0944) times 0.8660 - 0.5 - 0.9903
$$

$$
= 1.0472 times 0.8660 - 0.5 - 0.9903
$$
...
$$
(pi - 2 times 0.7854) times sin(0.7854) - cos(0.7854) - 0.9903
$$

$$
= (3.1416 - 1.5708) times 0.7071 - 0.7071 - 0.9903
$$

$$
= 1.5708 times 0.7071 - 0.7071 - 0.9903
$$
...
$$
(pi - 2 times 0.6981) times sin(0.6981) - cos(0.6981) - 0.9903
$$

$$
= (3.1416 - 1.3963) times 0.6428 - 0.7660 - 0.9903
$$

$$
= 1.7453 times 0.6428 - 0.7660 - 0.9903
$$
...
$$
(pi - 2 times 0.8378) times sin(0.8378) - cos(0.8378) - 0.9903
$$

$$
= (3.1416 - 1.6755) times 0.7431 - 0.6691 - 0.9903
$$

$$
= 1.4661 times 0.7431 - 0.6691 - 0.9903
$$
...
$$
(pi - 2 times 0.9076) times sin(0.9076) - cos(0.9076) - 0.9903
$$

$$
= (3.1416 - 1.8151) times 0.7880 - 0.6157 - 0.9903
$$

$$
= 1.3265 times 0.7880 - 0.6157 - 0.9903
$$
...
$$
(pi - 2 times 0.8814) times sin(0.8814) - cos(0.8814) - 0.9903
$$

$$
= (3.1416 - 1.7628) times 0.7716 - 0.6361 - 0.9903
$$

$$
= 1.3788 times 0.7716 - 0.6361 - 0.9903
$$
...
$$
P_("ef") = P_("max") sin delta_1 = 500 sin 50° = 500 times 0.7660 = 383.02 " MW"
$$

**Step 6: Maximum Sudden Increase**

$$
Delta P = P_("ef") - P_(e 0) = 383.02 - 69.6 = 313.42 " MW/phase"
$$

Three-phase: $3 times 313.42 = 940.3$ MW
...
6. **Check stability**: The system is stable if a solution exists with $A_2 >= A_1$.
...
$$
A_2 = integral_(delta_c)^(delta_1) (P_("max") sin delta - P_i) d delta
$$

Setting $A_1 = A_2$:

$$
P_i(delta_c - delta_0) = integral_(delta_c)^(delta_1) (P_("max") sin delta - P_i) d delta
$$

$$
P_i(delta_c - delta_0) = P_("max")(cos delta_c - cos delta_1) - P_i(delta_1 - delta_c)
$$

Rearranging:

$$
P_("max")(cos delta_c - cos delta_1) = P_i(delta_1 - delta_0)
$$
...
$$
frac(d^2 delta, dif t^2) = frac(pi f, H) P_i
$$

This is a constant acceleration equation. Integrating twice with initial conditions $t = 0$, $delta = delta_0$, and $frac(d delta, dif t) = 0$:

First integration:

$$
frac(d delta, dif t) = frac(pi f P_i, H) t
$$

Second integration:

$$
delta = frac(pi f P_i, 2 H) t^2 + delta_0
$$

At the clearing time $t_c$, the rotor angle is the clearing angle δ_c:

$$
delta_c = frac(pi f P_i, 2 H) t_c^2 + delta_0
$$

Solving for the clearing time:

$$
t_c = sqrt(frac(2 H(delta_c - delta_0), pi f P_i))
$$

The **critical clearing time** is obtained by substituting the critical clearing angle:

$$
t_("cr") = sqrt(frac(2 H(delta_("cr") - delta_0), pi f P_i))
$$
...
$$
A_1 = integral_(delta_0)^(delta_("cr")) (P_i - K_1 P_("max") sin delta) d delta
$$

The decelerating area is:

$$
A_2 = integral_(delta_("cr"))^(delta_("max")) (K_2 P_("max") sin delta - P_i) d delta
$$
...
$$
cos delta_("cr") = frac(1, (K_2 - K_1))[(delta_("max") - delta_0) sin delta_0 + K_2 cos delta_("max") - K_1 cos delta_0]
$$
...
$$
P_i = frac(80, 100) = 0.8 " p.u."
$$

$$
P_("max") = frac(400, 100) = 4 " p.u."
$$
...
$$
delta_0 = sin^(-1)(0.2) = 11.54° = 0.2014 " rad"
$$
...
$$
cos delta_c = cos(1.4835) + (1.4835 - 0.2014) times 0.2
$$

$$
cos delta_c = 0.0872 + 1.2821 times 0.2
$$
...
$$
delta_c = cos^(-1)(0.3436) = 69.9° = 1.220 " rad"
$$
...
$$
t_c = sqrt(frac(2 times 7 times (1.220 - 0.2014), pi times 50 times 0.8))
$$

$$
t_c = sqrt(frac(14 times 1.0186, 125.66))
$$

$$
t_c = sqrt(frac(14.26, 125.66)) = sqrt(0.1135) = 0.337 " s"
$$
...
$$
cos delta_("cr") = (pi - 2 times 0.2014) times 0.2 - cos(0.2014)
$$

$$
cos delta_("cr") = (3.1416 - 0.4028) times 0.2 - 0.9798
$$

$$
cos delta_("cr") = 2.7388 times 0.2 - 0.9798 = 0.5478 - 0.9798 = -0.4320
$$

$$
delta_("cr") = cos^(-1)(-0.4320) = 115.6° = 2.017 " rad"
$$
...
- During fault: $V_t -> 0.25 V_t$ → $K_1 = 0.25$
- After fault cleared: $P = 0.70 P_("max")$ → $K_2 = 0.70$

**Physical Reasoning for K₁**:

$$
P_("max") = frac(|E_g| dot |V_t|, x_d)
$$

When $V_t$ becomes $0.25 V_t$, the maximum power becomes $0.25 P_("max")$, hence $K_1 = 0.25$.
...
$$
P_i = 0.45 P_("max") = P_("max") sin delta_0
$$

$$
delta_0 = sin^(-1)(0.45) = 26.74° = 0.466 " rad"
$$
...
$$
sin delta_m^1 = frac(P_i, K_2 P_("max")) = frac(0.45 P_("max"), 0.7 P_("max")) = frac(0.45, 0.7) = 0.6429
$$

$$
delta_m^1 = sin^(-1)(0.6429) = 40° = 0.698 " rad"
$$

**Step 3: Compute δ_max**

$$
delta_("max") = pi - delta_m^1 = pi - 0.698 = 2.443 " rad"
$$
...
$$
cos delta_("cr") = frac(1, (0.7 - 0.25))[(2.443 - 0.466) times 0.45 + 0.7 cos(2.443) - 0.25 cos(0.466)]
$$

$$
cos delta_("cr") = frac(1, 0.45)[1.977 times 0.45 + 0.7 times (-0.766) - 0.25 times 0.893]
$$

$$
cos delta_("cr") = frac(1, 0.45)[0.8897 - 0.5362 - 0.2233]
$$

$$
cos delta_("cr") = frac(1, 0.45) times 0.1302 = 0.2893
$$

$$
delta_("cr") = cos^(-1)(0.2893) = 73.2° = 1.277 " rad"
$$
...
**System Data**:
- Generator: $j 0.20$ (direct axis transient reactance)
- Transformers: $j 0.05$ each side
- Transmission lines (double circuit): $j 0.38$ each
- Fault at midpoint of one line: splits into $j 0.19 + j 0.19$
- Generator voltage: $|E_g| = 1.20$ p.u.
- Infinite bus voltage: $|V| = 1.0 angle 0°$ p.u.
- $P_i = 1.0$ p.u. MW
...
$$
X_A = 0.2 + frac((0.05 + 0.38 + 0.05), 2) + 0.2 = 0.2 + 0.24 + 0.2 = 0.64 " p.u."
$$

The pre-fault power-angle curve is:

$$
P_(e,A) = frac(1.2 times 1.0, 0.64) sin delta = 1.875 sin delta
$$
...
$$
delta_0 = sin^(-1)(frac(1, 1.875)) = sin^(-1)(0.5333) = 32.23° = 0.562 " rad"
$$
...
The Δ branches are:
- Branch 1 (gen side to F): $j 0.05 + j 0.19 = j 0.24$ (transformer 1 + half of faulted line)
- Branch 2 (F to infinite bus side): $j 0.19 + j 0.05 = j 0.24$ (half of faulted line + transformer 2)
- Branch 3 (gen side to infinite bus side): $j 0.38 + j 0.05 + j 0.05 = j 0.48$ (healthy line + both transformers)

**Δ to Y conversion**:

$$
Z_a = frac(Z_(12) Z_(31), Z_(12) + Z_(23) + Z_(31)) = frac(j 0.24 times j 0.48, j 0.24 + j 0.24 + j 0.48) = frac(-0.1152, j 0.96) = j 0.12
$$

$$
Z_b = frac(Z_(12) Z_(23), Z_(12) + Z_(23) + Z_(31)) = frac(j 0.24 times j 0.24, j 0.96) = frac(-0.0576, j 0.96) = j 0.06
$$

$$
Z_c = frac(Z_(23) Z_(31), Z_(12) + Z_(23) + Z_(31)) = frac(j 0.24 times j 0.48, j 0.96) = j 0.12
$$

So:
- $Y_1 = j 0.12$ (from generator side to center)
- $Y_2 = j 0.12$ (from infinite bus side to center)
- $Y_3 = j 0.06$ (from fault point to center)
...
$$
X_B = 0.32 + 0.32 + frac(0.32 times 0.32, 0.06) = 0.64 + frac(0.1024, 0.06) = 0.64 + 1.7067 = 2.3467 " p.u."
$$

The during-fault power-angle curve is:

$$
P_(e,B) = frac(1.2 times 1.0, 2.3466) sin delta = 0.5113 sin delta
$$
...
$$
X_C = 0.2 + 0.05 + 0.38 + 0.05 + 0.2 = 0.88 " p.u."
$$

$$
P_(e,C) = frac(1.2 times 1.0, 0.88) sin delta = 1.363 sin delta
$$

**Step 5: Compute Constants K₁ and K₂**

$$
K_1 = frac(P_(max,"during fault"), P_(max,"pre-fault")) = frac(0.5113, 1.875) = 0.2727
$$

$$
K_2 = frac(P_(max,"post-fault"), P_(max,"pre-fault")) = frac(1.363, 1.875) = 0.727
$$
...
$$
delta_m^1 = sin^(-1)(frac(1, 1.363)) = sin^(-1)(0.7337) = 47.19° = 0.8237 " rad"
$$

**Step 7: Compute δ_max**

$$
delta_("max") = pi - delta_m^1 = pi - 0.8237 = 2.317 " rad" = 132.75°
$$
...
$$
cos delta_("cr") = frac(1, (0.727 - 0.2727))[(2.317 - 0.562) times 0.5333 + 0.727 cos(2.317) - 0.2727 cos(0.562)]
$$

$$
cos delta_("cr") = frac(1, 0.4543)[1.755 times 0.5333 + 0.727 times (-0.6756) - 0.2727 times 0.8462]
$$

$$
cos delta_("cr") = frac(1, 0.4543)[0.9360 - 0.4912 - 0.2308]
$$

$$
cos delta_("cr") = frac(1, 0.4543) times 0.2140 = 0.4711
$$
...
**Given Data**:
- Initial: $P_(i 0) = 0.35 P_("max")$
- Load doubled: $P_i = 2 times 0.35 P_("max") = 0.7 P_("max")$
...
$$
delta_0 = sin^(-1)(frac(P_(i 0), P_("max"))) = sin^(-1)(0.35) = 20.49° = 0.357 " rad"
$$

**Step 2: Compute δ₁ (angle after load doubling)**

$$
delta_1 = sin^(-1)(frac(P_i, P_("max"))) = sin^(-1)(0.7) = 44.43° = 0.775 " rad"
$$
...
$$
(delta_2 - 0.357) times 0.7 + cos delta_2 - 0.9369 = 0
$$
...
$$
0.7 times 1.25 + cos(1.25) = 0.875 + 0.3153 = 1.1903
$$

Too high. Try $delta_2 = 1.24$ rad (71.0°):

$$
0.7 times 1.24 + cos(1.24) = 0.868 + 0.3240 = 1.1920
$$

Too high. Try $delta_2 = 1.26$ rad (72.2°):

$$
0.7 times 1.26 + cos(1.26) = 0.882 + 0.3065 = 1.1885
$$

Too low. Try $delta_2 = 1.255$ rad (71.9°):

$$
0.7 times 1.255 + cos(1.255) = 0.8785 + 0.3109 = 1.1894
$$

Too low. Try $delta_2 = 1.252$ rad (71.7°):

$$
0.7 times 1.252 + cos(1.252) = 0.8764 + 0.3132 = 1.1896
$$
...
$$
0.7 times 1.251 + cos(1.251) = 0.8757 + 0.3143 = 1.1900
$$
...
5. **Critical clearing time formula**: The formula $t_("cr") = sqrt(frac(2 H(delta_("cr") - delta_0), pi f P_i))$ assumes $P_e = 0$ during the fault. This is only valid for a fault at the generator terminals.
...
- During a fault with $P_e = 0$, the rotor angle follows $delta = frac(pi f P_i, 2 H) t^2 + delta_0$
- The critical clearing time is $t_("cr") = sqrt(frac(2 H(delta_("cr") - delta_0), pi f P_i))$
- For faults on transmission lines, three power-angle curves exist: pre-fault, during-fault, and post-fault
- The general critical clearing angle formula is $cos delta_("cr") = frac(1, (K_2 - K_1))[(delta_("max") - delta_0) sin delta_0 + K_2 cos delta_("max") - K_1 cos delta_0]$
...
- Curve B (during fault): $P_e = K_1 dot P_("max") sin delta$ where $K_1 < K_2$
- Curve C (post-fault): $P_e = K_2 dot P_("max") sin delta$ where $K_2 > K_1$

The physical meaning of the constants:
- $K_1 = frac(P_(max,"during fault"), P_(max,"before fault"))$
- $K_2 = frac(P_(max,"after fault"), P_(max,"before fault"))$
...
| Formula | Description |
|---------|-------------|
| $"KE" = frac(1, 2) J omega_(s-"mech")^2 times 10^(-6)$ | Rotor kinetic energy (MJ) |
| $M = J(frac(2, P))^2 omega_(s-"elect") times 10^(-6)$ | Angular momentum (MJ-sec/elect-rad) |
| $G times H = "KE"$ | Inertia constant definition |
| $M = frac("GH", pi f)$ | M in MJ-sec/elect-radian |
| $M = frac("GH", 180 f)$ | M in MJ-sec/elect-degree |
| $M_("pu") = frac(H, pi f)$ | Per-unit inertia constant (radian) |
| $M_("pu") = frac(H, 180 f)$ | Per-unit inertia constant (degree) |
| $frac(H, pi f) frac(d^2 delta, dif t^2) = P_i - P_e$ | Swing equation (per unit) |
| $P_S = P_R = frac(|V_S||V_R|, X) sin delta$ | Lossless line power |
| $P_e = frac(|E_g||V_t|, x_d) sin delta$ | Synchronous machine power |
| $integral_(delta_0)^(delta) P_a d delta = 0$ | Equal area criterion |
...
| $t_c = sqrt(frac(2 H(delta_c - delta_0), pi f P_i))$ | Clearing time |
| $t_("cr") = sqrt(frac(2 H(delta_("cr") - delta_0), pi f P_i))$ | Critical clearing time |
...
$$
delta_0 = sin^(-1)(0.5333) = 32.23° = 0.562 " rad"
$$
...
$$
delta_m^1 = sin^(-1)(0.6667) = 41.81° = 0.730 " rad"
$$

The maximum angle:

$$
delta_("max") = pi - delta_m^1 = pi - 0.730 = 2.412 " rad"
$$
...
$$
cos delta_("cr") = frac(1, (0.8 - 0.2667))[(2.412 - 0.562) times 0.5333 + 0.8 cos(2.412) - 0.2667 cos(0.562)]
$$

$$
cos delta_("cr") = frac(1, 0.5333)[1.850 times 0.5333 + 0.8 times (-0.7431) - 0.2667 times 0.8462]
$$

$$
cos delta_("cr") = frac(1, 0.5333)[0.9866 - 0.5945 - 0.2257]
$$

$$
cos delta_("cr") = frac(1, 0.5333) times 0.1664 = 0.3120
$$

$$
delta_("cr") = cos^(-1)(0.3120) = 71.8° = 1.253 " rad"
$$
...
$$
t_("cr") = sqrt(frac(2 times 5 times (1.253 - 0.562), pi times 60 times 0.8))
$$

$$
t_("cr") = sqrt(frac(10 times 0.691, 150.80))
$$

$$
t_("cr") = sqrt(frac(6.91, 150.80)) = sqrt(0.0458) = 0.214 " s"
$$
...
6. **Critical clearing time formula**: Only valid when $P_e = 0$ during the fault. For faults where $P_e != 0$ during the fault, a more general formula is needed.
...
![Inertia constant and swing-equation coefficients](/content-assets/studies/power-system-analysis/Week%2012/Assets/img-6.jpg)
...
![Equivalent inertia formula and numerical example](/content-assets/studies/power-system-analysis/Week%2012/Assets/img-15.jpg)
...
![Infinite-bus Thevenin reduction](/content-assets/studies/power-system-analysis/Week%2012/Assets/img-28.jpg)
...
![Stable and unstable rotor-angle responses](/content-assets/studies/power-system-analysis/Week%2012/Assets/img-35.jpg)
...
![Power-angle characteristic and allowable swing](/content-assets/studies/power-system-analysis/Week%2012/Assets/img-45.jpg)
...
![Critical-clearing-angle derivation](/content-assets/studies/power-system-analysis/Week%2012/Assets/img-52.jpg)
...
![Equal-area curves before, during, and after a fault](/content-assets/studies/power-system-analysis/Week%2012/Assets/img-58.jpg)
...
![Sample power-system network for a stability example](/content-assets/studies/power-system-analysis/Week%2012/Assets/img-71.jpg)
...
5. **Using the wrong formula for M**: $M = "GH"/(pi f)$ for radians, $M = "GH"/(180 f)$ for degrees. Mixing these up gives incorrect results.
...
| Angular momentum M | $M = "GH"/(pi f)$ (radians) or $M = "GH"/(180 f)$ (degrees) | MJ-sec/elect-rad or MJ-sec/elect-degree |
...
**Swing Equation:**
$$
frac(H, pi f) frac(d^2 delta, dif t^2) = P_i - P_e " (per unit)"
$$
...
6. **Compute critical clearing time**: $t_("cr") = sqrt(2 H(delta_("cr") - delta_0)/(pi f P_i))$
...
Options:
(a) $frac(H, pi f) frac(d^2 delta, dif t^2) = P_i - P_e$
(b) $frac("GH", pi f) frac(d^2 delta, dif t^2) = P_i - P_e$
(c) $M frac(d^2 delta, dif t^2) = P_i + P_e$
(d) $frac(H, 180 f) frac(d^2 delta, dif t^2) = P_i - P_e$ (with δ in radians)

> Answer and explanation
> The correct answer is (a). The swing equation in per unit is $frac(H, pi f) frac(d^2 delta, dif t^2) = P_i - P_e$, where H is in MJ/MVA, f is in Hz, δ is in electrical radians, and powers are in per unit. Option (b) has an extra G factor (which would give MW units). Option (c) has the wrong sign. Option (d) uses 180f which is for δ in degrees, not radians.
...
Options:
(a) $delta = delta_0 + frac(pi f P_i, 2 H) t^2$
(b) $delta = delta_0 + frac(pi f P_i, H) t$
(c) $delta = delta_0 + frac(2 H, pi f P_i) t^2$
(d) $delta = delta_0 e^(-frac(pi f P_i, H) t)$

> Answer and explanation
> The correct answer is (a). During a three-phase fault at the generator terminals, $P_e = 0$, so the swing equation becomes $frac(d^2 delta, dif t^2) = frac(pi f, H) P_i$. Integrating twice with initial conditions $delta(0) = delta_0$ and $frac(d delta, dif t)(0) = 0$ gives $delta = delta_0 + frac(pi f P_i, 2 H) t^2$. This is a constant acceleration motion.
...
> Answer and explanation
> The correct answers are (a), (b), and (c). The critical clearing time is $t_("cr") = sqrt(frac(2 H(delta_("cr") - delta_0), pi f P_i))$. It depends on the inertia constant H, the system frequency f, and the mechanical input power P_i. The circuit breaker rated current does not appear in the formula — it is a protection equipment rating, not a system parameter affecting the critical clearing time.
...
> Answer and explanation
> Given: Initial power $P_(i 0) = 0.35 P_("max")$, load doubled to $P_i = 0.7 P_("max")$.
>
> Step 1: Find the initial angle:
$$
delta_0 = sin^(-1)(0.35) = 20.49° = 0.357 " rad"
$$
>
> Step 2: Find the angle after load doubling:
$$
delta_1 = sin^(-1)(0.7) = 44.43° = 0.775 " rad"
$$
>
...
$$
$$
(delta_2 - 0.357) times 0.7 + cos delta_2 - cos(0.357) = 0
$$
$$
0.7 delta_2 - 0.2499 + cos delta_2 - 0.9369 = 0
$$
...
> Try $delta_2 = 1.25$ rad: $0.7 times 1.25 + cos(1.25) = 0.875 + 0.3153 = 1.1903$ (too high)
> Try $delta_2 = 1.252$ rad: $0.7 times 1.252 + cos(1.252) = 0.8764 + 0.3132 = 1.1896$ (too low)
> Try $delta_2 = 1.251$ rad: $0.7 times 1.251 + cos(1.251) = 0.8757 + 0.3143 = 1.1900$ (too high)
>
