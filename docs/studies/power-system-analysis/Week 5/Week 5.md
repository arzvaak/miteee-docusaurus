...
**Shunt Capacitors:**
- **Primary Objective:** Reduce power loss. By supplying the lagging reactive power locally, the current flowing through the line decreases, thereby reducing the $I^2 R$ losses.
...
**Series Capacitors:**
- **Primary Objective:** Improve voltage magnitude. By connecting a capacitor in series, its capacitive reactance ($-j X_C$) directly cancels a portion of the line's inductive reactance ($+j X_L$).
...
$$
Q_C = |I_C|^2 X_C = frac(|V_R|^2, X_C^2) dot X_C = frac(|V_R|^2, X_C)
$$
...
Let $Q_(C,"nominal") = |V_(R,"nominal")|^2 / X_C$. Then, at any other voltage $V_R$:
$$
Q_C = Q_(C,"nominal") times ( frac(|V_R|, |V_(R,"nominal")|) )^2
$$

**Worked Illustration:**
A capacitor is rated at 5 MVAR at 33 kV. If the bus voltage drops to 30 kV, the actual reactive power injected is:
$$
Q_C = 5 times ( frac(30, 33) )^2 = 5 times frac(100, 121) approx 4.13 " MVAR"
$$
...
$$
V_s = A V_R + B I_R space "(volts)" space dots.h (1)
$$
$$
I_s = C V_R + D I_R space "(Amp)" space dots.h (2)
$$

In matrix form:
$$
mat(V_s; I_s) = mat(A, B; C, D) mat(V_R; I_R) space dots.h (3)
$$
...
| $Z$ | Total series impedance: $Z = z dot l$ | Ω |
| $Y$ | Total shunt admittance: $Y = y dot l$ | S |
| $l$ | Line length | m |
...
**Model:**
Since there is no shunt admittance, the sending-end and receiving-end currents are identical ($I_S = I_R$). The line is simply a series impedance $Z = R + j X$.
...
$$
|V_S| cos(delta_S - delta_R) = |V_R| + |I|R cos delta_R + |I|X sin delta_R space dots.h (5)
$$
...
$$
|V_S| approx |V_R| + |I|(R cos delta_R + X sin delta_R) space dots.h (6)
$$
...
| Voltage drop is a limiting factor | Series capacitors are very effective | They directly reduce the $"IX"_L$ component of voltage drop |
...
- The approximate voltage drop equation $|V_S| approx |V_R| + |I|(R cos delta_R plus.minus X sin delta_R)$ is a powerful tool for quick calculations.
...
$$
"Regulation" = frac(|V_R^("NL")| - |V_R^("FL")|, |V_R^("FL")|) times 100 space dots.h (7)
$$
...
$$
V_R^("NL") = frac(V_S, A) space dots.h (8)
$$

Substituting this into the definition of regulation (Equation 7):
$$
"Regulation" = frac(|V_S/A| - |V_R^("FL")|, |V_R^("FL")|) times 100 = frac(|V_S| - |A||V_R^("FL")|, |A||V_R^("FL")|) times 100 space dots.h (9)
$$

**Voltage Regulation for a Short Line:**
For a short line, $A = 1$. Therefore, Equation (9) simplifies to:
$$
"Regulation" = frac(|V_S| - |V_R|, |V_R|) times 100 space dots.h (10)
$$
...
$$
"Percentage Voltage Regulation" = frac(|I|(R cos delta_R + X sin delta_R), |V_R|) times 100 space dots.h (11)
$$

**For leading power factor:**
$$
"Percentage Voltage Regulation" = frac(|I|(R cos delta_R - X sin delta_R), |V_R|) times 100 space dots.h (12)
$$
...
$$
I_L = I_R + frac(Y, 2) V_R space dots.h (13)
$$

**KVL around the loop:**
$$
V_S = V_R + Z I_L space dots.h (14)
$$

Substituting (13) into (14):
$$
V_S = V_R + Z ( I_R + frac(Y, 2) V_R ) = (1 + frac("ZY", 2)) V_R + Z I_R space dots.h (15)
$$

**KCL at the sending end:**
$$
I_S = I_L + frac(Y, 2) V_S space dots.h (16)
$$

Substituting (13) and (15) into (16):
$$
I_S = ( I_R + frac(Y, 2) V_R ) + frac(Y, 2) [ (1 + frac("ZY", 2)) V_R + Z I_R ]
$$
$$
I_S = Y(1 + frac("ZY", 4)) V_R + (1 + frac("ZY", 2)) I_R space dots.h (17)
$$
...
The series impedance of the segment is $z dot Delta x$, and the shunt admittance is $y dot Delta x$, where $z$ and $y$ are the per-unit-length impedance and admittance, respectively.

**Derivation of Differential Equations:**
**KVL for the segment:**
$$
V(x + Delta x) = z dot Delta x dot I(x) + V(x)
$$
Rearranging and taking the limit as $Delta x -> 0$:
$$
frac(d V(x), dif x) = z dot I(x) space dots.h (20)
$$

**KCL for the segment:**
$$
I(x + Delta x) = I(x) + y dot Delta x dot V(x + Delta x)
$$
Rearranging and taking the limit as $Delta x -> 0$:
$$
frac(d I(x), dif x) = y dot V(x) space dots.h (22)
$$

**The Wave Equation:**
Differentiating Equation (20) with respect to $x$ and substituting from Equation (22):
$$
frac(d^2 V(x), dif x^2) = z dot frac(d I(x), dif x) = z dot y dot V(x)
$$
$$
frac(d^2 V(x), dif x^2) - "zy" dot V(x) = 0 space dots.h (23)
$$

**Propagation Constant:**
We define the **propagation constant** $gamma$ such that:
$$
gamma^2 = "zy" space dots.h (24)
$$
$$
gamma = alpha + j beta = sqrt("zy") space dots.h (27)
$$
...
$$
V(x) = C_1 e^(gamma x) + C_2 e^(-gamma x) space dots.h (26)
$$

**Characteristic Impedance:**
From Equation (20), we can find the current:
$$
I(x) = frac(1, z) dot frac(d V(x), dif x) = frac(gamma, z)(C_1 e^(gamma x) - C_2 e^(-gamma x))
$$

Since $gamma = sqrt("zy")$, we have $gamma/z = sqrt(y/z) = 1/Z_C$, where $Z_C$ is the **characteristic impedance**:
$$
Z_C = sqrt(frac(z, y)) = sqrt(frac(Z, Y)) space dots.h (29)
$$

Therefore, the current is:
$$
I(x) = frac(1, Z_C)(C_1 e^(gamma x) - C_2 e^(-gamma x)) space dots.h (28)
$$

**Boundary Conditions:**
At the receiving end ($x = 0$), $V(0) = V_R$ and $I(0) = I_R$. Substituting into Equations (26) and (28):
$$
V_R = C_1 + C_2 space dots.h (30)
$$
$$
I_R = frac(1, Z_C)(C_1 - C_2) space dots.h (31)
$$

Solving for $C_1$ and $C_2$:
$$
C_1 = frac(V_R + Z_C I_R, 2) space dots.h (32)
$$
$$
C_2 = frac(V_R - Z_C I_R, 2) space dots.h (33)
$$

**Voltage and Current at Any Point:**
Substituting $C_1$ and $C_2$ back into Equations (26) and (28), we get:
$$
V(x) = frac((V_R + Z_C I_R), 2) e^(gamma x) + frac((V_R - Z_C I_R), 2) e^(-gamma x) space dots.h (34)
$$
$$
I(x) = frac((V_R + Z_C I_R), 2 Z_C) e^(gamma x) - frac((V_R - Z_C I_R), 2 Z_C) e^(-gamma x) space dots.h (35)
$$

These can be rearranged into a more elegant form using hyperbolic functions:
$$
V(x) = cosh(gamma x) V_R + Z_C sinh(gamma x) I_R space dots.h (38)
$$
$$
I(x) = frac(1, Z_C) sinh(gamma x) V_R + cosh(gamma x) I_R space dots.h (39)
$$

**Sending-End Relations:**
At the sending end ($x = l$), $V(l) = V_S$ and $I(l) = I_S$. Therefore:
$$
V_S = cosh(gamma l) V_R + Z_C sinh(gamma l) I_R space dots.h (40)
$$
$$
I_S = frac(1, Z_C) sinh(gamma l) V_R + cosh(gamma l) I_R space dots.h (41)
$$
...
- For a short line, regulation is $frac(|V_S| - |V_R|, |V_R|) times 100$, which depends on the load power factor.
...
- Series impedance: $Z' = Z dot frac(sinh(gamma l), gamma l)$
- Shunt admittance (each half): $frac(Y', 2) = frac(Y, 2) dot frac(tanh(gamma l/2), (gamma l/2))$
...
$$
Z' = frac(Z, gamma l) dot sinh(gamma l) = Z dot frac(sinh(gamma l), gamma l) space dots.h (47)
$$
...
Using the hyperbolic identity $tanh(gamma l/2) = frac(cosh(gamma l) - 1, sinh(gamma l))$, and substituting $Z' = Z_C sinh(gamma l)$:
$$
frac(Y', 2) = frac(cosh(gamma l) - 1, Z_C sinh(gamma l)) = frac(1, Z_C) dot frac(cosh(gamma l) - 1, sinh(gamma l)) = frac(1, Z_C) tanh(frac(gamma l, 2))
$$

Since $1/Z_C = Y/(gamma l)$, we get:
$$
frac(Y', 2) = frac(Y, gamma l) dot tanh(frac(gamma l, 2)) = frac(Y, 2) dot frac(tanh(gamma l/2), (gamma l/2)) space dots.h (48)
$$

**Key Observation:** The correction factors $frac(sinh(gamma l), gamma l)$ and $frac(tanh(gamma l/2), (gamma l/2))$ are approximately equal to unity for lines up to a few hundred kilometers. This is why the nominal π model is often accurate enough. However, for very long lines, these corrections become significant.
...
1. **Line Constants:**
   $$
R = 0.0195 times 20 = 0.39 Omega
$$
   $$
X = 2 pi times 60 times 0.60 times 10^(-3) times 20 = 4.52 Omega
$$

2. **Load Current:**
   $$
|I| = frac(P, V_R cos delta_R) = frac(4500 times 10^3, 10.2 times 10^3 times 0.8) = 551.47 "A"
$$
...
$$
   $$
|V_S| = 10200 + 551.47(0.39 times 0.8 + 4.52 times 0.6)
$$
   $$
|V_S| = 10200 + 551.47(0.312 + 2.712) = 10200 + 1667.6 = 11867.6 "V" = 11.867 "kV"
$$

4. **Voltage Regulation:**
   $$
"Regulation" = frac(|V_S| - |V_R|, |V_R|) times 100 = frac(11.867 - 10.2, 10.2) times 100 = 16.34%
$$

**Part (b):**
1. **Target Regulation:**
   $$
"Regulation"_("new") = 16.34% times 0.6 = 9.804%
$$

2. **Required Sending-End Voltage:**
   $$
frac(|V_S| - 10.2, 10.2) = 0.09804 => |V_S| = 11.2 "kV"
$$
...
$$
   $$
11200 - 10200 = |I_R|(0.39 cos delta_R^1 + 4.52 sin delta_R^1) space dots.h (1)
$$
   ...
   $$
|I_R| = frac(4500 times 10^3, 10200 cos delta_R^1) space dots.h (2)
$$

4. **Solving for the New Power Factor:**
   Substituting (2) into (1):
   $$
1000 = frac(4500 times 10^3, 10200 cos delta_R^1) (0.39 cos delta_R^1 + 4.52 sin delta_R^1)
$$
   $$
1000 = frac(4500 times 10^3, 10200) (0.39 + 4.52 tan delta_R^1)
$$
   $$
1000 = 441.18 (0.39 + 4.52 tan delta_R^1)
$$
   $$
2.267 = 0.39 + 4.52 tan delta_R^1
$$
   $$
4.52 tan delta_R^1 = 1.877
$$
   $$
tan delta_R^1 = 0.415 => delta_R^1 = 22.5^degree
$$
   $$
...
5. **New Line Current:**
   $$
|I_R| = frac(4500 times 10^3, 10200 times 0.9238) = 477.56 "A"
$$

6. **Capacitor Current:**
   We can find the capacitor current using phasors.
   - Load current: $I = 551.47 angle -cos^(-1)(0.8) = 551.47 angle -36.87^degree "A"$
   - Line current: $I_R = 477.56 angle -22.5^degree "A"$
   - Capacitor current: $I_C = I - I_R = 551.47 angle -36.87^degree - 477.56 angle -22.5^degree$
   - Converting to rectangular form:
     - $I = 441.18 - j 330.88$
     - $I_R = 441.18 - j 182.78$
     - $I_C = 0 - j 148.10 = j 148.13 "A"$ (leading $V_R$ by 90°)
...
$$
   Since $X_C = 1/(omega C)$:
   $$
C = frac(1, 2 pi times 60 times 68.86) = 38.5 mu"F"
$$
...
1. **Case (a) - Without Capacitor:**
   ...
   $$
eta = frac(P_("out"), P_("out") + P_("loss")) = frac(4500, 4500 + (551.47)^2 times 0.39 times 10^(-3)) = frac(4500, 4500 + 118.6) = 97.43%
$$

2. **Case (b) - With Capacitor:**
   $$
eta = frac(4500, 4500 + (477.56)^2 times 0.39 times 10^(-3)) = frac(4500, 4500 + 88.9) = 98.06%
$$
...
$$
Q_L = P_L tan phi = 4500 times frac(0.6, 0.8) = 4500 times 0.75 = 3375 "kVAR"
$$

**Combined reactive power (load + capacitor):**
$$
Q_L^1 = 4500 times tan delta_R^1 = 4500 times 0.415 = 1867.7 "kVAR"
$$

**Capacitor reactive power:**
$$
Q_C = Q_L - Q_L^1 = 3375 - 1867.7 = 1507.3 "kVAR"
$$

**Verification using $Q_C = I_C^2 X_C$:**
$$
X_C = frac(Q_C, I_C^2) = frac(1507.3 times 10^3, (148.13)^2) = 68.7 Omega
$$
...
- The correction factors $sinh(gamma l)/(gamma l)$ and $tanh(gamma l/2)/(gamma l/2)$ account for the distributed nature of the line.
...
$$
R = 0.15 times 60 = 9 Omega
$$
$$
X = 2 pi times 50 times 1.4 times 10^(-3) times 60 = 26.39 Omega
$$
...
1. **Receiving End Voltage (per phase, reference):**
   $$
V_R = frac(220, sqrt(3)) angle 0^degree = 127 angle 0^degree "kV"
$$

2. **Power Factor Angle:**
   $$
cos phi = 0.8 => phi = 36.87^degree
$$

3. **Complex Power (3-phase):**
   For a lagging load, $Q > 0$.
   $$
S = 300 angle 36.87^degree "MVA" = 240 + j 180 "MVA"
$$
...
4. **Receiving End Current:**
   Using the formula $I_R = frac(S^(star), 3 V_R^(star))$:
   $$
I_R = frac(300 times 10^6 angle -36.87^degree, 3 times 127 times 10^3 angle 0^degree) = 787.4 angle -36.87^degree "A"
$$
   ...
   **Worked mini-example:**
   - $V = 50 angle 15^degree$ V
   - $I = 5 angle -30^degree$ A
   - Angle between V and I: $15^degree + 30^degree = 45^degree$
   - Using $P - j Q = V^(star)I$:
     - $V^(star) = 50 angle -15^degree$
     - $V^(star)I = (50 angle -15^degree)(5 angle -30^degree) = 250 angle -45^degree$
     - $P = 250 cos 45^degree = frac(250, sqrt(2))$ W
     - $Q = 250 sin 45^degree = frac(250, sqrt(2))$ VAR
   ...
   **Derivation chain:**
   - $S = P + j Q = "VI"^(star)$
   - Taking conjugate: $S^(star) = P - j Q = V^(star)I$
   - Therefore: $I = frac(S^(star), V^(star))$
...
$$
   $$
|V_S| = 127 + 0.7874(9 times 0.8 + 26.39 times 0.6) = 127 + 0.7874(7.2 + 15.83)
$$
   $$
|V_S| = 127 + 18.13 = 145.13 "kV (phase)"
$$
   **Line-to-line:**
   $$
|V_S|_(L-L) = sqrt(3) times 145.13 = 251.37 "kV"
$$

6. **Voltage Regulation:**
   $$
"VR" = frac(|V_S| - |V_R|, |V_R|) times 100 = frac(251.37 - 220, 220) times 100 = 14.26%
$$
...
7. **Power Loss and Efficiency:**
   - Per-phase real power loss: $P_("Loss") = |I|^2 R = (787.4)^2 times 9 times 10^(-6) = 5.58 "MW"$
   - Per-phase receiving end power: $P_R = frac(300, 3) times 0.8 = 80 "MW"$
   - Per-phase sending end power: $P_S = P_R + P_("Loss") = 80 + 5.58 = 85.58 "MW"$
   - Efficiency: $eta = frac(P_R, P_S) times 100 = frac(80, 85.58) times 100 = 93.47%$
...
1. **Receiving End Current:**
   ...
   $$
I_R = 787.4 angle +36.87^degree "A"
$$
...
$$
   $$
|V_S| = 127 - 6.79 = 120.2 "kV (phase)"
$$
   **Line-to-line:**
   $$
|V_S|_(L-L) = sqrt(3) times 120.2 = 208.2 "kV"
$$

3. **Voltage Regulation:**
   $$
"VR" = frac(208.2 - 220, 220) times 100 = -5.36%
$$
...
1. **Resistance:**
   $$
R = 0.075 times 150 = 11.25 Omega
$$

2. **Inductance:**
   Conductor radius: $r = 1.5/2 = 0.75 "cm" = 0.0075 "m"$.
   Using the formula from the inductance chapter:
   $$
L = 2 times 10^(-7) times ln(frac(d, r)) times "length"
$$
   $$
L = 2 times 10^(-7) times ln(frac(2, 0.0075)) times 150 times 1000 = 0.1675 "H"
$$

3. **Inductive Reactance:**
   $$
X = 2 pi f L = 2 pi times 50 times 0.1675 = 52.62 Omega
$$

4. **Capacitance:**
   Using the formula from the capacitance chapter:
   $$
C = frac(2 pi epsilon.alt_0, ln(d/r)) times "length"
$$
   $$
C = frac(2 pi times 8.854 times 10^(-12), ln(2/0.0075)) times 150 times 1000 = 1.49 mu"F"
$$

5. **Charging Admittance:**
   $$
Y = j omega C = j(2 pi times 50)(1.49 times 10^(-6)) = j 468 times 10^(-6) "mho"
$$
   $$
frac(Y, 2) = j 234 times 10^(-6) "mho"
$$

6. **Series Impedance:**
   $$
Z = R + j X = 11.25 + j 52.62 = 53.809 angle 77.9^degree Omega
$$

**Step 2: Receiving End Quantities**
1. **Load Current:**
   $$
I_R = frac(P, sqrt(3) V_R cos phi) = frac(20 times 10^6, sqrt(3) times 66 times 10^3 times 0.8) = 218.7 "A"
$$
   $$
I_R = 218.7 angle -36.86^degree "A"
$$

2. **Receiving End Phase Voltage:**
   $$
V_R = frac(66, sqrt(3)) angle 0^degree = 38.104 angle 0^degree "kV"
$$

**Step 3: Sending End Voltage Using Equation (15)**
$$
V_S = (1 + frac("ZY", 2)) V_R + "ZI"_R
$$

1. **Compute $frac("ZY", 2)$:**
   $$
frac("ZY", 2) = frac((11.25 + j 52.62)(j 468 times 10^(-6)), 2) = frac(-0.0246 + j 0.0053, 2) = -0.0123 + j 0.00264
$$

2. **Substitution:**
   $$
V_S = (1 - 0.0123 + j 0.00264)(38.104 angle 0^degree) + (53.809 angle 77.9^degree)(0.2187 angle -36.86^degree)
$$
   $$
V_S = (0.9877 + j 0.00264)(38.104) + (11.77 angle 41.04^degree)
$$
   $$
V_S = 37.63 + j 0.10 + 8.88 + j 7.73 = 46.51 + j 7.83
$$
   $$
V_S = 47.15 angle 9.54^degree "kV (phase)"
$$

3. **Line-to-line:**
   $$
V_(S,L-L) = sqrt(3) times 47.15 = 81.66 "kV"
$$

**Step 4: Voltage Regulation Using Equation (9)**
Since $A != 1$, we must use the general formula:
$$
"VR" = frac(frac(|V_S|, |A|) - |V_R|, |V_R|) times 100
$$

1. **Compute |A|:**
   $$
A = 1 + frac("ZY", 2) = 0.9877 + j 0.00264
$$
   $$
...
2. **Substitution:**
   $$
"VR" = frac(frac(81.66, 0.9877) - 66, 66) times 100 = frac(82.68 - 66, 66) times 100 = 25.26%
$$

**Step 5: Power Loss and Efficiency**
1. **Per-phase power loss:**
   $$
P_("Loss") = |I|^2 R = (218.7)^2 times 11.25 times 10^(-6) = 0.538 "MW"
$$

2. **Per-phase receiving end power:**
   $$
P_R = frac(20, 3) = 6.667 "MW"
$$

3. **Per-phase sending end power:**
   $$
P_S = P_R + P_("Loss") = 6.667 + 0.538 = 7.205 "MW"
$$

4. **Efficiency:**
   $$
eta = frac(P_R, P_S) times 100 = frac(6.667, 7.205) times 100 = 92.54%
$$

**Key Takeaways from Example 3:**
- The nominal π method is used for medium lines.
- Charging capacitance must be included.
- The voltage regulation formula must use $|V_S|/|A|$ when $A != 1$.
...
**Step 1: Line Parameters**
$$
R = 0.16 times 160 = 25.6 Omega
$$
$$
X = 2 pi times 50 times 1.2 times 10^(-3) times 160 = 60.3 Omega
$$
$$
Y = j(2 pi times 50)(0.0082 times 10^(-6) times 160) = j 4.12 times 10^(-4) "mho"
$$
$$
Z = R + j X = 25.6 + j 60.3 = 65.51 angle 67^degree Omega
$$

**Step 2: Receiving End Quantities**
1. **Phase Voltage:**
   $$
V_R = frac(132, sqrt(3)) angle 0^degree = 76.21 angle 0^degree "kV"
$$

2. **Receiving End Current:**
   $$
|I_R| = frac(100 times 10^6, sqrt(3) times 132 times 10^3) = 437.38 "A"
$$
   $$
I_R = 437.38 angle -36.87^degree "A"
$$

**Step 3: ABCD Parameters**
1. **Compute $frac("ZY", 2)$:**
   $$
frac("ZY", 2) = frac((65.51 angle 67^degree)(4.12 times 10^(-4) angle 90^degree), 2) = frac(0.027 angle 157^degree, 2) = -0.0124 + j 0.0053
$$

2. **Parameters:**
   - $A = D = 1 + frac("ZY", 2) = 0.9876 + j 0.0053$
   - $B = Z = 65.51 angle 67^degree Omega$
   - $C = Y(1 + frac("ZY", 4)) approx j 4.12 times 10^(-4) "mho"$
...
$$
$$
V_S = (0.9876 + j 0.0053)(76.21 angle 0^degree) + frac((65.51 angle 67^degree)(437.38 angle -36.87^degree), 1000)
$$
$$
V_S = 75.26 + j 0.40 + 28.66 angle 30.13^degree
$$
$$
V_S = 75.26 + j 0.40 + 24.78 + j 14.38 = 100.04 + j 14.78
$$
$$
V_S = 101.07 angle 8.18^degree "kV (phase)"
$$

**Line-to-line:**
$$
V_(S,L-L) = sqrt(3) times 101.07 = 175.05 "kV"
$$
...
$$
$$
I_S = (j 4.12 times 10^(-4))(76.21 angle 0^degree) + (0.9876 + j 0.0053)(437.38 angle -36.87^degree)
$$
$$
I_S = 0.0314 angle 90^degree + 431.9 angle -36.56^degree
$$
$$
I_S = j 0.0314 + 346.9 - j 257.2 = 346.9 - j 257.2
$$
$$
I_S = 431.9 angle -36.56^degree "A"
$$

**Step 6: Sending End Power Factor**
The angle between $V_S$ and $I_S$ is:
$$
theta = 8.18^degree - (-36.56^degree) = 44.74^degree
$$
$$
"PF" = cos(44.74^degree) = 0.71 "(lagging)"
$$
...
| Resistance | 25.3 Ω | Ω |
| Reactance | 66.5 Ω | Ω |
| Charging admittance | $0.442 times 10^(-3)$ | mho |
...
$$
$$
Y = j 0.442 times 10^(-3) "mho"
$$

**Step 2: Part (a) - ABCD Constants for Long Line**
1. **Propagation Constant times Length:**
   $$
gamma l = sqrt("ZY") = sqrt((25.3 + j 66.5)(j 0.442 times 10^(-3)))
$$
   $$
gamma l = sqrt(-0.0294 + j 0.0112) = 0.0327 + j 0.174
$$

   **Derivation note from lecturer:**
   > "$gamma l$ actually $sqrt("ZY")$ in this is a, this is actually small small z right. So, small $"zy" * l$ is equal to you can write root over it is actually inside if you bring $l^2$. So, $"zl" * gamma l$ that is actually $gamma l = sqrt(c a pi t a l Z c a pi t a l Y)$"

2. **ABCD Parameters:**
   - $A = D = cosh(gamma l) = cosh(0.0327 + j 0.174) = 0.986 angle 0.32^degree$
   - $Z_C = sqrt(frac(Z, Y)) = sqrt(frac(25.3 + j 66.5, j 0.442 times 10^(-3))) = 393 - j 72.3 Omega$
   - $B = Z_C sinh(gamma l) = 70.3 angle 69.2^degree Omega$
   - $C = frac(1, Z_C) sinh(gamma l) = j 0.44 times 10^(-4) "mho"$

**Step 3: Part (b) - Sending End Voltage, Current, and Power Factor**
1. **Load Current:**
   $$
|I_R| = frac(60 times 10^6, sqrt(3) times 124 times 10^3) = 279.36 "A"
$$
   $$
I_R = 279.36 angle -36.87^degree "A"
$$

2. **Receiving End Phase Voltage:**
   $$
V_R = frac(124, sqrt(3)) angle 0^degree = 71.6 angle 0^degree "kV"
$$
...
$$
   $$
V_S = (0.986 angle 0.32^degree)(71.6 angle 0^degree) + frac((70.3 angle 69.2^degree)(279.36 angle -36.87^degree), 1000)
$$
   $$
V_S = 70.6 angle 0.32^degree + 19.64 angle 32.33^degree
$$
   $$
V_S = 70.6 + j 0.39 + 16.59 + j 10.50 = 87.19 + j 10.89
$$
   $$
V_S = 87.84 angle 7.1^degree "kV (phase)"
$$

   **Line-to-line:**
   $$
V_(S,L-L) = sqrt(3) times 87.84 = 152.14 "kV"
$$
...
$$
   $$
I_S = (j 0.44 times 10^(-4))(71.6 angle 0^degree) + (0.986 angle 0.32^degree)(279.36 angle -36.87^degree)
$$
   $$
I_S = 0.00315 angle 90^degree + 275.4 angle -36.55^degree
$$
   $$
I_S = j 0.00315 + 221.2 - j 164.1 = 221.2 - j 164.1
$$
   $$
I_S = 257.78 angle -30.86^degree "A"
$$

5. **Sending End Power Factor:**
   Angle between $V_S$ and $I_S$: $7.1^degree + 30.86^degree = 37.96^degree$.
   $$
"PF" = cos(37.96^degree) = 0.788 "(lagging)"
$$

**Step 4: Part (c) - Regulation**
1. **Sending End Power:**
   $$
P_S = sqrt(3) times 152.14 times frac(257.78, 1000) times 0.788 = 53.52 "MW"
$$

2. **Receiving End Power:**
   $$
P_R = 60 times 0.8 = 48 "MW"
$$

3. **Voltage Regulation:**
   $$
"VR" = frac(frac(|V_S|, |A|) - |V_R|, |V_R|) times 100 = frac(frac(152.14, 0.986) - 124, 124) times 100 = frac(154.3 - 124, 124) times 100 = 24.43%
$$

**Step 5: Part (d) - Efficiency**
$$
eta = frac(P_R, P_S) times 100 = frac(48, 53.52) times 100 = 89.68%
$$
...
**Step 1: Given Parameters**
$$
Z = 33 + j 104 = 109.11 angle 72.4^degree Omega
$$
$$
Y = j 10^(-3) "mho"
$$
...
$$
I_R = frac(50 times 10^6, sqrt(3) times 208 times 10^3 times 0.8) angle -36.87^degree = 0.173 angle -36.87^degree "kA"
$$

**Receiving end phase voltage:**
$$
V_R = frac(208, sqrt(3)) angle 0^degree = 120.08 angle 0^degree "kV"
$$

**Step 2: Part (a) - Short Line Approximation**
1. **Sending End Voltage:**
   $$
V_S = V_R + I_"RZ" = 120.08 angle 0^degree + (0.173 angle -36.87^degree)(109.11 angle 72.4^degree)
$$
   $$
V_S = 120.08 + 18.88 angle 35.53^degree = 120.08 + 15.36 + j 10.98 = 135.44 + j 10.98
$$
   $$
V_S = 135.87 angle 4.62^degree "kV (phase)"
$$

   **Line-to-line:**
   $$
V_(S,L-L) = sqrt(3) times 135.87 = 235.33 "kV"
$$

2. **Sending End Current:**
   $$
I_S = I_R = 0.173 angle -36.87^degree "kA"
$$

3. **Power Factor:**
   Angle between $V_S$ and $I_S$: $4.62^degree + 36.87^degree = 41.49^degree$.
   $$
"PF" = cos(41.49^degree) = 0.75 "(lagging)"
$$

4. **Sending End Power:**
   $$
P_S = sqrt(3) times 235.33 times 0.173 times 0.75 = 52.88 "MW"
$$

**Step 3: Part (b) - Nominal π Method**
1. **ABCD Parameters:**
   - $A = D = 1 + frac("YZ", 2) = 1 + frac((j 10^(-3))(109.11 angle 72.4^degree), 2) = 1 + 0.0545 angle 162.4^degree = 1 - 0.0519 + j 0.0165 = 0.9481 angle 1^degree$
   - $B = Z = 109.11 angle 72.4^degree$
   - $C = Y(1 + frac("YZ", 4)) approx j 10^(-3)$

2. **Sending End Voltage:**
   $$
V_S = "AV"_R + "BI"_R = (0.9481 angle 1^degree)(120.08 angle 0^degree) + (109.11 angle 72.4^degree)(0.173 angle -36.87^degree)
$$
   $$
V_S = 113.85 angle 1^degree + 18.88 angle 35.53^degree = 113.83 + j 1.99 + 15.36 + j 10.98 = 129.19 + j 12.97
$$
   $$
V_S = 129.84 angle 5.72^degree "kV (phase)"
$$

   **Line-to-line:**
   $$
V_(S,L-L) = sqrt(3) times 129.84 = 224.85 "kV"
$$

3. **Sending End Current:**
   $$
I_S = "CV"_R + "DI"_R = (j 10^(-3))(120.08 angle 0^degree) + (0.9481 angle 1^degree)(0.173 angle -36.87^degree)
$$
   $$
I_S = 0.120 angle 90^degree + 0.164 angle -35.87^degree = j 0.120 + 0.133 - j 0.096 = 0.133 + j 0.024
$$
   $$
I_S = 0.135 angle 10.23^degree "kA"
$$

4. **Power Factor:**
   Angle between $V_S$ and $I_S$: $10.23^degree - 5.72^degree = 4.51^degree$.
   $$
"PF" = cos(4.51^degree) = 0.997 "(leading)"
$$
...
5. **Sending End Power:**
   $$
P_S = sqrt(3) times 224.85 times 0.135 times 0.997 = 52.4 "MW"
$$

**Step 4: Part (c) - Exact Transmission Line Equations**
1. **Propagation Constant and Characteristic Impedance:**
   $$
gamma l = sqrt("ZY") = sqrt((109.11 angle 72.4^degree)(10^(-3) angle 90^degree)) = sqrt(0.1091 angle 162.4^degree) = 0.33 angle 81.2^degree
$$
   $$
Z_C = sqrt(frac(Z, Y)) = sqrt(frac(109.11 angle 72.4^degree, 10^(-3) angle 90^degree)) = sqrt(109110 angle -17.6^degree) = 330.31 angle -8.8^degree
$$

2. **ABCD Parameters:**
   - $A = D = cosh(gamma l) = cosh(0.33 angle 81.2^degree) = 0.9481 angle 1^degree$
   - $sinh(gamma l) = 0.33 angle 81.2^degree$
   - $B = Z_C sinh(gamma l) = (330.31 angle -8.8^degree)(0.33 angle 81.2^degree) = 109 angle 72.4^degree$
   - $C = frac(sinh(gamma l), Z_C) = frac(0.33 angle 81.2^degree, 330.31 angle -8.8^degree) approx j 10^(-3)$

3. **Sending End Voltage:**
   $$
V_S = "AV"_R + "BI"_R = 129.81 angle 5.72^degree "kV (phase)"
$$
   **Line-to-line:**
   $$
V_(S,L-L) = sqrt(3) times 129.81 = 224.83 "kV"
$$

4. **Sending End Current:**
   $$
I_S = "CV"_R + "DI"_R = 0.135 angle 10.23^degree "kA"
$$

5. **Power Factor:**
   $$
"PF" = cos(10.23^degree - 5.72^degree) = 0.997 "(leading)"
$$

6. **Sending End Power:**
   $$
P_S = sqrt(3) times 224.83 times 0.135 times 0.997 = 52.4 "MW"
$$
...
$$
V(x) = C_1 e^(alpha x) e^(j beta x) + C_2 e^(-alpha x) e^(-j beta x) space dots.h (49)
$$
...
$$
v(t,x) = sqrt(2) upright(upright("Re"))(C_1 e^(alpha x) e^(j(omega t + beta x))) + sqrt(2) upright(upright("Re"))(C_2 e^(-alpha x) e^(j(omega t - beta x))) space dots.h (50)
$$
...
$$
V_1(t,x) = sqrt(2) C_1 e^(alpha x) cos(omega t + beta x) space dots.h (52)
$$
$$
V_2(t,x) = sqrt(2) C_2 e^(-alpha x) cos(omega t - beta x) space dots.h (53)
$$
...
To find the velocity of the reflected wave, we track a point of constant phase, e.g., the peak. The condition for a peak is $omega t - beta x = 2 k pi$. Solving for $x$:
$$
x = frac(omega, beta) t - frac(2 k pi, beta) space dots.h (54)
$$

Taking the derivative with respect to time gives the velocity:
$$
v = frac(dif x, dif t) = frac(omega, beta) = frac(2 pi f, beta) space dots.h (56)
$$
...
![Short-line circuit and phasor diagrams with and without shunt compensation](/content-assets/studies/power-system-analysis/Week%205/Assets/img-2.jpg)
...
![Shunt-capacitor reactive-power derivation](/content-assets/studies/power-system-analysis/Week%205/Assets/img-3.jpg)
...
![Nominal-pi model for a medium transmission line](/content-assets/studies/power-system-analysis/Week%205/Assets/img-16.jpg)
...
![Differential segment of a distributed-parameter transmission line](/content-assets/studies/power-system-analysis/Week%205/Assets/img-21.jpg)
...
![Exact equivalent-pi model for a long transmission line](/content-assets/studies/power-system-analysis/Week%205/Assets/img-32.jpg)
...
![Nominal-pi voltage and current equations](/content-assets/studies/power-system-analysis/Week%205/Assets/img-61.jpg)
...
![Long-line ABCD parameter calculation](/content-assets/studies/power-system-analysis/Week%205/Assets/img-64.jpg)
...
![Forward and backward voltage waves on a transmission line](/content-assets/studies/power-system-analysis/Week%205/Assets/img-77.jpg)
...
1. **Using Load Current Instead of Line Current:** When a shunt capacitor is connected, the current flowing through the line ($I_R$) is different from the load current ($I$). Always use the line current in the voltage drop equation $|V_S| = |V_R| + |I|(R cos delta_R plus.minus X sin delta_R)$.
...
4. **Using the Wrong Regulation Formula:** For short lines, $A=1$, so $"VR" = (|V_S| - |V_R|)/|V_R| times 100$. For medium and long lines, $A != 1$, so you must use the general formula $"VR" = (|V_S|/|A| - |V_R|)/|V_R| times 100$.
5. **Incorrect Complex Power Convention:** For a lagging load, the reactive power $Q$ is positive ($S = P + j Q$). For a leading load, $Q$ is negative ($S = P - j Q$). This is the opposite of the generator convention.
6. **Unit Inconsistency:** When using the formula $I_R = S^(star)/(3 V_R^(star))$, ensure $S$ is in VA, $V_R$ is in V, and $I_R$ will be in A. When voltages are in kV and currents in A, be careful with the factor of 1000 in calculations.
...
12. **Conjugate in current formula:** $I_R = frac(S^(star), V_R^(star))$ — the conjugate captures the power factor angle.
...
- $Z' = Z dot frac(sinh(gamma l), gamma l)$
- $frac(Y', 2) = frac(Y, 2) dot frac(tanh(gamma l/2), (gamma l/2))$

**5. Voltage Regulation**
- General: $"Regulation" = frac(|V_R^("NL")| - |V_R^("FL")|, |V_R^("FL")|) times 100$
- Using ABCD: $"Regulation" = frac(|V_S| - |A||V_R^("FL")|, |A||V_R^("FL")|) times 100$
- Short Line (Lagging pf): $frac(|I|(R cos delta_R + X sin delta_R), |V_R|) times 100$
- Short Line (Leading pf): $frac(|I|(R cos delta_R - X sin delta_R), |V_R|) times 100$

**6. Efficiency**
- $eta = frac(P_("out"), P_("out") + P_("loss")) = frac(P_R, P_S) times 100$
- Line loss: $P_("loss") = 3|I|^2 R$ (3-phase)
...
- Capacitor reactive power: $Q_C = omega C |V_R|^2 ∝ V^2$
...
> Answer and explanation
> The correct answer is (c). The reactive power injected by a capacitor is proportional to the square of the voltage: $Q_C = Q_(C,"nominal") times (V/V_("nominal"))^2$. Therefore, $Q_C = 10 times (30/33)^2 = 10 times (900/1089) approx 8.26 " MVAR"$.
...
**Q14. (Numerical) A 3-phase, 50 Hz, 100 km long transmission line has a total series impedance of $(10 + j 40) Omega$ and a total shunt admittance of $j 10^(-3)$ S. It delivers a load of 50 MW at 0.8 pf lagging and 132 kV. Using the nominal π model, calculate the sending-end voltage (line-to-line).**

> Answer and explanation
> 1. **Receiving end quantities:**
>    - $V_R = 132/sqrt(3) = 76.21 angle 0^degree " kV"$
>    - $I_R = frac(50 times 10^6, sqrt(3) times 132 times 10^3 times 0.8) = 273.36 " A" = 0.273 angle -36.87^degree " kA"$
> 2. **ABCD parameters:**
>    - $Z = 10 + j 40 = 41.23 angle 75.96^degree Omega$
>    - $Y = j 10^(-3) " S"$
>    - $A = 1 + frac("ZY", 2) = 1 + frac((10+j 40)(j 10^(-3)), 2) = 1 + frac(-0.04 + j 0.01, 2) = 0.98 + j 0.005$
> 3. **Sending end voltage (phase):**
>    - $V_S = A V_R + Z I_R$
>    - $V_S = (0.98 + j 0.005)(76.21) + (41.23 angle 75.96^degree)(0.273 angle -36.87^degree)$
>    - $V_S = 74.69 + j 0.38 + 11.26 angle 39.09^degree = 74.69 + j 0.38 + 8.74 + j 7.10 = 83.43 + j 7.48$
>    - $V_S = 83.76 angle 5.12^degree " kV"$
> 4. **Sending end voltage (line-to-line):**
>    - $V_(S,L-L) = sqrt(3) times 83.76 = 145.08 " kV"$
...
> Answer and explanation
> 1. **Magnitude of A:**
>    - $|A| = |0.98 + j 0.005| = sqrt(0.98^2 + 0.005^2) = 0.980$
> 2. **Voltage regulation:**
>    - $"VR" = frac(|V_S|/|A| - |V_R|, |V_R|) times 100$
>    - $"VR" = frac(145.08/0.980 - 132, 132) times 100 = frac(148.04 - 132, 132) times 100 = 12.15%$

---

**Q16. (Numerical) A long transmission line has a characteristic impedance $Z_C = 400 angle -10^degree Omega$ and $gamma l = 0.1 + j 0.5$. The receiving end is terminated with $V_R = 200 angle 0^degree " kV"$ (phase) and $I_R = 500 angle -30^degree " A"$. Calculate the sending-end voltage (phase).**

> Answer and explanation
> 1. **Calculate hyperbolic functions:**
>    - $gamma l = 0.1 + j 0.5$
>    - $cosh(gamma l) = cosh(0.1) cos(0.5) + j sinh(0.1) sin(0.5) = (1.005)(0.8776) + j(0.1002)(0.4794) = 0.882 + j 0.048$
>    - $sinh(gamma l) = sinh(0.1) cos(0.5) + j cosh(0.1) sin(0.5) = (0.1002)(0.8776) + j(1.005)(0.4794) = 0.088 + j 0.482$
> 2. **Calculate sending-end voltage:**
>    - $V_S = cosh(gamma l) V_R + Z_C sinh(gamma l) I_R$
>    - $V_S = (0.882 + j 0.048)(200) + (400 angle -10^degree)(0.5 angle -30^degree)$
>    - $V_S = 176.4 + j 9.6 + 200 angle -40^degree = 176.4 + j 9.6 + 153.2 - j 128.6$
>    - $V_S = 329.6 - j 119.0 = 350.4 angle -19.86^degree " kV"$
