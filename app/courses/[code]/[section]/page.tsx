...
export async function generateMetadata({ params }: SscCglSubjectPageProps) {
  ...
  const subject = code.toUpperCase() === "SSC-CGL" ? getSscCglSubjectDefinition(section) : null;
  if (!subject) return {};

  return buildPageMetadata({
    title: `${subject.shortTitle} Study Map - SSC CGL`,
    description: `${subject.description} Open the complete ${subject.shortTitle} table of contents, continue saved reading, and move into topic practice.`,
...
export default async function SscCglSubjectPage({ params }: SscCglSubjectPageProps) {
  ...
  const topicNotes: SscCglSubjectLandingNote[] = subjectTopics.map((topic) => {
    ...
    return {
      ...
      studyHref: note ? `/notes/${note.slug}` : `/exams/ssc-cgl/topics/${topic.slug}`,
  ...
  const notes = [...guideNotes, ...topicNotes];
    ...
    <>
      <JsonLd
        data={buildBreadcrumbJsonLd([
          { name: "Home", pathname: "/" },
          { name: "Subjects", pathname: "/courses" },
          { name: "SSC CGL", pathname: "/courses/SSC-CGL" },
