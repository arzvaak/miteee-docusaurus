"""OCR and author the Power System Analysis weekly notes."""
...
SOURCE_ROOT = Path(r"G:\Power System Analysis")
SOURCE_PDF = SOURCE_ROOT / "117105140.pdf"
LECTURE_INDEX = SOURCE_ROOT / "lecture-index.csv"
OCR_ROOT = ROOT / "data" / "psa-ocr"
DOCS_ROOT = ROOT / "docs" / "sem5" / "psa"
...
# One-based, inclusive physical ranges. The official PDF is ordered
# lexicographically (1, 10...19, 2, 20...), so weeks are assembled by lecture.
LECTURE_RANGES = {
    1: (5, 21), 2: (214, 230), 3: (441, 458), 4: (673, 688), 5: (869, 885),
    6: (1067, 1084), 7: (1108, 1126), 8: (1127, 1142), 9: (1143, 1164), 10: (22, 42),
    11: (43, 59), 12: (60, 77), 13: (78, 96), 14: (97, 113), 15: (114, 132),
    16: (133, 152), 17: (153, 173), 18: (174, 192), 19: (193, 213), 20: (231, 251),
    21: (252, 269), 22: (270, 292), 23: (293, 311), 24: (312, 335), 25: (336, 359),
    26: (360, 380), 27: (381, 401), 28: (402, 419), 29: (420, 440), 30: (459, 483),
    31: (484, 507), 32: (508, 535), 33: (536, 559), 34: (560, 577), 35: (578, 598),
    36: (599, 616), 37: (617, 634), 38: (635, 656), 39: (657, 672), 40: (689, 708),
    41: (709, 723), 42: (724, 747), 43: (748, 760), 44: (761, 773), 45: (774, 786),
    46: (787, 811), 47: (812, 830), 48: (831, 852), 49: (853, 868), 50: (886, 905),
    51: (906, 925), 52: (926, 941), 53: (942, 957), 54: (958, 976), 55: (977, 993),
    56: (994, 1010), 57: (1011, 1028), 58: (1029, 1049), 59: (1050, 1066), 60: (1085, 1107),
}

WEEK_LECTURES = {week: tuple(range((week - 1) * 5 + 1, week * 5 + 1)) for week in range(1, 13)}

WEEK_TITLES = {
    1: "Power-System Structure, Resistance, and Inductance",
    2: "Transmission-Line Inductance and Capacitance",
    3: "Capacitance, Components, and the Per-Unit System",
    4: "Power-System Components and Per-Unit Networks",
    5: "Transmission-Line Models and Performance",
    6: "Load-Flow Formulation and Iterative Solution",
    7: "Advanced Load-Flow Methods",
    8: "Load Flow and Optimal System Operation",
    9: "Economic Operation and Three-Phase Faults",
    10: "Three-Phase Faults and Symmetrical Components",
    11: "Symmetrical Components and Stability Foundations",
...
def split_week_pdf(reader: PdfReader, week: int, target: Path) -> None:
    writer = PdfWriter()
    for lecture in WEEK_LECTURES[week]:
        start, end = LECTURE_RANGES[lecture]
...
def save_ocr_response(response: dict[str, Any], week: int) -> dict[str, Any]:
    ...
    image_number = 0
    page_plan = [
        (lecture, physical_page)
        for lecture in WEEK_LECTURES[week]
        for physical_page in range(LECTURE_RANGES[lecture][0], LECTURE_RANGES[lecture][1] + 1)
    ]
    response_pages = response.get("pages") or []
    if len(response_pages) != len(page_plan):
        raise RuntimeError(
            f"Week {week}: Mistral returned {len(response_pages)} pages for a {len(page_plan)}-page source chunk"
        )
    for offset, page in enumerate(response_pages):
        lecture, pdf_page = page_plan[offset] if offset < len(page_plan) else (None, None)
        ...
        combined.append(f"<!-- Lecture {lecture}; physical PDF page {pdf_page} -->\n\n{markdown}")
    text = "\n\n".join(combined).strip() + "\n"
...
def run_ocr(force: bool = False, only_week: int | None = None) -> None:
    ...
    weeks = [only_week] if only_week else list(WEEK_LECTURES)
    for week in weeks:
        ...
        page_count = sum(
            LECTURE_RANGES[lecture][1] - LECTURE_RANGES[lecture][0] + 1
            for lecture in WEEK_LECTURES[week]
        )
    ...
    assignments: dict[int, list[str]] = {week: [] for week in WEEK_LECTURES}
...
def digest_messages(week: int, part: int, total: int, source: str) -> list[dict[str, str]]:
    system = (
        "You are an exacting power-system analysis lecturer. Work only from the supplied OCR. "
        "Preserve equations, network models, algorithms, examples, numerical iterations, and diagram references. "
    ...
    user = (
        ...
        "and every source question, exercise, worked example, and useful source-image reference exactly as written. This is an intermediate source packet, not "
        f"the final note.\n\nSOURCE OCR:\n{source}"
    )
...
def final_messages(
    ...
    page_provenance = "; ".join(
        f"Lecture {lecture}: physical PDF pages {LECTURE_RANGES[lecture][0]}-{LECTURE_RANGES[lecture][1]}"
        for lecture in WEEK_LECTURES[week]
    )
    ...
    requirements = [
        ...
        f"Use a specific description and YAML-list tags: power-system-analysis, load-flow, stability, week-{week}.",
        "Start with an orientation and detailed learning outcomes.",
        "Add a syllabus map naming all five lectures/topics in the week.",
        f"State this exact source-page provenance in the syllabus map: {page_provenance}.",
        "Write 12,000-16,000 words excluding frontmatter. Do not compress the lectures into a revision summary.",
        "Give each of the five lectures its own major section with physical intuition, complete theory, modelling assumptions, derivations, formula definitions and units, algorithms, practical engineering context, exam traps, and a lecture-end recap.",
        "Include at least four valid, focused Mermaid diagrams showing network modelling, analysis workflow, algorithm sequence, fault analysis, or stability relationships.",
        ...
        "Add Common mistakes and engineering checks, followed by a dense Quick revision sheet.",
        ...
        "Include every source question, exercise, and worked example that is materially recoverable; add a Source exercise coverage section recording any unreadable or omitted item.",
        "End with source provenance: NPTEL Power System Analysis, Prof. Debapriya Das, IIT Kharagpur; Mistral OCR 4 extraction; DeepSeek V4 Flash drafting; locally reviewed and generated on 2026-08-05. Do not call models authoritative sources.",
    ...
    system = (
        "You author publication-ready MIT EEE Power System Analysis notes in the student's first-person study voice. "
...
def worked_example_messages(week: int, start: int, count: int, digests: str) -> list[dict[str, str]]:
    system = (
        "You write rigorous worked examples for an undergraduate power-system analysis course. Use only the "
    ...
    user = (
        ...
        "Each example must state Given, Find, analysis principle or formula, a complete multi-step solution, "
...
def diagram_messages(week: int, count: int, digests: str) -> list[dict[str, str]]:
    system = (
        "You create technically accurate Mermaid diagrams for undergraduate power-system analysis notes. "
        "Use only source-supported relationships. Output Markdown only."
    )
    user = (
        f"Write a section titled `## Analysis Logic Diagrams` for Week {week}, containing exactly {count} "
        "distinct Mermaid diagrams. Use flowchart, sequenceDiagram, or stateDiagram-v2 as appropriate. Cover "
        "network modelling, load-flow algorithms, fault-analysis sequence, and stability reasoning "
...
def insert_before_revision(note: str, supplement: str) -> str:
    insertion = re.search(
        r"^## (?:Common Mistakes|Quick Revision|Practice Quiz|Source Provenance)\b",
        note,
        re.M | re.I,
...
def ensure_tables(note: str, week: int, digests: str, key: str) -> str:
    current = len(re.findall(r"^\s*\|?\s*:?-{3,}.*\|", note, re.M))
    if current >= 8:
        return note
    needed = 8 - current
    print(f"Week {week}: add {needed} evidence-based tables", flush=True)
    messages = [
        {
            "role": "system",
            "content": "You create concise, technically exact Markdown tables for undergraduate Power System Analysis notes. Use only the supplied evidence. Output Markdown only.",
        },
        {
            "role": "user",
            "content": (
                f"Write a section titled `## Additional Comparison and Revision Tables` containing exactly {needed} useful, non-duplicative Markdown tables for Week {week}. "
                "Each table needs a descriptive `###` heading and a short interpretation paragraph. Cover assumptions, formula meanings and units, method comparisons, convergence or modelling checks, fault/stability relationships, or exam traps as supported by the evidence. Do not add images, quizzes, provenance, or invented numerical claims.\n\n"
                f"EVIDENCE DIGESTS:\n{digests}"
            ),
        },
    ]
    supplement = deepseek(messages, key, 12000)
    supplement = re.sub(r"^```(?:markdown)?\s*", "", supplement.strip(), flags=re.I)
    supplement = re.sub(r"\s*```$", "", supplement).strip()
    return insert_before_revision(note, supplement)


def image_candidates(week: int, note: str, limit: int = 50) -> str:
    ocr = (OCR_ROOT / f"week-{week:02d}" / "ocr.md").read_text(encoding="utf-8")
    used = set(re.findall(r"!\[[^\]]*\]\((assets/week-[^)]+)\)", note))
    candidates: list[str] = []
    for match in re.finditer(r"!\[([^\]]*)\]\((assets/week-[^)]+)\)", ocr):
        link = match.group(2)
        if link in used:
            continue
        context = " ".join(ocr[max(0, match.start() - 240):match.end() + 300].split())
        candidates.append(f"- LINK: {link}\n  OCR CONTEXT: {context}")
        if len(candidates) >= limit:
            break
    return "\n".join(candidates)


def ensure_source_images(note: str, week: int, key: str) -> str:
    current = len(re.findall(r"!\[[^\]]*\]\([^)]+\)", note))
    if current >= 8:
        return note
    needed = 8 - current
    candidates = image_candidates(week, note)
    if not candidates:
        raise RuntimeError(f"Week {week}: no unused OCR image candidates available")
    print(f"Week {week}: add {needed} source visual walkthroughs", flush=True)
    messages = [
        {
            "role": "system",
            "content": "You select source visuals using Mistral vision-OCR context for Power System Analysis notes. Never invent labels or values that are absent from the context. Output Markdown only.",
        },
        {
            "role": "user",
            "content": (
                f"Write a section titled `## Additional Source Visual Walkthroughs` using exactly {needed} distinct candidate images for Week {week}. "
                "For each, add a descriptive `###` heading, embed the exact LINK with a useful alt text, state the lecture/physical-page provenance visible in the OCR context, and explain what the visual establishes and how to read it. If a label or value is uncertain, say so explicitly rather than guessing. Do not add tables, quiz questions, or provenance.\n\n"
                f"MISTRAL OCR IMAGE CANDIDATES:\n{candidates}"
            ),
        },
    ]
    supplement = deepseek(messages, key, 12000)
...
def run_author(
    ...
    weeks = [only_week] if only_week else [
        week for week in WEEK_LECTURES if start_week is None or week >= start_week
    ]
    for week in weeks:
...
def run_repair(only_week: int | None = None) -> None:
    ...
    for week in weeks:
        ...
        note = ensure_worked_examples(note, week, digests, key)
        note = ensure_mermaid_diagrams(note, week, digests, key)
        note = ensure_tables(note, week, digests, key)
        note = ensure_source_images(note, week, key)
        note = normalize_frontmatter(note, week)
...
def main() -> int:
    ...
    parser.add_argument("--start-week", type=int, choices=range(1, 13))
